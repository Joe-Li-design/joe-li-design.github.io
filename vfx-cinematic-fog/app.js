...
const controlSchema = [
  ...
  { id: "p1", label: "Scale", group: "Effect", min: 0.4, max: 1.8, step: 0.01, default: REFERENCE_PRESET.p1 },
  { id: "p2", label: "Layers", group: "Effect", min: 0.4, max: 1.8, step: 0.01, default: REFERENCE_PRESET.p2 },
  { id: "p3", label: "Drift", group: "Effect", min: 0.4, max: 1.8, step: 0.01, default: REFERENCE_PRESET.p3 },
  { id: "p4", label: "Depth", group: "Effect", min: 0.4, max: 1.8, step: 0.01, default: REFERENCE_PRESET.p4 },
];
const params = {};
...
const material = new THREE.ShaderMaterial({
  ...
  fragmentShader: `
    ...
    vec3 pal(float t){
      ...
      float drift = (u_time * 0.06 + sin(u_time * 0.23) * 0.10) * u_hueDrift;
      float hue = fract(u_hue/360.0 + drift + t*(0.06 + 0.14*richness));
      float sat = clamp(u_saturation * (0.82 + 0.30*richness), 0.3, 1.35);
    ...
    void main(){
      ...
      if(EFFECT_MODE == 0){
        vec2 q = uv;
        float p1c = clamp(u_p1, 0.12, 2.6);
        float p2c = clamp(u_p2, 0.12, 2.6);
        float p3c = clamp(u_p3, 0.12, 2.6);
        float p4c = clamp(u_p4, 0.12, 2.6);
        float heightFog = smoothstep(1.25, -0.45, q.y);
        float field = 0.0;
        float scatter = 0.0;
        float occlude = 0.0;
        float bands = 3.0 + floor(p2c * 2.0);
        for(int i=0;i<5;i++){
          float fi = float(i);
          if(fi >= bands) break;
          float layerT = fi / max(bands - 1.0, 1.0);
          float yBase = mix(-0.62, 0.46, layerT);
          float drift = sin(q.x*(1.5 + fi*0.45) + t*(0.16 + 0.08*fi)*p3c + fi*1.7) * (0.05 + 0.03*p3c);
          float fold = sin(q.x*(3.8 + fi*0.7) - t*(0.10 + 0.05*fi)*p4c + fi*2.3) * 0.03 * p4c;
          float sheetY = yBase + drift + fold;
          float thickness = mix(0.09, 0.22, layerT) * (0.85 + 0.18*p1c);
          float slab = exp(-abs(q.y - sheetY) / max(thickness, 0.0001));
          float veil = 0.65 + 0.35*sin(q.x*(6.0 + fi*1.6) - t*(0.34 + fi*0.08)*p3c + fi*1.2);
          float density = slab * veil;
          field += density * (0.42 + 0.24*layerT);
          float beam = exp(-abs(q.x - (-0.42 + fi*0.22 + sin(t*0.08 + fi)*0.08)) * (4.0 + 1.8*p4c));
          scatter += density * beam * (0.16 + 0.08*fi);
          float foregroundMask = smoothstep(0.28, -0.75, q.y - layerT*0.52);
          occlude += density * foregroundMask * (0.07 + 0.03*fi);
        }
        float lowRoll = exp(-abs(q.y + 0.72 - 0.05*sin(q.x*2.6 - t*0.12)) * (3.8 + 1.4*p1c));
        float vignetteFog = smoothstep(1.55*p4c, 0.22, length(q));
        float shaft = 0.0;
        for(int i=0;i<3;i++){
          float fi = float(i);
          float axis = -0.56 + fi*0.52 + sin(t*(0.07 + fi*0.03) + fi*1.6) * 0.07;
          float fan = max(0.0, 1.0 - abs((q.x - axis) * (1.15 + 0.25*fi) + (q.y + 0.52) * (0.52 + 0.11*fi)));
          float shaftBand = pow(fan, 2.4 + fi*0.8) * smoothstep(0.78, -0.15, q.y);
          shaft += shaftBand;
        }
        v = (field*0.58 + lowRoll*0.33 + scatter*0.38 + shaft*0.2) * heightFog * vignetteFog;
        v *= (1.0 - occlude*0.22);
        float hueField = field*0.08 + scatter*0.22 + shaft*0.18 + (noise(vec2(q.x*4.0, q.y*3.2 - t*0.05)) - 0.5)*0.04;
        vec3 fogA = pal(0.10 + hueField);
        vec3 fogB = pal(0.28 + scatter*0.18 + hueField*0.9 + shaft*0.16);
        col = mix(fogA, fogB, clamp(scatter*0.75 + shaft*0.45, 0.0, 1.0)) * v;
