...
`,Un=[{value:"value",label:"Value"},{value:"cloud",label:"Cloud"},{value:"voronoi",label:"Voronoi"},{value:"ridged",label:"Ridged"},{value:"warp",label:"Domain Warp"},{value:"turbulence",label:"Turbulence"}],xi={value:0,cloud:1,voronoi:2,ridged:3,warp:4,turbulence:5};function Ea(e){return xi[e]??1}const Ce=8,_n=`
...
`;function ve(e){return Math.min(1,Math.max(0,Number.isFinite(e)?e:0))}function Ra(e,t,a){return Math.min(a,Math.max(t,Number.isFinite(e)?e:0))}function wi(e,t,a,n,o){let i=ve(o);for(let r=0;r<8;r++){const s=1-i,c=s*s*s*e+3*s*s*i*t+3*s*i*i*a+i*i*i*n-o,u=3*s*s*(t-e)+6*s*i*(a-t)+3*i*i*(n-a);if(Math.abs(u)<1e-6)break;i=ve(i-c/u)}return i}function ki(e){if(typeof e.handleAngle=="number"&&typeof e.handleDist=="number"&&e.handleDist>0){const t=e.handleDist*.33;return{tanDx:Math.cos(e.handleAngle)*t,tanDy:Math.sin(e.handleAngle)*t,tanSet:!0}}return typeof e.bend=="number"&&e.bend!==0?{tanDx:0,tanDy:Ra(e.bend,-1,1)*.25,tanSet:!0}:{tanDx:0,tanDy:0,tanSet:!1}}function Tt(e){if(e&&Array.isArray(e.points)&&e.points.length>=2){const t=e.points.map(a=>{const o=a.tanSet&&typeof a.tanDx=="number"&&typeof a.tanDy=="number"?{tanDx:a.tanDx,tanDy:a.tanDy,tanSet:!0}:ki(a);return{x:ve(a.x),y:ve(a.y),type:a.type==="constant"?"constant":"bezier",tanDx:o.tanSet?o.tanDx:0,tanDy:o.tanSet?o.tanDy:0,tanSet:!!o.tanSet}}).sort((a,n)=>a.x-n.x).slice(0,Ce);return t[0]={...t[0],x:0},t[t.length-1]={...t[t.length-1],x:1},{points:t}}return e&&typeof e.x=="number"&&typeof e.y=="number"?{points:[{x:0,y:0,type:"bezier",tanDx:0,tanDy:0,tanSet:!1},{x:ve(e.x),y:ve(e.y),type:"bezier",tanDx:0,tanDy:0,tanSet:!1},{x:1,y:1,type:"bezier",tanDx:0,tanDy:0,tanSet:!1}]}:{points:[{x:0,y:0,type:"bezier",tanDx:0,tanDy:0,tanSet:!1},{x:1,y:1,type:"bezier",tanDx:0,tanDy:0,tanSet:!1}]}}function Si(e,t){const{points:a}=Tt(e),n=ve(t);for(let o=0;o<a.length-1;o++){const i=a[o],r=a[o+1];if(n>=i.x&&(n<r.x||o===a.length-2)){if(i.type==="constant")return i.y;const s=Math.max(r.x-i.x,1e-5),c=Ra(i.tanSet?i.x+i.tanDx:i.x+s/3,i.x,r.x),u=i.tanSet?i.y+i.tanDy:i.y,d=Ra(r.tanSet?r.x-r.tanDx:r.x-s/3,i.x,r.x),f=r.tanSet?r.y-r.tanDy:r.y,p=wi(i.x,c,d,r.x,n),h=1-p;return h*h*h*i.y+3*h*h*p*u+3*h*p*p*f+p*p*p*r.y}}return a[a.length-1].y}function On(e){const{points:t}=Tt(e),a=[],n=[];for(let o=0;o<Ce;o++){const i=t[Math.min(o,t.length-1)];a.push([i.x,i.y,i.type==="constant"?1:0,0]),n.push([i.tanDx||0,i.tanDy||0,i.tanSet?1:0,0])}return{points:a,tans:n,count:t.length}}const Lt=8;function io(e){const t=typeof e=="string"?/^#?([0-9a-fA-F]{6})$/.exec(e.trim()):null;return t?`#${t[1].toLowerCase()}`:"#ffffff"}function It(e){const t=io(e).slice(1);return[parseInt(t.slice(0,2),16)/255,parseInt(t.slice(2,4),16)/255,parseInt(t.slice(4,6),16)/255]}function Ci([e,t,a]){const n=o=>Math.round(ve(o)*255).toString(16).padStart(2,"0");return`#${n(e)}${n(t)}${n(a)}`}function gt(e){let t=e&&Array.isArray(e.stops)?e.stops:null;(!t||t.length<2)&&(t=[{t:0,color:"#1b2a6b"},{t:1,color:"#ffcf6b"}]);const a=t.map(n=>({t:ve(Number(n&&n.t)),color:io(n&&n.color)})).sort((n,o)=>n.t-o.t).slice(0,Lt);return a.length<2&&a.push({t:1,color:"#ffffff"}),{stops:a}}function Ei(e,t){const{stops:a}=gt(e),n=ve(t);if(n<=a[0].t)return a[0].color;if(n>=a[a.length-1].t)return a[a.length-1].color;for(let o=0;o<a.length-1;o++){const i=a[o],r=a[o+1];if(n>=i.t&&n<=r.t){const s=Math.max(r.t-i.t,1e-5),c=(n-i.t)/s,u=It(i.color),d=It(r.color);return Ci([u[0]+(d[0]-u[0])*c,u[1]+(d[1]-u[1])*c,u[2]+(d[2]-u[2])*c])}}return a[a.length-1].color}function ro(e){const{stops:t}=gt(e);return`linear-gradient(90deg, ${t.map(a=>`${a.color} ${(a.t*100).toFixed(1)}%`).join(", ")})`}function Di(e){const{stops:t}=gt(e),a=[];for(let n=0;n<Lt;n++){const o=t[Math.min(n,t.length-1)],i=It(o.color);a.push([i[0],i[1],i[2],o.t])}return{stops:a,count:t.length}}const Mi=`
  #define FX_GRAD_MAX ${Lt}
...
`,lo=[{id:"glow",label:"Glow / Deep Glow",group:"Effect",icon:'<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.3" stroke-linecap="round"><circle cx="8" cy="8" r="2.6"/><path d="M8 1.4v2M8 12.6v2M1.4 8h2M12.6 8h2M3.4 3.4l1.4 1.4M11.2 11.2l1.4 1.4M3.4 12.6l1.4-1.4M11.2 4.8l1.4-1.4"/></svg>',params:[{key:"threshold",label:"Threshold",min:0,max:1,step:.01,default:.55},{key:"intensity",label:"Intensity",min:0,max:3,step:.02,default:1.1},{key:"radius",label:"Radius",min:.5,max:6,step:.05,default:2.2},{key:"steps",label:"Steps (Quality)",min:2,max:8,step:1,default:4}],passes:[{key:"glow",fragmentShader:`
        ...
        `,updateUniforms(e,t){const{stops:a,count:n}=Di(t.gradient);for(let o=0;o<a.length;o++)e.uGradStops.value[o].set(a[o][0],a[o][1],a[o][2],a[o][3]);e.uGradCount.value=n,e.uContrast.value=t.contrast,e.uInvert.value=t.invert?1:0,e.uMix.value=t.mix}}]},{id:"uvNoise",label:"UV Noise",group:"Distort",icon:'<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.2"><circle cx="4.5" cy="5" r="1.6"/><circle cx="11" cy="4.5" r="1.1"/><circle cx="10" cy="10.5" r="2"/><circle cx="4" cy="11" r="1.2"/></svg>',params:[{key:"noiseType",label:"Noise Type",type:"select",default:"cloud",options:Un},{key:"amount",label:"Amount",min:0,max:.15,step:.001,default:.035},{key:"scale",label:"Scale",min:1,max:30,step:.5,default:5},{key:"speed",label:"Speed",min:0,max:3,step:.02,default:.4}],passes:[{key:"uvNoise",fragmentShader:`
          uniform sampler2D tDiffuse;
          uniform float uAmount;
          uniform float uScale;
          uniform float uSpeed;
        ...
        `,updateUniforms(e,t){e.uAngle.value=t.angle,e.uAmount.value=t.amount,e.uSteps.value=t.steps,e.uRandomness.value=t.randomness}}]},{id:"uvBlur",label:"UV Blur",group:"Blur",icon:'<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.2"><path d="M1.5 5.5c1.2-1.6 2.4-1.6 3.6 0s2.4 1.6 3.6 0 2.4-1.6 3.6 0" opacity="0.4"/><path d="M1.5 8.5c1.2-1.6 2.4-1.6 3.6 0s2.4 1.6 3.6 0 2.4-1.6 3.6 0" opacity="0.7"/><path d="M1.5 11.5c1.2-1.6 2.4-1.6 3.6 0s2.4 1.6 3.6 0 2.4-1.6 3.6 0" opacity="1"/></svg>',params:[{key:"followDistort",label:"Sync Distort",type:"toggle",default:!0},{key:"strength",label:"Strength",min:0,max:.15,step:.001,default:.05},{key:"noiseType",label:"Noise Type",type:"select",default:"cloud",options:Un},{key:"scale",label:"Scale",min:1,max:30,step:.5,default:5},{key:"speed",label:"Speed",min:0,max:3,step:.02,default:.4}],passes:[{key:"uvBlur",fragmentShader:`
          uniform sampler2D tDiffuse;
        ...
        `,updateUniforms(e,t,a){e.uMode.value=t.flowType==="radial"?1:t.flowType==="weave"?2:t.flowType==="noise"?3:0,e.uAngle.value=t.flowAngle,e.uArc.value=t.lightAngle,e.uFrequency.value=t.frequency,e.uStrength.value=t.strength,e.uJitter.value=t.organic,e.uSpeed.value=t.speed,e.uRoughness.value=t.contentFollow,e.uFalloff.value=t.contentRelief,e.uShadowHue.value=t.shadowHue,e.uHighlightHue.value=t.highlightHue,e.uSaturation.value=t.saturation,e.uChroma.value=t.colorize,e.uContrast.value=t.punch,e.uMix.value=t.mix,e.uTime.value=a}}]}];function Re(e){return lo.find(t=>t.id===e)||null}function Pi(e){if(!e||!e.stack||!e.instance)return null;const t=e.stack.indexOf(e.instance);if(t<0)return null;for(let a=t-1;a>=0;a--){const n=e.stack[a];if(!n.enabled)continue;const o=Re(n.defId);if(o&&o.group==="Distort")return{inst:n,def:o}}return null}function tn(e){const t=Re(e);if(!t)return{};const a={};return t.params.forEach(n=>{a[n.key]=n.default&&typeof n.default=="object"?JSON.parse(JSON.stringify(n.default)):n.default}),a}const zn=`
  uniform sampler2D tDiffuse;
  varying vec2 vUv;
  void main() { gl_FragColor = texture2D(tDiffuse, vUv); }
`;class Ii{constructor(t){this.canvas=t,this.renderer=new L.WebGLRenderer({canvas:t,alpha:!1,antialias:!1,preserveDrawingBuffer:!0}),this.renderer.setPixelRatio(Math.min(window.devicePixelRatio||1,2)),this.renderer.debug.onShaderError=(a,n,o,i)=>{const r=a.getShaderInfoLog(o),s=a.getShaderInfoLog(i),c=a.getProgramInfoLog(n);console.error(`[postfx] Shader compile/link error:
--- vertex log ---
`+r+`
--- fragment log ---
`+s+`
--- program log ---
`+c)},t.addEventListener("webglcontextlost",this._onContextLost=a=>{a.preventDefault(),console.error("[postfx] WebGL context lost on compositor canvas.")}),t.addEventListener("webglcontextrestored",this._onContextRestored=()=>{console.warn("[postfx] WebGL context restored on compositor canvas.")}),this.scene=new L.Scene,this.camera=new L.OrthographicCamera(-1,1,1,-1,0,1),this.quad=new L.Mesh(new L.PlaneGeometry(2,2),new L.ShaderMaterial({vertexShader:Nn,fragmentShader:zn,uniforms:{tDiffuse:{value:null}}})),this.scene.add(this.quad),this.materialCache=new Map,this.canvasTexture=null,this.sourceCanvas=null,this.sourceWidth=0,this.sourceHeight=0,this.uniformResolutionScale=1,this.rtA=null,this.rtB=null,this.width=0,this.height=0}_materialFor(t){let a=this.materialCache.get(t.key);if(a||(a=new L.ShaderMaterial({vertexShader:Nn,fragmentShader:t.fragmentShader||zn,uniforms:{tDiffuse:{value:null},uResolution:{value:new L.Vector2(1,1)},uTime:{value:0},uThreshold:{value:.5},uIntensity:{value:1},uRadius:{value:1},uSteps:{value:4},uAmount:{value:0},uBlend:{value:0},uBlur:{value:0},uBulge:{value:0},uPinch:{value:0},uUseCurve:{value:0},uEdge:{value:0},uScale:{value:1},uSpeed:{value:0},uDirection:{value:new L.Vector2(1,0)},uSize:{value:8},uDotSize:{value:8},uAngle:{value:0},uMono:{value:0},uVariation:{value:.35},uStrength:{value:.35},uOffset:{value:.5},uShadowHue:{value:220},uHighlightHue:{value:40},uSaturation:{value:.65},uMix:{value:.85},uCurvePoints:{value:Array.from({length:Ce},()=>new L.Vector4(0,0,0,0))},uCurveTans:{value:Array.from({length:Ce},()=>new L.Vector4(0,0,0,0))},uCurveCount:{value:2},uEdgeShape:{value:0},uEdgeSoftness:{value:.35},uNoiseType:{value:0},uSpin:{value:0},tImage:{value:null},uHasImage:{value:0},uMode:{value:0},tDistortImage:{value:null},uHasImageDistort:{value:0},uGradStops:{value:Array.from({length:Lt},()=>new L.Vector4(0,0,0,0))},uGradCount:{value:2},uContrast:{value:0},uBrightness:{value:0},uBlackPoint:{value:0},uWhitePoint:{value:1},uGamma:{value:1},uHighlights:{value:0},uShadows:{value:0},uHueShift:{value:0},uSatScale:{value:1},uLightness:{value:0},uInvert:{value:0},uShape:{value:0},uColorMode:{value:0},uInkColor:{value:new L.Vector3(0,0,0)},uPaperColor:{value:new L.Vector3(1,1,1)},uRandomness:{value:0},uArc:{value:0},uArcPosition:{value:.5},uRoughness:{value:0},uChroma:{value:0},uCenter:{value:new L.Vector2(.5,.5)},uBlades:{value:0},uFocusPos:{value:.5},uFocusWidth:{value:.2},uFrequency:{value:8},uFalloff:{value:1},uTiltX:{value:0},uTiltY:{value:0},uJitter:{value:0},uStock:{value:1},uGrain:{value:.45},uGrainSize:{value:1.4},uGrainAnimated:{value:1},uHalation:{value:.55},uHaloSize:{value:3.2},uFade:{value:.08},uTemperature:{value:0},uTintShift:{value:0},uVignette:{value:.4},uBrushType:{value:0},uBrushSize:{value:9},uStrokeStrength:{value:.72},uDetail:{value:.55},uEdgeBlend:{value:.6},uStrokePresence:{value:.65}}}),this.materialCache.set(t.key,a)),t.key==="oilPaint"){const n=a.uniforms;n.uBrushType||(n.uBrushType={value:0}),n.uBrushSize||(n.uBrushSize={value:9}),n.uStrokeStrength||(n.uStrokeStrength={value:.72}),n.uDetail||(n.uDetail={value:.55}),n.uEdgeBlend||(n.uEdgeBlend={value:.6}),n.uStrokePresence||(n.uStrokePresence={value:.65}),n.uMix||(n.uMix={value:.85})}return a}getInstanceTexture(t){if(!t)return null;const a=t.params&&t.params.image;if(!a)return t._glTex&&(t._glTex.dispose(),t._glTex=null,t._glTexUrl=null),null;if(t._glTex&&t._glTexUrl===a)return t._glTex;t._glTex&&t._glTex.dispose();const n=new Image,o=new L.Texture(n);return o.minFilter=L.LinearFilter,o.magFilter=L.LinearFilter,o.generateMipmaps=!1,n.onload=()=>{o.needsUpdate=!0},n.src=a,t._glTex=o,t._glTexUrl=a,o}setSource(t){this.sourceCanvas!==t&&(this._recreateCanvasTexture(t),this._debugLoggedThisSource=!1)}_recreateCanvasTexture(t){this.sourceCanvas=t,this.canvasTexture&&this.canvasTexture.dispose(),this.canvasTexture=t?new L.CanvasTexture(t):null,this.canvasTexture&&(this.canvasTexture.minFilter=L.LinearFilter,this.canvasTexture.magFilter=L.LinearFilter,this.canvasTexture.generateMipmaps=!1),this.sourceWidth=t?t.width:0,this.sourceHeight=t?t.height:0}resize(t,a){if(t<=0||a<=0||this.width===t&&this.height===a)return;this.width=t,this.height=a,this.renderer.setSize(t,a,!1);const n=this.renderer.getPixelRatio(),o=Math.max(1,Math.floor(t*n)),i=Math.max(1,Math.floor(a*n));this.rtA&&this.rtA.dispose(),this.rtB&&this.rtB.dispose(),this.rtA=new L.WebGLRenderTarget(o,i,{depthBuffer:!1,stencilBuffer:!1}),this.rtB=new L.WebGLRenderTarget(o,i,{depthBuffer:!1,stencilBuffer:!1})}renderFrame(t,a){if(!this.canvasTexture||!this.rtA||!this.rtB)return;const n=this.sourceCanvas?this.sourceCanvas.width:0,o=this.sourceCanvas?this.sourceCanvas.height:0;(n!==this.sourceWidth||o!==this.sourceHeight)&&this._recreateCanvasTexture(this.sourceCanvas),this.canvasTexture.needsUpdate=!0;const i=[];t.forEach(u=>{if(!u.enabled)return;const d=Re(u.defId);d&&d.passes.forEach(f=>i.push({pass:f,params:u.params,instance:u}))});const r=[this.rtA,this.rtB];let s=this.canvasTexture,c=0;if(i.length===0){const u=this.quad.material;u.uniforms.tDiffuse.value=s,this.renderer.setRenderTarget(null),this.renderer.render(this.scene,this.camera);return}i.forEach(({pass:u,params:d,instance:f},p)=>{const h=p===i.length-1,v=this._materialFor(u);v.uniforms.tDiffuse.value=s;const m=Math.max(1e-6,this.uniformResolutionScale||1);v.uniforms.uResolution.value.set(this.rtA.width/m,this.rtA.height/m),u.updateUniforms(v.uniforms,d,a,{instance:f,compositor:this,stack:t}),this.quad.material=v;const g=h?null:r[c%2];this.renderer.setRenderTarget(g),this.renderer.render(this.scene,this.camera),h||(s=g.texture,c+=1)})}captureAtScale(t,a,n){if(!this.width||!this.height)return null;const o=this.renderer.getPixelRatio(),i=this.rtA,r=this.rtB,s=Math.max(.01,Number(n)||1),c=o*s,u=Math.max(1,Math.floor(this.width*c)),d=Math.max(1,Math.floor(this.height*c));this.renderer.setPixelRatio(c),this.renderer.setSize(this.width,this.height,!1),this.rtA=new L.WebGLRenderTarget(u,d,{depthBuffer:!1,stencilBuffer:!1}),this.rtB=new L.WebGLRenderTarget(u,d,{depthBuffer:!1,stencilBuffer:!1});let f=null;try{this.uniformResolutionScale=s,this.renderFrame(t,a),f=this.canvas.toDataURL("image/png")}finally{this.uniformResolutionScale=1,this.rtA.dispose(),this.rtB.dispose(),this.rtA=i,this.rtB=r,this.renderer.setPixelRatio(o),this.renderer.setSize(this.width,this.height,!1),this.renderFrame(t,a)}return f?{dataUrl:f,width:u,height:d}:null}dispose(){this.materialCache.forEach(t=>t.dispose()),this.materialCache.clear(),this.canvasTexture&&this.canvasTexture.dispose(),this.rtA&&this.rtA.dispose(),this.rtB&&this.rtB.dispose(),this._onContextLost&&this.canvas.removeEventListener("webglcontextlost",this._onContextLost),this._onContextRestored&&this.canvas.removeEventListener("webglcontextrestored",this._onContextRestored),this.renderer.dispose()}}const Ai="/assets/Default-DtKDocRU.png",De="shaderops:active-profile:v1",je="shaderops:profiles:v1",ft="shaderops:profile:",so="shaderops-profile",Bi=3,Wt="shaderops:profile-default-migration:v2",Ve="shaderops:project-previews:v1",co="shaderops:unified-checkpoints:",uo="shaderops:image-loader:v1:",Be="shaderops-cache:",Ti=`${Be}checkpoint-previews:v1:`,Li=`${Be}project-previews:v2:`,Ri=`${Be}project-revisions:v1:`,Fa="__deleted__:",fo=1;function y(){return window.localStorage}function pt(e,t){try{const a=JSON.parse(e);return a&&typeof a=="object"?a:t}catch{return t}}function po(e){return String(e||"").trim().replace(/\s+/g," ").slice(0,48)}function mo(){return`profile-${Date.now().toString(36)}-${Math.random().toString(36).slice(2,8)}`}function Fe(){const e=pt(y().getItem(je),[]);return Array.isArray(e)?e.filter(t=>t&&typeof t.id=="string"&&typeof t.name=="string"):[]}function be(){const e=y().getItem(De);return Fe().find(t=>t.id===e)||null}function Fi(){if(y().getItem(Wt)==="1")return;const e=be();if(!e)return;const t=`${ft}${e.id}:`,a=[];for(let n=0;n<y().length;n++){const o=y().key(n);o?.startsWith(t)&&a.push([o,o.slice(t.length),y().getItem(o)])}a.forEach(([n,o,i])=>{let r=i;o===Ve&&(r=ho(y().getItem(o),i)),y().removeItem(n),y().removeItem(o),y().setItem(o,r)}),y().setItem(Wt,"1")}function $i(e){const t=po(e);if(!t)throw new Error("A profile name is required.");const a={id:mo(),name:t,createdAt:new Date().toISOString(),updatedAt:new Date().toISOString()},n=[...Fe(),a];return y().setItem(je,JSON.stringify(n)),y().setItem(De,a.id),a}function Ni(e){const t=Fe();if(!t.some(o=>o.id===e))throw new Error("This profile is not available on this computer.");const a=`${ft}${e}:`;for(let o=y().length-1;o>=0;o--){const i=y().key(o);(i?.startsWith(a)||i?.startsWith(`${Ti}${e}:`)||i===`${Li}${e}`||i===`${Ri}${e}`)&&y().removeItem(i)}const n=t.filter(o=>o.id!==e);return y().setItem(je,JSON.stringify(n)),y().getItem(De)===e&&(n[0]?y().setItem(De,n[0].id):y().removeItem(De)),n[0]||null}function Ui(){for(let e=y().length-1;e>=0;e--){const t=y().key(e);(t===Ve||t?.endsWith(`:${Ve}`))&&y().removeItem(t)}}function an(e){const t=Fe().find(a=>a.id===e);if(!t)throw new Error("This profile is not available on this computer.");return y().setItem(De,t.id),t}function nn(){return be()?.id||"default"}function jt(e,t=nn()){return`${ft}${t}:${e}`}function $a(e,t){return jt(`${Fa}${e}`,t)}function ho(e,t){const a=pt(e,{}),n=pt(t,{});if(n?.version!==fo||!n.values)return JSON.stringify({...a,...n});const o={...a,...n.values};return Array.isArray(n.deleted)&&n.deleted.forEach(i=>delete o[i]),JSON.stringify(o)}function _i(e,t){const a=pt(e,{}),n=pt(t,{}),o={};Object.entries(n).forEach(([r,s])=>{a[r]!==s&&(o[r]=s)});const i=Object.keys(a).filter(r=>!(r in n));return JSON.stringify({version:fo,values:o,deleted:i})}function on(e,t){if(y().getItem($a(e,t))!==null)return null;const a=y().getItem(jt(e,t)),n=y().getItem(e);return a===null?n:e===Ve?ho(n,a):a}function Na(e){const t=new Set,a=`${ft}${e}:`;for(let n=0;n<y().length;n++){const o=y().key(n);if(o?.startsWith("shaderops:")&&!o.startsWith(Be)&&!o.startsWith(ft)&&o!==De&&o!==je&&o!==Wt)t.add(o);else if(o?.startsWith(a)){const i=o.slice(a.length);t.add(i.startsWith(Fa)?i.slice(Fa.length):i)}}return[...t].filter(n=>on(n,e)!==null)}function Hn(e){const t=Fe(),a=t.find(n=>n.id===e);a&&(a.updatedAt=new Date().toISOString(),y().setItem(je,JSON.stringify(t)))}function Oi(){const e=nn();return{getItem(t){return on(t,e)},setItem(t,a){y().removeItem($a(t,e));const n=t===Ve?_i(y().getItem(t),String(a)):String(a);y().setItem(jt(t,e),n),Hn(e)},removeItem(t){y().removeItem(jt(t,e)),y().setItem($a(t,e),"1"),Hn(e)},key(t){return Na(e)[t]??null},get length(){return Na(e).length}}}function zi(e=nn()){const t=Fe().find(n=>n.id===e);if(!t)throw new Error("This profile is not available on this computer.");const a={};return Na(e).forEach(n=>{if(n===Ve||n.startsWith(uo))return;const o=on(n,e);a[n]=n.startsWith(co)?vo(o):o}),{format:so,version:Bi,exportedAt:new Date().toISOString(),profile:{name:t.name,createdAt:t.createdAt},entries:a}}function vo(e){const t=pt(e,null);if(!t||!Array.isArray(t.slots))return e;const a=t.slots.map(n=>{if(!n||typeof n!="object")return n;const{preview:o,...i}=n;return typeof i.imagePath=="string"&&i.imagePath.startsWith("data:image/")&&(delete i.imagePath,delete i.imageName),i});return JSON.stringify({...t,slots:a})}function Hi(e){const t={...e.entries};for(const[a,n]of Object.entries(t))a===Ve||a.startsWith(uo)?delete t[a]:a.startsWith(co)&&(t[a]=vo(n));return{...e,entries:t}}function qi(e,t){if(!e||e.format!==so||!e.entries||typeof e.entries!="object")throw new Error("This is not a valid ShaderOps profile recovery file.");const a=Hi(e),n=Object.entries(a.entries).filter(([i,r])=>typeof i=="string"&&i.startsWith("shaderops:")&&!i.startsWith(Be)&&!i.startsWith(ft)&&i!==De&&i!==je&&i!==Wt&&typeof r=="string"),o=[];for(let i=0;i<y().length;i++){const r=y().key(i);r?.startsWith("shaderops:")&&!r.startsWith(Be)&&o.push([r,y().getItem(r)])}try{o.forEach(([r])=>y().removeItem(r));for(let r=y().length-1;r>=0;r--){const s=y().key(r);s?.startsWith(Be)&&y().removeItem(s)}n.forEach(([r,s])=>y().setItem(r,s));const i={id:mo(),name:po(t||e.profile?.name||"Recovered Profile"),createdAt:e.profile?.createdAt||new Date().toISOString(),updatedAt:new Date().toISOString()};return y().setItem(je,JSON.stringify([i])),y().setItem(De,i.id),i}catch(i){for(let r=y().length-1;r>=0;r--){const s=y().key(r);s?.startsWith("shaderops:")&&!s.startsWith(Be)&&y().removeItem(s)}throw o.forEach(([r,s])=>y().setItem(r,s)),new Error(i instanceof DOMException&&i.name==="QuotaExceededError"?"This recovery file is larger than this browser allows. The previous workspace was restored.":"Unable to restore this recovery file. The previous workspace was restored.")}}Fi();Ui();const E=Oi(),rn=be()?.id||"default",go=`shaderops-cache:project-previews:v2:${rn}`,yo=`shaderops-cache:project-revisions:v1:${rn}`,ln="shaderops-cache:checkpoint-previews:v1:",Gi="shaderops-thumbnail-cache",se="entries",Vt="shaderops:selected-project:v1",bo="shaderops:filters:v1",xo="shaderops:favorites:v1",wo="shaderops:project-order:v1",ko="shaderops:filter-add-order:v1",sn="shaderops:filter-rail-collapsed:v1",So="shaderops:curve-popup-size:v1",Co="shaderops-state",Ua=1,Wi=400,ji="shaderops:image-loader:v1:",yt="shaderops://image/default",Je="Default";function Vi(){return new Promise(e=>{if(!("indexedDB"in window)){e(null);return}const t=indexedDB.open(Gi,1);t.onupgradeneeded=()=>{t.result.objectStoreNames.contains(se)||t.result.createObjectStore(se)},t.onsuccess=()=>e(t.result),t.onerror=()=>{console.warn("ShaderOps: IndexedDB thumbnail cache is unavailable.",t.error),e(null)}})}const Rt=Vi();async function qn(e,t){const a=await Rt;return a?new Promise(n=>{const o=a.transaction(se,"readonly").objectStore(se).get(e);o.onsuccess=()=>n(o.result??t),o.onerror=()=>n(t)}):t}async function _a(e,t){const a=await Rt;a&&await new Promise((n,o)=>{const i=a.transaction(se,"readwrite").objectStore(se).put(t,e);i.onsuccess=()=>n(),i.onerror=()=>o(i.error)})}async function Da(e){const t=await Rt;t&&await new Promise(a=>{const o=t.transaction(se,"readwrite").objectStore(se).openCursor();o.onsuccess=()=>{const i=o.result;if(!i){a();return}String(i.key).startsWith(e)&&i.delete(),i.continue()},o.onerror=()=>a()})}async function Xi(e){const t=await Rt;return t?new Promise(a=>{const n=[],o=t.transaction(se,"readonly").objectStore(se).openCursor();o.onsuccess=()=>{const i=o.result;if(!i){a(n);return}String(i.key).startsWith(e)&&n.push([String(i.key),i.value]),i.continue()},o.onerror=()=>a(n)}):[]}async function Ki(){const e=await Rt;e&&await new Promise(t=>{const a=e.transaction(se,"readwrite").objectStore(se).clear();a.onsuccess=()=>t(),a.onerror=()=>t()})}const la=new Map,Ut=[];function Yi(e){const t=e.ownerDocument,a=t.createElement("div");a.className="w-pill";const n=t.createElement("div");n.className="w-pill__fill",a.appendChild(n);let o=null;const i=()=>{const s=e.dataset.pillMode||"",c=`${e.value}|${s}`;if(c===o)return;o=c;const u=Number(e.min)||0,d=Number(e.max)||100,f=Number(e.value),p=d===u?0:Math.min(1,Math.max(0,(f-u)/(d-u)));if(s==="center-zero"&&u<0&&d>0){const h=Math.min(1,Math.max(0,(0-u)/(d-u))),v=Math.min(p,h),m=Math.abs(p-h);a.classList.add("w-pill--center"),a.style.setProperty("--pill-center",`${(h*100).toFixed(3)}%`),n.classList.toggle("is-tip-left",p<h),n.style.left=`${(v*100).toFixed(3)}%`,n.style.width=`${(m*100).toFixed(3)}%`;return}a.classList.remove("w-pill--center"),a.style.removeProperty("--pill-center"),n.classList.remove("is-tip-left"),n.style.left="0%",n.style.width=`${(p*100).toFixed(3)}%`};i();const r=(s,c)=>{const u=a.getBoundingClientRect(),d=Number(e.min)||0,f=Number(e.max)||100,p=Number(e.step)||1,h=u.width===0?0:Math.min(1,Math.max(0,(s-u.left)/u.width));let v=d+h*(f-d);v=Math.round(v/p)*p,v=Math.min(f,Math.max(d,v)),e.value=String(Number(v.toFixed(6))),e.dispatchEvent(new Event("input",{bubbles:!0})),c||e.dispatchEvent(new Event("change",{bubbles:!0})),i()};return a.addEventListener("pointerdown",s=>{s.preventDefault(),a.setPointerCapture(s.pointerId),r(s.clientX,!0);const c=d=>r(d.clientX,!0),u=d=>{a.removeEventListener("pointermove",c),a.removeEventListener("pointerup",u),a.removeEventListener("pointercancel",u),r(d.clientX,!1)};a.addEventListener("pointermove",c),a.addEventListener("pointerup",u),a.addEventListener("pointercancel",u)}),e.insertAdjacentElement("beforebegin",a),e.style.display="none",e.dataset.pillMounted="1",{slider:e,paint:i}}function cn(e){if(!e)return;e.querySelectorAll('.control-row input[type="range"], .filter-param-row input[type="range"]').forEach(a=>{a.dataset.pillMounted||Ut.push(Yi(a))})}(function(){const t=()=>{for(let a=Ut.length-1;a>=0;a--){const n=Ut[a];if(!n.slider.isConnected){Ut.splice(a,1);continue}n.paint()}requestAnimationFrame(t)};requestAnimationFrame(t)})();const Ji=document.getElementById("app");document.documentElement.classList.toggle("app-client-shell",navigator.userAgent.includes("Electron"));Ji.innerHTML=`
...
`;const l={projects:[],selectedId:null,previews:{},captureInFlight:!1,prewarmQueue:[],prewarmActive:!1,previewRevisions:{},currentProjectRevisions:{},filterStacks:{},filterPreviewOn:{},filterHistory:{},navPage:0,favorites:[]},ie=document.getElementById("projectList"),Oa=document.getElementById("projectTitle"),za=document.getElementById("projectMethod"),D=document.getElementById("projectFrame"),Ha=document.getElementById("deleteProjectButton"),sa=document.getElementById("navGridButton"),it=document.getElementById("navPager"),Zi=document.getElementById("navPagerLabel"),V=document.getElementById("navGridPanel"),un=document.getElementById("navGridBackdrop"),qa=document.getElementById("navGridFav"),Ga=document.getElementById("navGrid2d"),Wa=document.getElementById("navGrid3d"),dn=document.getElementById("shaderStage"),R=document.getElementById("fxCanvas"),fn=document.getElementById("filterRail"),Qi=document.getElementById("filterRailGear"),er=document.getElementById("filterRailCollapse"),ca=document.getElementById("filterAddButton"),Z=document.getElementById("filterAddMenu"),_=document.getElementById("filterStackList"),tr=document.getElementById("filterStackEmpty"),Xt=document.getElementById("filterPreviewToggle"),Eo=document.getElementById("filterExportButton"),he=document.getElementById("filterCopyButton"),ar=document.getElementById("profileButton"),nr=document.getElementById("profileButtonName"),_t=document.getElementById("profileOverlay"),or=document.getElementById("profileDialogTitle"),ir=document.getElementById("profileDialogDescription"),Mt=document.getElementById("profileNameInput"),Ma=document.getElementById("profileExistingList"),rr=document.getElementById("profileRestoreHint"),Xe=document.getElementById("profileDialogStatus"),Do=document.getElementById("profileContinueButton"),Mo=document.getElementById("profileDownloadButton"),lr=document.getElementById("profileRestoreLabel"),ja=document.getElementById("profileRestoreInput"),Kt=document.getElementById("exportSizeOverlay"),sr=document.getElementById("exportSizeCurrent"),Va=document.getElementById("exportSizeOptions"),cr=document.getElementById("exportSizeCancel"),Y=document.getElementById("exportBatchCount"),Ge=document.getElementById("exportBatchScale"),ur=document.getElementById("exportBatchMode"),Ot=document.getElementById("exportBatchRun"),Gn=document.getElementById("exportBatchStatus"),Wn=document.getElementById("exportBatchPathName"),Xa=document.getElementById("exportBatchChoosePath"),pn=document.getElementById("exportBatchClearPath"),rt=document.getElementById("deleteConfirmOverlay"),dr=document.getElementById("deleteConfirmMessage"),fr=document.getElementById("deleteConfirmButton"),pr=document.getElementById("deleteConfirmCancel"),bt=document.getElementById("unifiedPanel"),mr=document.getElementById("unifiedPanelGear"),zt=document.getElementById("unifiedPanelGroups"),Po=document.getElementById("globalCheckpoints"),hr=document.getElementById("unifiedDiceButton"),vr=document.getElementById("unifiedWaveButton"),Ht=document.getElementById("unifiedPauseButton"),gr=document.getElementById("unifiedUndoButton"),yr=document.getElementById("unifiedRedoButton"),Pa=document.getElementById("unifiedCheckpointSlots"),Io=document.getElementById("unifiedCheckpointAddShell");document.getElementById("unifiedCheckpointAdd");const br=document.getElementById("topbarReloadButton"),Q=document.getElementById("thumbHoverPreview"),xr=document.getElementById("thumbHoverPreviewTitle"),mn=document.getElementById("thumbHoverPreviewHero"),jn=document.getElementById("thumbHoverPreviewCheckpoints");let Ao="settings";function wr(){nr.textContent=be()?.name||"Profile"}function kr(e){try{an(e),window.location.reload()}catch(t){Xe.textContent=t instanceof Error?t.message:"Unable to switch profile."}}function Bo(){const e=D.contentWindow;try{e?.ShaderOpsControls?.suspendPersistence?.()}catch{}l.selectedId&&e?.postMessage({type:"shaderops/suspend-persistence",projectId:l.selectedId},"*")}function To(){const e=D.contentWindow;try{e?.ShaderOpsControls?.resumePersistence?.()}catch{}l.selectedId&&e?.postMessage({type:"shaderops/resume-persistence",projectId:l.selectedId},"*")}function Lo(){const e=Fe(),t=be()?.id;Ma.replaceChildren(),Ma.hidden=e.length===0,e.forEach(a=>{const n=document.createElement("span");n.className=`profile-existing-list__item${a.id===t?" is-active":""}`;const o=document.createElement("button");o.type="button",o.className="profile-existing-list__switch",o.textContent=a.name,o.title=`Switch to ${a.name}`,o.addEventListener("click",()=>kr(a.id));const i=document.createElement("button");i.type="button",i.className="profile-existing-list__delete",i.textContent="×",i.title=`Delete ${a.name}`,i.setAttribute("aria-label",`Delete ${a.name}`),i.addEventListener("click",async()=>{if(!window.confirm(`Delete profile "${a.name}" from this browser?`))return;const r=a.id===be()?.id;try{r&&Bo(),await Promise.all([Da(`shaderops-cache:project-previews:v2:${a.id}`),Da(`shaderops-cache:project-revisions:v1:${a.id}`),Da(`${ln}${a.id}:`)]),Ni(a.id),r?window.location.reload():Lo()}catch(s){r&&To(),Xe.textContent=s instanceof Error?s.message:"Unable to delete this profile."}}),n.append(o,i),Ma.appendChild(n)})}function Ro(e){Ao=e;const t=be(),a=e==="onboarding";or.textContent=a?"Welcome to ShaderOps":"Profile & recovery",ir.textContent=a?"Choose a name once. Your favorites, checkpoints, looks, and filters save automatically on this computer.":`Working as ${t?.name||"this profile"}. Your work is stored locally and saves automatically.`,Mt.value="",Mt.placeholder=a?"Your name":"Create another profile",Do.textContent=a?"Continue":"Create profile",Mo.hidden=a||!t,lr.hidden=a,rr.hidden=a,Xe.textContent="",Lo(),_t.hidden=!1,window.setTimeout(()=>Mt.focus(),0)}function Fo(){try{const e=$i(Mt.value);an(e.id),window.location.reload()}catch(e){Xe.textContent=e instanceof Error?e.message:"Enter a profile name to continue."}}function Sr(){try{const e=be(),t=new Blob([JSON.stringify(zi(),null,2)],{type:"application/json"}),a=URL.createObjectURL(t),n=document.createElement("a");n.href=a,n.download=`${(e?.name||"shaderops-profile").replace(/[^\w.-]+/g,"-")}.shaderops-profile.json`,n.click(),URL.revokeObjectURL(a),Xe.textContent="Recovery file downloaded."}catch(e){Xe.textContent=e instanceof Error?e.message:"Unable to create recovery file."}}async function Cr(e){if(e)try{const t=JSON.parse(await e.text()),a=Fe().map(i=>i.name),n=a.length?` Existing profiles that will be replaced: ${a.join(", ")}.`:"";if(!window.confirm(`Restore this recovery file and replace all ShaderOps data in this browser?${n}`))return;await Ki(),la.clear(),Bo();const o=qi(t);an(o.id),window.location.reload()}catch(t){To(),Xe.textContent=t instanceof Error?t.message:"Unable to restore this recovery file."}finally{ja.value=""}}ar.addEventListener("click",()=>Ro("settings"));Do.addEventListener("click",Fo);Mt.addEventListener("keydown",e=>{e.key==="Enter"&&Fo()});Mo.addEventListener("click",Sr);ja.addEventListener("change",()=>Cr(ja.files?.[0]));_t.addEventListener("click",e=>{e.target===_t&&Ao!=="onboarding"&&(_t.hidden=!0)});wr();be()||Ro("onboarding");const hn="shaderops:unified-panel-collapsed:v1",Vn=["Global","Color"],G=new Map,nt=new Map;function $o(e){return!!(e&&e.bridged)}function Ze(){const e=l.projects.find(t=>t.id===l.selectedId);return $o(e)?e.id:null}function $e(e,t,a){D.contentWindow?.postMessage({type:"shaderops/action",projectId:e,action:t,payload:a},"*")}function tt(e,t,a,n){D.contentWindow?.postMessage({type:"shaderops/set-param",projectId:e,id:t,value:a,commit:n},"*")}function Yt(e,t,a,n=!0){D.contentWindow?.postMessage({type:"shaderops/set-state",projectId:e,payload:{params:t,extras:a},commit:n},"*")}function Er(e,t=1500){return new Promise(a=>{const n=()=>a(!0),o=nt.get(e)||[];o.push(n),nt.set(e,o),setTimeout(()=>{const i=nt.get(e);!i||!i.includes(n)||(nt.set(e,i.filter(r=>r!==n)),a(!1))},t)})}const Ka=new Map,qt=new Map;function No(e){return`${ji}${e}`}function Ya(){return{loaded:!0,name:Je,sourcePath:yt}}function Uo(e){if(!Te(e))return Ya();const t=e.loaded!==!1,a=typeof e.name=="string"&&e.name.trim()?e.name.trim():Je,n=typeof e.sourcePath=="string"&&e.sourcePath.trim()?e.sourcePath.trim():yt;return t?{loaded:!0,name:a,sourcePath:n}:Ya()}function Dr(e,t){E.setItem(No(e),JSON.stringify(t))}function vn(e,t){const a=Uo(t);return Ka.set(e,a),Dr(e,a),a}function ua(e){const t=Ka.get(e);if(t)return t;const a=xe(E.getItem(No(e)),null),n=Uo(a);return Ka.set(e,n),n}function da(e){return Array.isArray(e)&&e.some(t=>t&&t.type==="image-loader")}function Mr(e){return new Promise((t,a)=>{const n=new Image;n.onload=()=>{const i=Math.min(1,1024/Math.max(n.width,n.height)),r=Math.max(1,Math.floor(n.width*i)),s=Math.max(1,Math.floor(n.height*i)),c=document.createElement("canvas");c.width=r,c.height=s;const u=c.getContext("2d",{willReadFrequently:!0});u.drawImage(n,0,0,r,s),t({imageData:u.getImageData(0,0,r,s),dataUrl:c.toDataURL("image/png")})},n.onerror=()=>a(new Error(`Unable to decode image source: ${e.slice(0,64)}`)),n.src=e})}function Pr(e){return e===yt?Ai:e}async function Ir(e){const t=typeof e=="string"&&e.trim()?e.trim():yt,a=t;if(!qt.has(a)){const n=Pr(t);qt.set(a,Mr(n).then(({imageData:o})=>o))}return qt.get(a)}async function gn(e,t,a){if(!e||!a||!a.loaded||!a.sourcePath)return!1;const n=await Ir(a.sourcePath);return e.postMessage({type:"shaderops/image-load",projectId:t,name:a.name||Je,sourcePath:a.sourcePath,imageData:n},"*"),!0}async function Jt(e,t=D.contentWindow){const a=vn(e,Ya());await gn(t,e,a),l.selectedId===e&&ne(e)}async function Ar(e,t=D.contentWindow){const a=ua(e);try{if(await gn(t,e,a)){l.selectedId===e&&ne(e);return}}catch(n){console.warn(`ShaderOps: failed to restore image source for ${e}, falling back to Default.`,n)}await Jt(e,t)}async function Br(e,t,a,n=D.contentWindow){const o=typeof t=="string"?t.trim():"";if(!o){await Jt(e,n);return}const i=vn(e,{loaded:!0,name:a||Je,sourcePath:o});try{await gn(n,e,i)}catch(r){console.warn(`ShaderOps: checkpoint image path restore failed for ${e}; using Default.`,r),await Jt(e,n);return}l.selectedId===e&&ne(e)}let de=null;function Tr(e){de&&de.remove(),de=document.createElement("input"),de.type="file",de.accept="image/*",de.style.display="none",document.body.appendChild(de),de.addEventListener("change",()=>{const t=de.files&&de.files[0];if(!t)return;const a=URL.createObjectURL(t),n=new Image;n.onload=()=>{const i=Math.min(1,1024/Math.max(n.width,n.height)),r=Math.max(1,Math.floor(n.width*i)),s=Math.max(1,Math.floor(n.height*i)),c=document.createElement("canvas");c.width=r,c.height=s;const u=c.getContext("2d",{willReadFrequently:!0});u.drawImage(n,0,0,r,s);const d=u.getImageData(0,0,r,s),f=c.toDataURL("image/png");vn(e,{loaded:!0,name:t.name||"image",sourcePath:f}),qt.set(f,Promise.resolve(d)),D.contentWindow?.postMessage({type:"shaderops/image-load",projectId:e,name:t.name,sourcePath:f,imageData:d},"*"),URL.revokeObjectURL(a),l.selectedId===e&&ne(e)},n.onerror=()=>URL.revokeObjectURL(a),n.src=a}),de.click()}function Lr(e){Jt(e,D.contentWindow)}function _o(e){Ht.querySelector(".icon-play").classList.toggle("is-hidden",!e),Ht.querySelector(".icon-pause").classList.toggle("is-hidden",!!e),Ht.title=e?"Resume (Space)":"Pause (Space)"}function Oo(){bt.hidden=!1,Po.hidden=!1}function Ia(){bt.hidden=!0,Po.hidden=!0}function zo(e){return`shaderops:locked-controls:${e}`}function Ho(e){const t=xe(E.getItem(zo(e)),[]);return new Set(Array.isArray(t)?t:[])}function Gt(e,t){E.setItem(zo(e),JSON.stringify([...t]))}function Aa(){return`<svg viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
  ...
  </svg>`}function fa(e){const t=G.get(e);if(!t)return;const a={};t.lockedIds.forEach(n=>{n in t.params&&(a[n]=t.params[n])}),t.pendingLockRestore=Object.keys(a).length?a:null,$e(e,"randomizeAll")}function qo(e){return`shaderops:unified-checkpoints:${e}`}function Go(e){const t=be()?.id||"default";return`${ln}${t}:${e}`}function Qe(e){const t=xe(E.getItem(qo(e)),null);if(t&&Array.isArray(t.slots)){const a=la.get(Go(e))||{};return{slots:t.slots.map(n=>n&&typeof n=="object"&&typeof a?.[n.id]=="string"?{...n,preview:a[n.id]}:n),nextId:t.nextId||t.slots.length+1,activeId:t.activeId??null}}return{slots:[],nextId:1,activeId:null}}function pa(e,t){const a={},n=t.slots.map(i=>{if(!i||typeof i!="object")return i;typeof i.preview=="string"&&(a[i.id]=i.preview);const{preview:r,...s}=i;return s});E.setItem(qo(e),JSON.stringify({...t,slots:n}));const o=Go(e);la.set(o,a),_a(o,a).catch(i=>{console.warn("ShaderOps: checkpoint preview cache is full; keeping checkpoint data without images.",i)})}function Wo(e,t){try{return JSON.parse(JSON.stringify(e))}catch{return t}}async function jo(e){return e!==l.selectedId||await ba(),l.previews[e]||null}function Ja(e,t){const a=G.get(e);if(!a)return;const n=Qe(e),o=n.slots.find(s=>s.id===t);if(!o)return;const i={};Object.entries(o.values||{}).forEach(([s,c])=>{s in a.params&&(i[s]=c)}),Object.assign(a.params,i);const r=o.extras&&typeof o.extras=="object";r&&Object.assign(a.extras,o.extras),Yt(e,i,r?o.extras:void 0,!0),o.filters&&Array.isArray(o.filters.stack)&&(B(e),l.filterPreviewOn[e]=o.filters.previewOn!==!1,xn(e,JSON.stringify(o.filters.stack))),da(a.schema)&&Object.prototype.hasOwnProperty.call(o,"imagePath")&&Br(e,o.imagePath,o.imageName,D.contentWindow),n.activeId=t,pa(e,n),l.selectedId===e&&yn(e),Ft(e)}async function Rr(e,t){const a=G.get(e);if(!a)return;const n=Qe(e),o=n.slots.find(i=>i.id===t);if(o){if(o.values={...a.params},o.extras=Wo(a.extras||{},{}),o.filters={stack:JSON.parse(xt(e)),previewOn:Ne(e)},da(a.schema)){const i=ua(e);o.imagePath=i.sourcePath||yt,o.imageName=i.name||Je}o.preview=await jo(e),n.activeId=t,pa(e,n),Ft(e)}}function Fr(e,t){const a=Qe(e);a.slots=a.slots.filter(n=>n.id!==t),a.activeId===t&&(a.activeId=null),pa(e,a),Ft(e)}async function $r(e){const t=G.get(e);if(!t)return;const a=Qe(e),n=a.nextId++,o=await jo(e),i={id:n,values:{...t.params},extras:Wo(t.extras||{},{}),filters:{stack:JSON.parse(xt(e)),previewOn:Ne(e)},preview:o};if(da(t.schema)){const r=ua(e);i.imagePath=r.sourcePath||yt,i.imageName=r.name||Je}a.slots.push(i),a.activeId=n,pa(e,a),Ft(e)}function Vo(e,{onTap:t,onHoldComplete:a,holdMs:n=800}={}){const o=e.querySelector(".ucp-ring");let i=null,r=!1,s=!1;const c=()=>{i&&(clearTimeout(i),i=null),e.classList.remove("holding")},u=p=>{p.button!==void 0&&p.button!==0||(r=!0,s=!1,e.classList.add("holding"),o&&o.style.setProperty("--ucp-ring-duration",`${n}ms`),i=setTimeout(()=>{s=!0,e.classList.remove("holding"),e.classList.add("completed"),setTimeout(()=>e.classList.remove("completed"),260),a?.()},n))},d=()=>{r&&(r=!1,c(),s||t?.())},f=()=>{r&&(r=!1,c())};e.addEventListener("pointerdown",u),e.addEventListener("pointerup",d),e.addEventListener("pointerleave",f),e.addEventListener("pointercancel",f)}let Se=null;function Nr(){return Se||(Se=document.createElement("div"),Se.className="ucp-menu",Se.hidden=!0,document.body.appendChild(Se),document.addEventListener("click",()=>Za()),window.addEventListener("blur",()=>Za()),Se)}function Za(){Se&&(Se.hidden=!0)}function Ur(e,t,a){const n=Nr();n.innerHTML="",a.forEach(s=>{const c=document.createElement("button");c.type="button",c.className="ucp-menu-item",c.textContent=s.label,c.addEventListener("click",u=>{u.stopPropagation(),Za(),s.onClick()}),n.appendChild(c)}),n.hidden=!1;const o=n.getBoundingClientRect(),i=Math.min(e,window.innerWidth-o.width-8),r=Math.min(t,window.innerHeight-o.height-8);n.style.left=`${Math.max(8,i)}px`,n.style.top=`${Math.max(8,r)}px`}function Ft(e){Pa.innerHTML="";const t=Qe(e);t.slots.forEach((a,n)=>{const o=document.createElement("span");o.className="ucp-btn-shell",t.activeId===a.id&&o.classList.add("is-active");const i=document.createElement("button");i.type="button",i.className="ucp-slot",i.textContent=String(n+1),i.title="Tap: load this checkpoint Â· Hold 0.8s: overwrite with the current look Â· Right-click: delete",o.appendChild(i);const r=document.createElementNS("http://www.w3.org/2000/svg","svg");r.setAttribute("viewBox","0 0 30 30"),r.setAttribute("class","ucp-ring"),r.setAttribute("aria-hidden","true");const s=document.createElementNS("http://www.w3.org/2000/svg","circle");s.setAttribute("cx","15"),s.setAttribute("cy","15"),s.setAttribute("r","13.3"),s.setAttribute("pathLength","100"),r.appendChild(s),o.appendChild(r),Vo(o,{onTap:()=>Ja(e,a.id),onHoldComplete:()=>{Rr(e,a.id)}}),o.addEventListener("contextmenu",c=>{c.preventDefault(),Ur(c.clientX,c.clientY,[{label:"Delete checkpoint",onClick:()=>Fr(e,a.id)}])}),Pa.appendChild(o)}),Pa.appendChild(Io)}function lt(e,t){if(!e||!e.showIf)return!0;const a=e.showIf;if(typeof a=="function")try{return!!a(t||{})}catch{return!0}if(typeof a=="string")return!!(t||{})[a];if(Array.isArray(a))return a.every(n=>lt({showIf:n},t));if(a&&typeof a=="object"){if(Array.isArray(a.all))return a.all.every(n=>lt({showIf:n},t));if(Array.isArray(a.any))return a.any.some(n=>lt({showIf:n},t));if(a.eq&&typeof a.eq=="object")return Object.entries(a.eq).every(([n,o])=>String((t||{})[n])===String(o));if(a.neq&&typeof a.neq=="object")return Object.entries(a.neq).every(([n,o])=>String((t||{})[n])!==String(o))}return!0}function ne(e){const t=G.get(e);if(!t)return;t.lockedIds||(t.lockedIds=Ho(e)),zt.innerHTML="";const a=new Map;(t.schema||[]).filter(c=>!c.hidden&&lt(c,t.params)).forEach(c=>{const u=c.group||"Controls";a.has(u)||a.set(u,[]),a.get(u).push(c)});const n=[...Vn.filter(c=>a.has(c)),...[...a.keys()].filter(c=>!Vn.includes(c))],o=`shaderops:unified-group-collapsed:${e}`,i=xe(E.getItem(o),{}),r=(c,u)=>{const d=document.createElement("button");d.type="button",d.className="shaderops-lock-button",d.innerHTML=Aa();const f=()=>{const p=t.lockedIds.has(c);d.classList.toggle("is-locked",p),u&&u.classList.toggle("is-locked",p),d.title=p?"Unlock this value":"Lock this value (skip on All Random)"};return f(),d.addEventListener("click",p=>{p.preventDefault(),p.stopPropagation(),t.lockedIds.has(c)?t.lockedIds.delete(c):t.lockedIds.add(c),Gt(e,t.lockedIds),f()}),d},s=(c,u)=>{const d=String(c||"").trim(),f=String(u||"").trim();return f?`${d}
${f}`:d};n.forEach((c,u)=>{const d=a.get(c),f=document.createElement("div");f.className="unified-control-group";const p=c.toLowerCase();i[p]&&f.classList.add("is-collapsed");const h=document.createElement("h3");h.textContent=c,h.setAttribute("role","button"),h.setAttribute("tabindex","0");const v=()=>{f.classList.toggle("is-collapsed"),i[p]=f.classList.contains("is-collapsed"),E.setItem(o,JSON.stringify(i))};if(h.addEventListener("click",v),h.addEventListener("keydown",m=>{(m.key==="Enter"||m.key===" ")&&(m.preventDefault(),v())}),f.appendChild(h),u===0){const m=document.createElement("button");m.type="button",m.className="unified-panel-close",m.title="Collapse controls",m.innerHTML='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" aria-hidden="true"><path d="M3.5 3.5l9 9M12.5 3.5l-9 9"/></svg>',m.addEventListener("click",g=>{g.preventDefault(),g.stopPropagation(),Xo()}),h.appendChild(m)}d.forEach(m=>{const g=document.createElement("div");g.dataset.controlId=m.id;const S=document.createElement("label"),M=String(m.label||m.id);if(S.textContent=M,S.title=s(M,m.hint),m.type==="shape-stack"){g.className="filter-param-row filter-param-row--shape-stack";const b=document.createElement("div");b.className="shaderops-shape-stack";const T=Array.isArray(m.shapes)?m.shapes:[],N=t.params[m.selectedParam],k=T.find(te=>String(te.value)===String(N))||null,P=T.filter(te=>Number(t.params[te.enabledParam])>.5),J=document.createElement("button");if(J.type="button",J.className="shaderops-shape-stack-trigger",J.textContent=k?`${k.label} Â· ${P.length} active`:`${P.length} active`,J.addEventListener("click",()=>{t.openShapeStackId=t.openShapeStackId===m.id?null:m.id,ne(e)}),b.appendChild(J),t.openShapeStackId===m.id){const te=document.createElement("div");te.className="shaderops-shape-stack-menu",T.forEach(w=>{const H=document.createElement("div");H.className="shaderops-shape-stack-item",k&&String(k.value)===String(w.value)&&H.classList.add("is-selected");const ae=document.createElement("input");ae.type="checkbox",ae.checked=Number(t.params[w.enabledParam])>.5,ae.addEventListener("click",I=>I.stopPropagation()),ae.addEventListener("change",()=>{const I={[w.enabledParam]:ae.checked?1:0};if(ae.checked)I[m.selectedParam]=w.value;else if(String(t.params[m.selectedParam])===String(w.value)){const Et=T.find(Ie=>Ie!==w&&Number(t.params[Ie.enabledParam])>.5);I[m.selectedParam]=Et?Et.value:0}Object.assign(t.params,I),Yt(e,I,void 0,!0),ne(e)});const Pe=document.createElement("button");Pe.type="button",Pe.className="shaderops-shape-stack-item-button",Pe.textContent=w.label,Pe.addEventListener("click",()=>{const I={[w.enabledParam]:1,[m.selectedParam]:w.value};Object.assign(t.params,I),Yt(e,I,void 0,!0),ne(e)}),H.appendChild(ae),H.appendChild(Pe),te.appendChild(H)}),b.appendChild(te)}if(k&&Array.isArray(k.controls)&&Number(t.params[k.enabledParam])>.5){const te=document.createElement("div");te.className="shaderops-shape-stack-detail",k.controls.filter(w=>!w.hidden&&lt(w,t.params)).forEach(w=>{const H=document.createElement("div");H.dataset.controlId=w.id;const ae=document.createElement("label"),Pe=String(w.label||w.id);if(ae.textContent=Pe,ae.title=s(Pe,w.hint),w.type==="select"){H.className="filter-param-row filter-param-row--select";const U=document.createElement("select");U.className="filter-param-select";const Dt=t.params[w.id]??w.default??0;(w.options||[]).forEach(ze=>{const He=document.createElement("option");He.value=String(ze.value),He.textContent=ze.label,String(ze.value)===String(Dt)&&(He.selected=!0),U.appendChild(He)}),U.addEventListener("change",()=>{const ze=U.value,He=Number(ze),$n=Number.isFinite(He)&&ze.trim()!==""?He:ze;t.params[w.id]=$n,tt(e,w.id,$n,!0)});const Oe=document.createElement("span");Oe.className="shaderops-lock-wrap shaderops-lock-wrap--aux shaderops-lock-wrap--full",Oe.appendChild(U),Oe.appendChild(r(w.id,Oe)),H.appendChild(ae),H.appendChild(Oe),te.appendChild(H);return}H.className="filter-param-row";const I=document.createElement("input");I.type="range",I.min=String(w.min??0),I.max=String(w.max??1),I.step=String(w.step??.01),w.pillMode&&(I.dataset.pillMode=String(w.pillMode));const Et=t.params[w.id]??w.default??0;I.value=String(Et);const Ie=document.createElement("span");Ie.className="shaderops-lock-wrap";const Ue=document.createElement("input");Ue.type="number",Ue.step=I.step,Ue.value=String(Et);const _e=document.createElement("button");_e.type="button",_e.className="shaderops-lock-button",_e.innerHTML=Aa();const Fn=()=>{const U=t.lockedIds.has(w.id);_e.classList.toggle("is-locked",U),Ie.classList.toggle("is-locked",U),_e.title=U?"Unlock this value":"Lock this value (skip on All Random)"};Fn(),_e.addEventListener("click",U=>{U.preventDefault(),U.stopPropagation(),t.lockedIds.has(w.id)?t.lockedIds.delete(w.id):t.lockedIds.add(w.id),Gt(e,t.lockedIds),Fn()});const Ca=(U,Dt)=>{t.params[w.id]=U,tt(e,w.id,U,Dt)};I.addEventListener("input",()=>{const U=Number(I.value);Ue.value=I.value,Ca(U,!1)}),I.addEventListener("change",()=>{Ca(Number(I.value),!0)}),Ue.addEventListener("change",()=>{let U=Number(Ue.value);Number.isFinite(U)||(U=Number(I.value));const Dt=Number(I.min),Oe=Number(I.max);I.value=String(Math.min(Oe,Math.max(Dt,U))),Ca(U,!0)}),Ie.appendChild(Ue),Ie.appendChild(_e),H.appendChild(ae),H.appendChild(I),H.appendChild(Ie),te.appendChild(H)}),b.appendChild(te)}g.appendChild(S),g.appendChild(b),f.appendChild(g);return}if(m.type==="image-loader"){g.className="filter-param-row filter-param-row--image-loader";const b=ua(e),T=document.createElement("button");T.type="button",T.className="image-loader-load",T.textContent="Load Image",T.title="Load an image â€” strokes will sample its colors",T.addEventListener("click",()=>Tr(e));const N=document.createElement("span");N.className="image-loader-name",N.textContent=b.loaded?b.name:Je,N.title=b.loaded?b.name:"Using default image";const k=document.createElement("button");k.type="button",k.className="image-loader-clear",k.textContent="✕",k.title="Reset to Default image",k.hidden=!b.loaded,k.addEventListener("click",()=>Lr(e));const P=document.createElement("span");P.className="image-loader-wrap",P.appendChild(T),P.appendChild(N),P.appendChild(k),g.appendChild(S),g.appendChild(P),f.appendChild(g);return}if(m.type==="toggle"){g.className="filter-param-row filter-param-row--toggle";const b=Number(t.params[m.id]??m.default??0)>.5,T=document.createElement("label");T.className="filter-param-switch",T.innerHTML=`
        ...
        `;const N=T.querySelector(".filter-param-toggle");N.addEventListener("change",()=>{const P=N.checked?1:0;t.params[m.id]=P,tt(e,m.id,P,!0),ne(e)});const k=document.createElement("span");k.className="shaderops-lock-wrap shaderops-lock-wrap--aux",k.appendChild(T),k.appendChild(r(m.id,k)),g.appendChild(S),g.appendChild(k),f.appendChild(g);return}if(m.type==="select"){g.className="filter-param-row filter-param-row--select";const b=document.createElement("select");b.className="filter-param-select";const T=t.params[m.id]??m.default??0;(m.options||[]).forEach(k=>{const P=document.createElement("option");P.value=String(k.value),P.textContent=k.label,String(k.value)===String(T)&&(P.selected=!0),b.appendChild(P)}),b.addEventListener("change",()=>{const k=b.value,P=Number(k),J=Number.isFinite(P)&&k.trim()!==""?P:k;t.params[m.id]=J,tt(e,m.id,J,!0),ne(e)});const N=document.createElement("span");N.className="shaderops-lock-wrap shaderops-lock-wrap--aux shaderops-lock-wrap--full",N.appendChild(b),N.appendChild(r(m.id,N)),g.appendChild(S),g.appendChild(N),f.appendChild(g);return}g.className="filter-param-row";const x=document.createElement("input");x.type="range",x.min=String(m.min??0),x.max=String(m.max??1),x.step=String(m.step??.01),m.pillMode&&(x.dataset.pillMode=String(m.pillMode));const X=t.params[m.id]??m.default??0;x.value=String(X);const ce=document.createElement("span");ce.className="shaderops-lock-wrap";const ee=document.createElement("input");ee.type="number",ee.step=x.step,ee.value=String(X);const ue=document.createElement("button");ue.type="button",ue.className="shaderops-lock-button",ue.innerHTML=Aa();const Ct=()=>{const b=t.lockedIds.has(m.id);ue.classList.toggle("is-locked",b),ce.classList.toggle("is-locked",b),ue.title=b?"Unlock this value":"Lock this value (skip on All Random)"};Ct(),ue.addEventListener("click",b=>{b.preventDefault(),b.stopPropagation(),t.lockedIds.has(m.id)?t.lockedIds.delete(m.id):t.lockedIds.add(m.id),Gt(e,t.lockedIds),Ct()});const et=(b,T)=>{t.params[m.id]=b,tt(e,m.id,b,T)};x.addEventListener("input",()=>{const b=Number(x.value);ee.value=x.value,et(b,!1)}),x.addEventListener("change",()=>{et(Number(x.value),!0)}),ee.addEventListener("change",()=>{let b=Number(ee.value);Number.isFinite(b)||(b=Number(x.value));const T=Number(x.min),N=Number(x.max);x.value=String(Math.min(N,Math.max(T,b))),et(b,!0)}),ce.appendChild(ee),ce.appendChild(ue),g.appendChild(S),g.appendChild(x),g.appendChild(ce),f.appendChild(g)}),zt.appendChild(f)}),cn(zt),_o(!!t.extras.paused),Ft(e)}function yn(e){const t=G.get(e);if(t){if((t.schema||[]).some(a=>a.type==="shape-stack")){ne(e);return}zt.querySelectorAll(".filter-param-row").forEach(a=>{const n=a.dataset.controlId;if(!(n in t.params))return;const o=a.querySelector('input[type="range"]'),i=a.querySelector('input[type="number"]'),r=a.querySelector("select.filter-param-select"),s=a.querySelector("input.filter-param-toggle"),c=t.params[n];o&&document.activeElement!==o&&(o.value=String(c)),i&&document.activeElement!==i&&(i.value=String(c)),r&&document.activeElement!==r&&(r.value=String(c)),s&&document.activeElement!==s&&(s.checked=Number(c)>.5)}),_o(!!t.extras.paused)}}function _r(e,t=null){const{type:a,projectId:n}=e;if(n){if(a==="shaderops/ready"){const o=Array.isArray(e.schema)?e.schema:[];if(G.set(n,{schema:o,params:{...e.values||{}},extras:{...e.extras||{}},lockedIds:Ho(n)}),da(o)){const i=t&&typeof t.postMessage=="function"?t:D.contentWindow;Ar(n,i)}l.selectedId===n&&(ne(n),Oo(),st())}else if(a==="shaderops/values-changed"){const o=G.get(n);if(!o)return;if(e.values&&Object.assign(o.params,e.values),e.extras&&Object.assign(o.extras,e.extras),o.pendingLockRestore){const r=o.pendingLockRestore;o.pendingLockRestore=null,Object.entries(r).forEach(([s,c])=>{o.params[s]=c,tt(n,s,c,!0)})}const i=nt.get(n);i&&i.length&&(nt.set(n,[]),i.forEach(r=>{try{r()}catch{}})),l.selectedId===n&&yn(n)}else if(a==="shaderops/keydown"){if(l.selectedId!==n)return;e.key==="r"?fa(n):e.key==="space"&&$e(n,"togglePause")}}}mr.addEventListener("click",()=>{bt.classList.remove("is-collapsed"),E.setItem(hn,"0")});function Xo(){bt.classList.add("is-collapsed"),E.setItem(hn,"1")}bt.addEventListener("dblclick",e=>{e.target.closest(".unified-panel-body")||Xo()});hr.addEventListener("click",()=>{const e=Ze();e&&fa(e)});vr.addEventListener("click",()=>{const e=Ze();e&&$e(e,"rerollSeed")});Ht.addEventListener("click",()=>{const e=Ze();e&&$e(e,"togglePause")});gr.addEventListener("click",()=>{const e=Ze();e&&$e(e,"undo")});yr.addEventListener("click",()=>{const e=Ze();e&&$e(e,"redo")});br?.addEventListener("click",()=>{window.location.reload()});Vo(Io,{onHoldComplete:()=>{const e=Ze();e&&$r(e)}});document.addEventListener("keydown",e=>{const t=Ze();t&&(e.target?.matches("input, textarea, select, button")||((e.key==="r"||e.key==="R")&&!e.metaKey&&!e.ctrlKey&&!e.altKey?(e.preventDefault(),fa(t)):e.code==="Space"&&(e.preventDefault(),$e(t,"togglePause"))))});E.getItem(hn)==="1"&&bt.classList.add("is-collapsed");function xe(e,t){if(e==null)return t;try{const a=JSON.parse(e);return a??t}catch{return t}}function Te(e){return e!==null&&typeof e=="object"&&!Array.isArray(e)}function Or(){const e="ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789";let t="";for(let a=0;a<3;a++)t+=e[Math.floor(Math.random()*e.length)];return t}function Ko(e,t=""){const a=new Date,n=String(a.getFullYear()),o=String(a.getMonth()+1).padStart(2,"0"),i=String(a.getDate()).padStart(2,"0"),r=String(a.getHours()).padStart(2,"0"),s=String(a.getMinutes()).padStart(2,"0"),c=String(a.getSeconds()).padStart(2,"0"),u=String(t||"").trim(),d=u?`-${u}`:"";return`${e||"shader"}-${n}${o}${i}-${r}${s}${c}${d}-${Or()}.png`}function zr(e){const t=G.get(e),a=JSON.parse(xt(e));return{source:"shaderops-png-state",schemaVersion:Ua,exportedAt:new Date().toISOString(),projectId:e,bridge:{params:t?{...t.params}:{},extras:t?{...t.extras}:{},lockedIds:t&&t.lockedIds?[...t.lockedIds]:[]},filters:{previewOn:Ne(e),stack:Array.isArray(a)?a:[]}}}function Hr(e,t=2500){return G.has(e)?Promise.resolve(!0):new Promise(a=>{const n=performance.now(),o=()=>{if(G.has(e)){a(!0);return}if(performance.now()-n>=t){a(!1);return}requestAnimationFrame(o)};o()})}const We=new Uint8Array([137,80,78,71,13,10,26,10]),Zt=new TextEncoder,Yo=new TextDecoder,qr=(()=>{const e=new Uint32Array(256);for(let t=0;t<256;t++){let a=t;for(let n=0;n<8;n++)a=a&1?3988292384^a>>>1:a>>>1;e[t]=a>>>0}return e})();function Gr(e){let t=4294967295;for(let a=0;a<e.length;a++)t=qr[(t^e[a])&255]^t>>>8;return(t^4294967295)>>>0}function Wr(e,t){const a=new Uint8Array(12+t.length),n=new DataView(a.buffer);n.setUint32(0,t.length),a.set(e,4),a.set(t,8);const o=new Uint8Array(4+t.length);return o.set(e,0),o.set(t,4),n.setUint32(8+t.length,Gr(o)),a}function jr(e){const t=e.reduce((o,i)=>o+i.length,0),a=new Uint8Array(t);let n=0;for(const o of e)a.set(o,n),n+=o.length;return a}function Vr(e){if(!e||typeof e!="string")return null;const t=e.indexOf(",");if(t===-1)return null;const a=e.slice(t+1),n=atob(a),o=new Uint8Array(n.length);for(let i=0;i<n.length;i++)o[i]=n.charCodeAt(i);return o}function Xr(e,t){const a=Zt.encode(e),n=Zt.encode(t),o=new Uint8Array(a.length+5+n.length);o.set(a,0);let i=a.length;return o[i++]=0,o[i++]=0,o[i++]=0,o[i++]=0,o[i++]=0,o.set(n,i),o}function Kr(e,t,a){if(!e||e.length<12)throw new Error("PNG data is empty.");for(let u=0;u<We.length;u++)if(e[u]!==We[u])throw new Error("Exported image is not a valid PNG.");const n=Zt.encode(t),o=Zt.encode("iTXt"),i=Wr(o,Xr(t,a)),r=[We];let s=We.length,c=!1;for(;s+12<=e.length;){const d=new DataView(e.buffer,e.byteOffset+s,8).getUint32(0),f=s+4,p=s+8,h=d+12;if(s+h>e.length)throw new Error("PNG chunk stream is truncated.");const v=e.subarray(f,f+4),m=e.subarray(p,p+d),S=v[0]===105&&v[1]===84&&v[2]===88&&v[3]===116&&m.length>=n.length+1&&m.subarray(0,n.length).every((x,X)=>x===n[X])&&m[n.length]===0,M=v[0]===73&&v[1]===69&&v[2]===78&&v[3]===68;S||(!c&&M&&(r.push(i),c=!0),r.push(e.subarray(s,s+h))),s+=h}return c||r.push(i),jr(r)}function Ba(e,t){let a=t;for(;a<e.length&&e[a]!==0;)a+=1;return{text:Yo.decode(e.subarray(t,a)),next:a+1}}function Yr(e,t){if(!e||e.length<12)return null;for(let n=0;n<We.length;n++)if(e[n]!==We[n])return null;let a=We.length;for(;a+12<=e.length;){const o=new DataView(e.buffer,e.byteOffset+a,8).getUint32(0),i=a+4,r=a+8,s=o+12;if(a+s>e.length)return null;const c=e.subarray(i,i+4),u=e.subarray(r,r+o),d=String.fromCharCode(c[0],c[1],c[2],c[3]);if(d==="iTXt"){const f=Ba(u,0);if(f.text===t&&f.next+3<=u.length){const p=u[f.next];let h=f.next+2;if(h=Ba(u,h).next,h=Ba(u,h).next,p===0&&h<=u.length)return Yo.decode(u.subarray(h))}}if(d==="IEND")break;a+=s}return null}function Jr(e){return Te(e)?{stack:(Array.isArray(e.stack)?e.stack:[]).map(n=>!Te(n)||typeof n.defId!="string"||!Re(n.defId)?null:{instanceId:typeof n.instanceId=="string"?n.instanceId:`f${Date.now()}_${Qo++}`,defId:n.defId,enabled:n.enabled!==!1,collapsed:!!n.collapsed,params:{...tn(n.defId),...Te(n.params)?n.params:{}}}).filter(Boolean),previewOn:e.previewOn!==!1}:null}async function Zr(e){if(!Te(e))throw new Error("PNG metadata is invalid JSON.");const t=Number(e.schemaVersion)||1;t>Ua&&console.warn(`PNG state schema v${t} is newer than this app (v${Ua}); applying compatible fields only.`);const a=typeof e.projectId=="string"?e.projectId:null,o=a&&l.projects.some(c=>c.id===a)?a:l.selectedId;if(!o)throw new Error("No target project is available for restore.");if(l.selectedId!==o&&await In(o),!await Hr(o))throw new Error("Project controls are not ready yet. Please try dropping the PNG again.");const r=G.get(o);if(r&&Te(e.bridge)){const c=Te(e.bridge.params)?{...e.bridge.params}:{},u=Te(e.bridge.extras)?{...e.bridge.extras}:void 0;Yt(o,c,u,!0),Object.assign(r.params,c),u&&Object.assign(r.extras,u),Array.isArray(e.bridge.lockedIds)&&(r.lockedIds=new Set(e.bridge.lockedIds.filter(d=>typeof d=="string")),Gt(o,r.lockedIds)),l.selectedId===o&&yn(o)}const s=Jr(e.filters);s&&(l.filterStacks[o]=s.stack,l.filterPreviewOn[o]=s.previewOn,me(),l.selectedId===o&&F()),we()}async function Qr(e){if(!e||!/\.png$/i.test(e.name||""))return!1;const t=new Uint8Array(await e.arrayBuffer()),a=Yr(t,Co);if(!a)return!1;const n=xe(a,null);if(!n)throw new Error("PNG state chunk exists but contains invalid JSON.");return await Zr(n),!0}async function el(){const[e,t,a]=await Promise.all([qn(go,{}),qn(yo,{}),Xi(`${ln}${rn}:`)]);l.previews=e&&typeof e=="object"&&!Array.isArray(e)?e:{},l.previewRevisions=t&&typeof t=="object"&&!Array.isArray(t)?t:{},a.forEach(([n,o])=>{o&&typeof o=="object"&&!Array.isArray(o)&&la.set(n,o)})}function tl(){const e=xe(E.getItem(xo),[]);l.favorites=Array.isArray(e)?e:[]}function Jo(){E.setItem(xo,JSON.stringify(l.favorites))}function bn(){E.setItem(wo,JSON.stringify(l.projects.map(e=>e.id)))}function al(){const e=xe(E.getItem(wo),null);if(!Array.isArray(e)||e.length===0)return;const t=new Map(l.projects.map(i=>[i.id,i])),a=e.map(i=>t.get(i)).filter(Boolean),n=new Set(a.map(i=>i.id)),o=l.projects.filter(i=>!n.has(i.id));l.projects=[...a,...o]}function nl(){const e=xe(E.getItem(ko),null);l.filterAddOrder=e&&typeof e=="object"?e:{}}function ol(){E.setItem(ko,JSON.stringify(l.filterAddOrder))}function il(e,t){const a=l.filterAddOrder&&l.filterAddOrder[e];if(!Array.isArray(a)||a.length===0)return t;const n=new Map(t.map(s=>[s.id,s])),o=a.map(s=>n.get(s)).filter(Boolean),i=new Set(o.map(s=>s.id)),r=t.filter(s=>!i.has(s.id));return[...o,...r]}async function rl(){const e={...l.previews};Object.keys(e).forEach(t=>{const a=l.currentProjectRevisions[t];a&&(l.previewRevisions[t]=a)});try{await Promise.all([_a(go,e),_a(yo,l.previewRevisions)])}catch(t){console.warn("ShaderOps: unable to persist the IndexedDB thumbnail cache.",t)}}function Zo(e,t=320,a=.72){return new Promise(n=>{if(!e||typeof e!="string"||!e.startsWith("data:image")){n(e);return}const o=new Image;o.onload=()=>{const i=Math.min(1,t/(o.width||t)),r=Math.max(1,Math.round((o.width||t)*i)),s=Math.max(1,Math.round((o.height||t)*i));try{const c=document.createElement("canvas");c.width=r,c.height=s,c.getContext("2d").drawImage(o,0,0,r,s),n(c.toDataURL("image/jpeg",a))}catch{n(e)}},o.onerror=()=>n(e),o.src=e})}function ll(){const e=xe(E.getItem(bo),{}),t=e&&typeof e=="object"&&!Array.isArray(e)?e:{};Object.entries(t).forEach(([a,n])=>{if(!n||typeof n!="object")return;const o=Array.isArray(n.stack)?n.stack:[];l.filterStacks[a]=o.map(i=>!i||typeof i!="object"?i:{...i,params:{...tn(i.defId),...i.params}}),l.filterPreviewOn[a]=n.previewOn!==!1}),fn.classList.toggle("collapsed",E.getItem(sn)==="1")}function me(){const e={};Object.keys(l.filterStacks).forEach(t=>{e[t]={stack:l.filterStacks[t],previewOn:l.filterPreviewOn[t]!==!1}}),E.setItem(bo,JSON.stringify(e))}function W(e){return l.filterStacks[e]||(l.filterStacks[e]=[]),l.filterStacks[e]}function Ne(e){return l.filterPreviewOn[e]!==!1}function Xn(e){return e?Ne(e)&&W(e).some(t=>t&&t.enabled):!1}function ma(e){return l.filterHistory[e]||(l.filterHistory[e]={undoStack:[],redoStack:[],suppress:!1,limit:100}),l.filterHistory[e]}function xt(e){return JSON.stringify(W(e))}function B(e,t){if(!e)return;const a=ma(e);if(a.suppress)return;const n=xt(e);a.undoStack[a.undoStack.length-1]!==n&&(a.undoStack.push(n),a.undoStack.length>a.limit&&a.undoStack.shift(),a.redoStack.length=0)}function xn(e,t){const a=ma(e);let n;try{n=JSON.parse(t)}catch{return}a.suppress=!0,l.filterStacks[e]=n,l.filterPreviewOn[e]===void 0&&(l.filterPreviewOn[e]=!0),me(),F(),!C.hidden&&z&&l.selectedId===e&&pe(),!A.hidden&&O&&l.selectedId===e&&Me(),a.suppress=!1}function sl(e){if(!e)return;const t=ma(e);if(t.undoStack.length===0)return;const a=xt(e),n=t.undoStack.pop();t.redoStack.push(a),xn(e,n)}function cl(e){if(!e)return;const t=ma(e);if(t.redoStack.length===0)return;const a=xt(e),n=t.redoStack.pop();t.undoStack.push(a),xn(e,n)}let Qo=1;function ul(e,t){if(!Re(t))return;B(e),W(e).unshift({instanceId:`f${Date.now()}_${Qo++}`,defId:t,enabled:!0,collapsed:!1,params:tn(t)}),l.filterPreviewOn[e]===void 0&&(l.filterPreviewOn[e]=!0),me(),F()}function dl(e,t){const a=W(e),n=a.findIndex(o=>o.instanceId===t);n!==-1&&(B(e),a.splice(n,1),me(),F())}function fl(e,t){const n=W(e).find(o=>o.instanceId===t);n&&(B(e),n.enabled=!n.enabled,me(),F())}function pl(e,t){const n=W(e).find(o=>o.instanceId===t);n&&(n.collapsed=!n.collapsed,me(),F())}function Ee(e,t,a,n,o){const r=W(e).find(s=>s.instanceId===t);r&&(o||B(e),r.params[a]=n,me())}const ml={oilPaint:{key:"preset",values:{custom:null,paletteKnife:{brushType:"palette",brushSize:10.5,strokeStrength:.84,detail:.74,edgeBlend:.33,strokePresence:.86,mix:.92},softBrush:{brushType:"round",brushSize:14,strokeStrength:.62,detail:.52,edgeBlend:.72,strokePresence:.56,mix:.78}}},filmEmulation:{key:"stock",values:{neutral:{grain:.35,grainSize:1.1,grainAnimated:!0,halation:.25,haloSize:2.6,contrast:.05,saturation:1,fade:.02,temperature:0,tint:0,vignette:.25},portra:{grain:.45,grainSize:1.4,grainAnimated:!0,halation:.55,haloSize:3.2,contrast:.12,saturation:1,fade:.08,temperature:0,tint:0,vignette:.4},velvia:{grain:.38,grainSize:1.2,grainAnimated:!0,halation:.4,haloSize:2.8,contrast:.28,saturation:1.45,fade:.05,temperature:.05,tint:.03,vignette:.45},trix:{grain:.9,grainSize:1,grainAnimated:!0,halation:.2,haloSize:2.3,contrast:.45,saturation:0,fade:.1,temperature:-.02,tint:.02,vignette:.55},cinestill:{grain:.65,grainSize:1.8,grainAnimated:!0,halation:1.4,haloSize:4.5,contrast:.18,saturation:1.05,fade:.1,temperature:-.2,tint:.05,vignette:.5},bleach:{grain:.5,grainSize:1.3,grainAnimated:!0,halation:.35,haloSize:2.8,contrast:.6,saturation:.55,fade:.03,temperature:0,tint:0,vignette:.62}}}};function hl(e,t,a,n){const i=W(e).find(c=>c.instanceId===t);if(!i)return;const r=ml[i.defId||i.filterId];if(!r||r.key!==a)return;const s=r.values?.[String(n)];!s||typeof s!="object"||Object.entries(s).forEach(([c,u])=>{Ee(e,t,c,u,!0)})}const Kn=["Color","Distort","Blur","Effect"];function vl(){const e=new Map;lo.forEach(a=>{const n=a.group||"Other";e.has(n)||e.set(n,[]),e.get(n).push(a)});const t=[...Kn.filter(a=>e.has(a)),...[...e.keys()].filter(a=>!Kn.includes(a))];Z.innerHTML=t.map(a=>`
        ...
        </div>`).join(""),Z.querySelectorAll(".filter-add-grid[data-group]").forEach(a=>{gl(a,a.dataset.group)})}function gl(e,t){let a=null;e.addEventListener("dragstart",n=>{const o=n.target.closest(".filter-add-option");if(!o){n.preventDefault();return}a=o,o.classList.add("dragging"),n.dataTransfer.effectAllowed="move",n.dataTransfer.setData("text/plain",o.dataset.filterId)}),e.addEventListener("dragover",n=>{if(!a)return;n.preventDefault(),n.dataTransfer.dropEffect="move";const o=n.target.closest(".filter-add-option");if(!o||o===a){o||e.appendChild(a);return}const i=o.getBoundingClientRect();n.clientX<i.left+i.width/2?o.parentNode.insertBefore(a,o):o.parentNode.insertBefore(a,o.nextSibling)}),e.addEventListener("drop",n=>{if(n.preventDefault(),!a)return;const o=[...e.querySelectorAll(".filter-add-option")].map(i=>i.dataset.filterId);l.filterAddOrder[t]=o,ol()}),e.addEventListener("dragend",()=>{a=null,e.querySelectorAll(".filter-add-option.dragging").forEach(n=>n.classList.remove("dragging"))})}document.body.appendChild(Z);function ei(){const e=ca.getBoundingClientRect(),t=Z.querySelectorAll(".filter-add-group").length||4,a=t*150+(t-1)*14+20,n=Math.max(320,Math.min(a,window.innerWidth-16));let o=e.right-n;o=Math.max(8,Math.min(o,window.innerWidth-n-8)),Z.style.width=`${n}px`,Z.style.left=`${o}px`,Z.style.top=`${e.bottom+6}px`}function yl(){ei(),Z.hidden=!1,ca.classList.add("open")}function ha(){Z.hidden=!0,ca.classList.remove("open")}window.addEventListener("resize",()=>{Z.hidden||ei()});function ti(e,t,a,n=48){let o="";for(let i=0;i<=n;i++){const r=i/n,s=Si(e,r),c=r*t,u=(1-s)*a;o+=i===0?`M${c.toFixed(2)},${u.toFixed(2)}`:` L${c.toFixed(2)},${u.toFixed(2)}`}return o}function ai(e,t,a,n,o,i=56){const r=Math.max(0,Math.min(.99,e)),s=Math.max(r+.001,Math.min(1,a)),c=Math.max(.05,t);let u="";for(let d=0;d<=i;d++){const f=d/i,p=Math.max(0,Math.min(1,(f-r)/(s-r))),h=Math.pow(p,1/c),v=f*n,m=(1-h)*o;u+=d===0?`M${v.toFixed(2)},${m.toFixed(2)}`:` L${v.toFixed(2)},${m.toFixed(2)}`}return u}function ni(e,t,a){const n=Math.max(0,Math.min(.99,e)),o=Math.max(n+.001,Math.min(1,a)),i=Math.max(.05,t),r=n+(o-n)*Math.pow(.5,i);return{black:n,mid:Math.max(n,Math.min(o,r)),white:o}}function wn(e){if(!e)return;const t=_.querySelector(`.filter-levels-graph[data-instance-id="${e}"]`);if(!t)return;const a=(m,g)=>{const S=_.querySelector(`input[data-instance-id="${e}"][data-param-key="${m}"]`),M=S?Number(S.value):g;return Number.isFinite(M)?M:g},n=a("blackPoint",0),o=a("gamma",1),i=a("whitePoint",1),r=t.querySelector(".filter-levels-curve"),s=t.querySelector(".filter-levels-marker--black"),c=t.querySelector(".filter-levels-marker--mid"),u=t.querySelector(".filter-levels-marker--white"),d=t.querySelector('.filter-levels-handle[data-handle="black"]'),f=t.querySelector('.filter-levels-handle[data-handle="mid"]'),p=t.querySelector('.filter-levels-handle[data-handle="white"]');if(!r||!s||!c||!u)return;const h=ai(n,o,i,100,46,56),v=ni(n,o,i);r.setAttribute("d",h),s.setAttribute("x1",(v.black*100).toFixed(2)),s.setAttribute("x2",(v.black*100).toFixed(2)),c.setAttribute("x1",(v.mid*100).toFixed(2)),c.setAttribute("x2",(v.mid*100).toFixed(2)),u.setAttribute("x1",(v.white*100).toFixed(2)),u.setAttribute("x2",(v.white*100).toFixed(2)),d&&(d.style.left=`${(v.black*100).toFixed(2)}%`),f&&(f.style.left=`${(v.mid*100).toFixed(2)}%`),p&&(p.style.left=`${(v.white*100).toFixed(2)}%`)}function Ta(e,t,a){Ee(l.selectedId,e,t,a,!0),_.querySelectorAll(`input[data-instance-id="${e}"][data-param-key="${t}"]`).forEach(n=>{n.value=a})}function bl(e,t){const a=e.params[t.key];if(t.type==="select"){const n=(t.options||[]).map(o=>`<option value="${o.value}" ${a===o.value?"selected":""}>${o.label}</option>`).join("");return`
      ...
      </div>`}if(t.type==="curve"){const n=Tt(a),o=ti(n,100,100,40);return`
      ...
      </div>`}if(t.type==="gradient"){const n=gt(a),o=ro(n);return`
      ...
      </div>`}if(t.type==="levelsGraph"){const n=Number.isFinite(e.params[t.blackKey])?e.params[t.blackKey]:0,o=Number.isFinite(e.params[t.gammaKey])?e.params[t.gammaKey]:1,i=Number.isFinite(e.params[t.whiteKey])?e.params[t.whiteKey]:1,r=ai(n,o,i,100,46,56),s=ni(n,o,i);return`
    ...
    <div class="filter-param-row">
    ...
    </div>`}function F(){const e=l.selectedId,t=e?W(e):[];if(tr.style.display=t.length===0?"block":"none",Xt.checked=e?Ne(e):!1,Xt.disabled=!e,Eo.disabled=!e,he.disabled=!e,_.innerHTML=t.map((a,n)=>{const o=Re(a.defId);if(!o)return"";const i=o.params.filter(r=>lt(r,a.params)).map(r=>bl(a,r)).join("");return`
      ...
      </div>`}).join(""),z&&!C.hidden){const a=_.querySelector(`[data-action="open-curve"][data-instance-id="${z.instanceId}"][data-param-key="${z.paramKey}"]`);a?(z.trigger=a,Ln()):wa()}if(O&&!A.hidden){const a=_.querySelector(`[data-action="open-gradient"][data-instance-id="${O.instanceId}"][data-param-key="${O.paramKey}"]`);a?(O.trigger=a,vi()):xa()}cn(_),_.querySelectorAll(".filter-levels-graph[data-instance-id]").forEach(a=>{wn(a.dataset.instanceId)}),xl()}let Yn=null,Qt=null,ea=null;function xl(){clearTimeout(Yn),Yn=setTimeout(wl,220)}function wt(){clearTimeout(Qt),ea||(ea=setTimeout(ta,3e3)),Qt=setTimeout(ta,700)}function ta(){clearTimeout(Qt),clearTimeout(ea),Qt=null,ea=null,rl()}async function kn(e){if(!e||!ge||l.selectedId!==e)return!1;const t=W(e).filter(i=>i.enabled);if(R.style.display==="none"||!Ne(e)||t.length===0)return!1;const a=[...t].reverse(),n=ge.captureAtScale(a,na,1);if(!n)return!1;const o=await Zo(n.dataUrl);return l.selectedId!==e?!1:(l.previews[e]=o,wt(),Cn(e),Dn(e),!0)}function wl(){const e=l.selectedId;e&&kn(e).then(t=>{t||ba()})}function $t(e){const t=["linear-gradient(135deg,#2a3f88 0%,#18274f 100%)","linear-gradient(135deg,#5d2a83 0%,#251749 100%)","linear-gradient(135deg,#85481e 0%,#3a2112 100%)","linear-gradient(135deg,#165b67 0%,#0f2e35 100%)"];return t[e%t.length]}function oi(e){const t=e.entry.includes("?")?"&":"?";return`${e.entry}${t}project=${encodeURIComponent(e.id)}`}function ii(e,t){if(!e||e.getElementById("shaderopsUiEnhancements")||!e.getElementById("app")&&!e.body)return;const a=e.createElement("style");a.id="shaderopsUiEnhancements",a.textContent=`
  ...
  `,e.head?.appendChild(a),cn(e);const n=e.getElementById("allRandomButton");n&&!n.querySelector("svg")&&(n.textContent="",n.title="All random (R)",n.innerHTML=`
      <svg viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
        <rect x="1.5" y="1.5" width="13" height="13" rx="3" stroke="currentColor" stroke-width="1.3"></rect>
        <circle cx="5" cy="5" r="1.15" fill="currentColor"></circle>
        <circle cx="11" cy="5" r="1.15" fill="currentColor"></circle>
      ...
      <svg viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
        ...
        </button>`}).join("")}function Cn(e){if(!e)return;const t=l.projects.findIndex(i=>i.id===e);if(t===-1)return;const a=l.previews[e],n=a?`background-image:url('${a}');background-size:cover;`:`background:${$t(t)};`,o=CSS.escape(e);document.querySelectorAll(`[data-project-id="${o}"]`).forEach(i=>i.setAttribute("style",n))}let Pt=null,re=null,Ke=null,Jn="rail",q=[],fe=0;function va(){Pt&&(clearTimeout(Pt),Pt=null)}function ke(){va(),re=null,Ke=null,q=[],fe=0,Q.hidden=!0}function Cl(e){return l.projects.find(t=>t.id===e)||null}function li(e,t){const a=Qe(e);return a.slots.length?a.slots.map((n,o)=>{const i=typeof n.preview=="string"&&n.preview?n.preview:null,r=l.previews[e]||null,s=i||r,c=s?`background-image:url('${s}');background-size:cover;background-position:center;${i?"":"opacity:0.48;"}`:t+"opacity:0.42;",u=s?`background-image:url('${s}');background-size:cover;background-position:center;`:t;return{slotId:n.id,image:s,hasRealImage:!!i,thumbStyle:c,heroStyle:u,title:i?`Checkpoint ${o+1}`:`Checkpoint ${o+1} (current project preview)`}}):[]}function En(){if(!q.length){jn.innerHTML='<span class="thumb-hover-preview__empty">No checkpoints</span>';return}jn.innerHTML=q.map((e,t)=>`<span class="thumb-hover-preview__cp${e.hasRealImage?"":" thumb-hover-preview__cp--empty"}${t===fe?" is-active":""}" style="${e.thumbStyle}" title="${e.title}"></span>`).join("")}function El(e,t){const a=Q.getBoundingClientRect(),n=e.getBoundingClientRect(),o=10,i=window.innerWidth-a.width-8,r=window.innerHeight-a.height-8;let s=n.right+o,c=n.top+n.height*.5-a.height*.5;if(t==="effect-panel"){const u=V.hidden?n:V.getBoundingClientRect();s=u.right+o,s>i&&(s=u.left-a.width-o)}else s>i&&(s=n.left-a.width-o);Q.style.left=`${Math.max(8,Math.min(s,i))}px`,Q.style.top=`${Math.max(8,Math.min(c,r))}px`}function Dl(e,t,a){const n=Cl(t);if(!n)return;const o=Math.max(0,l.projects.findIndex(u=>u.id===t)),i=`background:${$t(o)};`,r=l.previews[t]||null,s=r?`background-image:url('${r}');background-size:cover;background-position:center;`:i;q=li(t,i),fe=0;const c=q[0]?.heroStyle||s;xr.textContent=n.name,mn.setAttribute("style",c),En(),Q.dataset.source=a,Q.hidden=!1,El(e,a)}function Dn(e){if(Q.hidden||Ke!==e||!re?.isConnected)return;const t=Math.max(0,l.projects.findIndex(r=>r.id===e)),a=`background:${$t(t)};`,n=fe;q=li(e,a),fe=Math.min(n,Math.max(0,q.length-1));const o=l.previews[e],i=q[fe]?.heroStyle||(o?`background-image:url('${o}');background-size:cover;background-position:center;`:a);mn.setAttribute("style",i),En()}function si(e,t,a){va(),re=e,Ke=t,Jn=a,Pt=setTimeout(()=>{Pt=null,!(!re||!re.isConnected)&&Dl(re,Ke,Jn)},Wi)}function ci(e){if(!q.length)return;fe=(fe+e+q.length)%q.length;const t=q[fe]?.heroStyle;t&&mn.setAttribute("style",t),En()}function ui(e,t){if(Ke!==e||q.length===0||t&&re&&re.dataset.projectId!==t.dataset.projectId)return null;const a=q[fe]?.slotId;return a||(Qe(e).slots[fe]?.id??null)}function di(e,t){if(!t)return;if(G.has(e)){Ja(e,t);return}let a=0;const n=setInterval(()=>{if(a+=1,G.has(e)){clearInterval(n),Ja(e,t);return}a>30&&clearInterval(n)},50)}function Ml(e){const t=l.projects.findIndex(n=>n.id===e);if(t===-1)return;const a=ri();l.navPage=Math.floor(t/a),we()}const Pl='<svg viewBox="0 0 16 16" fill="currentColor" aria-hidden="true"><path d="M8 1.4l1.98 4.02 4.44.65-3.21 3.13.76 4.42L8 11.55l-3.97 2.07.76-4.42-3.21-3.13 4.44-.65z"/></svg>',Il='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.1" stroke-linejoin="round" aria-hidden="true"><path d="M8 1.4l1.98 4.02 4.44.65-3.21 3.13.76 4.42L8 11.55l-3.97 2.07.76-4.42-3.21-3.13 4.44-.65z"/></svg>';function La(e,t){const a=e.id===l.selectedId,n=l.favorites.includes(e.id),o=l.previews[e.id],i=o?`background-image:url('${o}');background-size:cover;`:`background:${$t(t)};`;return`
    ...
    </div>`}function aa(e){return!e||typeof e!="object"?!1:Array.isArray(e.tags)&&e.tags.some(t=>String(t).toLowerCase()==="3d")?!0:/\b3d\b/i.test(String(e.engine||""))}function ga(){const e=qa.closest(".nav-grid-section"),t=l.favorites.map(c=>l.projects.find(u=>u.id===c)).filter(Boolean);e.classList.toggle("empty",t.length===0),qa.innerHTML=t.map(c=>La(c,l.projects.indexOf(c))).join("");const a=new Set(l.favorites),n=Ga.closest(".nav-grid-section"),o=Wa.closest(".nav-grid-section"),i=l.projects.filter(c=>!a.has(c.id)),r=i.filter(c=>!aa(c)),s=i.filter(c=>aa(c));n&&n.classList.toggle("empty",r.length===0),o&&o.classList.toggle("empty",s.length===0),Ga.innerHTML=r.map(c=>La(c,l.projects.indexOf(c))).join(""),Wa.innerHTML=s.map(c=>La(c,l.projects.indexOf(c))).join("")}function fi(){const e=new Set(l.favorites),t=l.favorites.map(n=>l.projects.find(o=>o.id===n)).filter(Boolean),a=l.projects.filter(n=>!e.has(n.id));l.projects=[...t,...a]}function Al(e){const t=l.favorites.indexOf(e);t===-1?l.favorites.push(e):l.favorites.splice(t,1),Jo(),fi(),bn(),ga(),we()}function Bl(){ga(),V.hidden=!1,un.hidden=!1,sa.classList.add("active")}function ya(){V.hidden||(V.hidden=!0,un.hidden=!0,sa.classList.remove("active"),ke())}function Mn(e,t){let a=null,n=null;e.addEventListener("dragstart",o=>{if(o.target.closest("[data-fav-toggle]")){o.preventDefault();return}const i=o.target.closest(".nav-grid-thumb");if(!i){o.preventDefault();return}a=i.dataset.projectId,n=i,i.classList.add("dragging"),o.dataTransfer.effectAllowed="move",o.dataTransfer.setData("text/plain",a)}),e.addEventListener("dragover",o=>{if(!n)return;o.preventDefault(),o.dataTransfer.dropEffect="move";const i=o.target.closest(".nav-grid-thumb");if(!i||i===n){i||e.appendChild(n);return}const r=i.getBoundingClientRect();o.clientX<r.left+r.width/2?i.parentNode.insertBefore(n,i):i.parentNode.insertBefore(n,i.nextSibling)}),e.addEventListener("drop",o=>{if(o.preventDefault(),!a)return;const i=[...e.querySelectorAll(".nav-grid-thumb")].map(r=>r.dataset.projectId);t(i)}),e.addEventListener("dragend",()=>{a=null,n=null,e.querySelectorAll(".nav-grid-thumb.dragging").forEach(o=>o.classList.remove("dragging"))})}function Pn(){const e=l.projects.find(t=>t.id===l.selectedId);if(!e){Oa.textContent="No project selected",za.textContent="â€”",D.src="about:blank",Ha.disabled=!0,Ia();return}Oa.textContent=e.name,za.textContent=e.engine,D.classList.add("frame-swapping"),D.src=oi(e),Ha.disabled=!1,E.setItem(Vt,e.id),Qa=null,F(),$o(e)?(Ia(),G.get(e.id)&&(ne(e.id),Oo()),requestAnimationFrame(()=>requestAnimationFrame(st))):(Ia(),Tl(D,e.id))}function st(){D.classList.remove("frame-swapping")}function Tl(e,t){let a=0;const n=()=>{if(e!==D||l.selectedId!==t)return;a+=1;let o=null;try{o=e.contentDocument}catch{o=null}if(o&&o.readyState!=="loading"&&(o.getElementById("app")||o.body)){try{ii(o,t)}catch(i){console.warn("ShaderOps: unable to install project UI enhancements (early).",i)}requestAnimationFrame(()=>requestAnimationFrame(st));return}a<300?requestAnimationFrame(n):st()};requestAnimationFrame(n),setTimeout(st,1500)}window.addEventListener("message",async e=>{const t=e.data;if(t&&typeof t.type=="string"&&t.type.startsWith("shaderops/")&&(t.type==="shaderops/ready"||t.type==="shaderops/values-changed"||t.type==="shaderops/keydown")){_r(t,e.source||null);return}const a=Sn(e.data);if(!a)return;const{projectId:n}=e.data;if(n&&n!==l.selectedId&&Xn(n)&&l.previews[n]||n===l.selectedId&&n&&Xn(n)&&(await kn(n)||l.previews[n])||!n||!l.projects.some(i=>i.id===n))return;const o=await Zo(a);l.projects.some(i=>i.id===n)&&(l.previews[n]=o,wt(),Cn(n),Dn(n))});function ba(){if(l.captureInFlight)return Promise.resolve();const e=l.projects.find(a=>a.id===l.selectedId),t=D.contentWindow;return!e||!t?Promise.resolve():kn(e.id).then(a=>{if(!a)return l.captureInFlight=!0,new Promise(n=>{const o=setTimeout(()=>{window.removeEventListener("message",i),l.captureInFlight=!1,n()},1500),i=r=>{Sn(r.data)&&r.data.projectId===e.id&&(clearTimeout(o),window.removeEventListener("message",i),l.captureInFlight=!1,n())};window.addEventListener("message",i),t.postMessage({type:"shaderops/request-preview",projectId:e.id},"*")})})}function Ll(e){try{const a=[...e.contentDocument?.querySelectorAll("canvas")||[]].filter(i=>i.width>8&&i.height>8).sort((i,r)=>r.width*r.height-i.width*i.height)[0];if(!a)return null;const n=Math.min(1,320/Math.max(a.width,a.height)),o=document.createElement("canvas");return o.width=Math.max(1,Math.round(a.width*n)),o.height=Math.max(1,Math.round(a.height*n)),o.getContext("2d").drawImage(a,0,0,o.width,o.height),o.toDataURL("image/jpeg",.68)}catch{return null}}function Rl(){return new Promise(e=>{"requestIdleCallback"in window?window.requestIdleCallback(e,{timeout:3e3}):setTimeout(e,80)})}async function Fl(e){return new Promise(t=>{const a=document.createElement("iframe");a.style.cssText="position:fixed;width:320px;height:180px;opacity:0.001;pointer-events:none;left:-10000px;top:-10000px;border:none;",document.body.appendChild(a);let n=!1;const o=()=>{n||(n=!0,clearTimeout(i),window.removeEventListener("message",r),a.remove(),t())},i=setTimeout(o,4500),r=s=>{Sn(s.data)&&s.data.projectId===e.id&&o()};window.addEventListener("message",r),a.addEventListener("load",()=>{let s=0;const c=()=>{if(!n){s+=1;try{a.contentWindow?.postMessage({type:"shaderops/request-preview",projectId:e.id},"*")}catch{}setTimeout(()=>{if(n)return;const u=Ll(a),d=l.projects.some(f=>f.id===e.id);if(u&&d&&!l.previews[e.id]){l.previews[e.id]=u,wt(),Cn(e.id),Dn(e.id),o();return}s<10&&setTimeout(c,180)},140)}};setTimeout(c,350)}),a.src=oi(e)})}async function $l(){if(!l.prewarmActive){for(l.prewarmActive=!0;l.prewarmQueue.length>0;){const e=l.prewarmQueue.shift();l.previews[e.id]||(await Rl(),await Fl(e))}l.prewarmActive=!1,wt()}}async function In(e){e!==l.selectedId&&(await ba(),l.selectedId=e,we(),Pn())}D.addEventListener("load",()=>{const e=l.projects.find(t=>t.id===l.selectedId);if(e){try{ii(D.contentDocument,e.id)}catch(t){console.warn("ShaderOps: unable to install project UI enhancements.",t)}st(),setTimeout(()=>{try{D.contentWindow?.postMessage({type:"shaderops/request-preview",projectId:e.id},"*")}catch{}},2e3)}});async function Nl(){const[e,t]=await Promise.all([fetch("/registry.json",{cache:"no-store"}),fetch("/project-revisions.json",{cache:"no-store"}).catch(()=>null)]);if(!e.ok)throw new Error(`Unable to load registry (${e.status})`);const a=await e.json(),n=t?.ok?await t.json():{};l.currentProjectRevisions=n&&typeof n=="object"&&!Array.isArray(n)?n:{};const o=Array.isArray(a.projects)?a.projects:[],i=await Promise.all(o.map(d=>kl(d)));l.projects=o.filter((d,f)=>i[f]),al();const r=new Set(l.projects.map(d=>d.id));let s=!1;Object.keys(l.previews).forEach(d=>{const f=l.currentProjectRevisions[d]&&l.previewRevisions[d]!==l.currentProjectRevisions[d];(!r.has(d)||f)&&(delete l.previews[d],delete l.previewRevisions[d],s=!0)}),Object.keys(l.previewRevisions).forEach(d=>{r.has(d)||delete l.previewRevisions[d]}),s&&wt();const c=E.getItem(Vt),u=l.projects.find(d=>d.id===c)?c:l.projects[0]?.id??null;l.selectedId=u,we(),Pn(),l.prewarmQueue=l.projects.filter(d=>!l.previews[d.id]),l.prewarmQueue.length>0&&setTimeout($l,3e3)}async function Ul(e){if(l.projects.find(a=>a.id===e))try{l.selectedId===e&&(await ba(),D.src="about:blank",await new Promise(o=>{let i=!1;const r=()=>{i||(i=!0,D.removeEventListener("load",s),clearTimeout(c),o())},s=()=>r(),c=setTimeout(r,1200);D.addEventListener("load",s,{once:!0})}));const a=await fetch(`/api/projects/${encodeURIComponent(e)}`,{method:"DELETE"}),n=await a.json().catch(()=>({}));if(!a.ok){const o=n?.detail?`: ${n.detail}`:"",i=n?.error?`${n.error}${o}`:`Delete failed (${a.status})`;throw new Error(i)}if(l.projects=l.projects.filter(o=>o.id!==e),delete l.previews[e],wt(),l.selectedId===e){const o=l.projects[0]?.id??null;l.selectedId=o,o?E.setItem(Vt,o):E.removeItem(Vt)}l.prewarmQueue=l.prewarmQueue.filter(o=>o.id!==e),we(),Pn()}catch(a){const n=a instanceof Error?a.message:String(a);window.alert(`Unable to delete project: ${n}`)}}ie.addEventListener("click",async e=>{const t=e.target.closest("[data-project-id]");if(!t)return;const a=t.dataset.projectId,n=ui(a,t);await In(a),di(a,n),ke()});ie.addEventListener("pointerover",e=>{const t=e.target.closest(".project-thumb[data-project-id]");!t||!ie.contains(t)||si(t,t.dataset.projectId,"rail")});ie.addEventListener("pointerout",e=>{const t=e.target.closest(".project-thumb[data-project-id]");if(!t)return;const a=e.relatedTarget;a&&(t.contains(a)||Q.contains(a))||(va(),re===t&&ke())});ie.addEventListener("pointerleave",e=>{const t=e.relatedTarget;t&&Q.contains(t)||ke()});function Zn(e){l.navPage+=e,we()}function pi(e){ie.classList.contains("paged")&&(e.preventDefault(),e.deltaY>0?Zn(1):e.deltaY<0&&Zn(-1))}ie.addEventListener("wheel",e=>{const t=e.target.closest(".project-thumb[data-project-id]");if(t&&!Q.hidden&&re===t&&Ke===t.dataset.projectId&&q.length>0){e.preventDefault();const a=Math.abs(e.deltaY)>=Math.abs(e.deltaX)?e.deltaY:e.deltaX;a!==0&&ci(a>0?1:-1);return}pi(e)},{passive:!1});it.addEventListener("wheel",pi,{passive:!1});let Qn=null;window.addEventListener("resize",()=>{clearTimeout(Qn),Qn=setTimeout(we,120),ke()});sa.addEventListener("click",e=>{e.stopPropagation(),V.hidden?Bl():ya()});V.addEventListener("click",e=>{const t=e.target.closest("[data-fav-toggle]");if(t){e.stopPropagation(),Al(t.dataset.favToggle);return}const a=e.target.closest(".nav-grid-thumb[data-project-id]");if(!a)return;const n=a.dataset.projectId,o=ui(n,a);ya(),Ml(n),In(n).then(()=>{di(n,o)}),ke()});V.addEventListener("pointerover",e=>{if(V.hidden)return;const t=e.target.closest(".nav-grid-thumb[data-project-id]");!t||!V.contains(t)||si(t,t.dataset.projectId,"effect-panel")});V.addEventListener("pointerout",e=>{const t=e.target.closest(".nav-grid-thumb[data-project-id]");if(!t)return;const a=e.relatedTarget;a&&(t.contains(a)||Q.contains(a))||(va(),re===t&&ke())});V.addEventListener("pointerleave",e=>{const t=e.relatedTarget;t&&Q.contains(t)||ke()});V.addEventListener("wheel",e=>{const t=e.target.closest(".nav-grid-thumb[data-project-id]");if(!t||Q.hidden||re!==t||Ke!==t.dataset.projectId||q.length===0)return;e.preventDefault(),e.stopPropagation();const a=Math.abs(e.deltaY)>=Math.abs(e.deltaX)?e.deltaY:e.deltaX;a!==0&&ci(a>0?1:-1)},{passive:!1});un.addEventListener("click",()=>ya());document.addEventListener("click",e=>{V.hidden||V.contains(e.target)||sa.contains(e.target)||ya()});document.addEventListener("dragstart",()=>ke());window.addEventListener("blur",()=>ke());Mn(qa,e=>{l.favorites=e,Jo(),fi(),bn(),ga(),we()});function mi(e,t){const a=new Set(l.favorites),n=l.favorites.map(f=>l.projects.find(p=>p.id===f)).filter(Boolean),o=l.projects.filter(f=>!a.has(f.id)&&!aa(f)),i=l.projects.filter(f=>!a.has(f.id)&&aa(f)),r=new Map(l.projects.map(f=>[f.id,f])),s=e.map(f=>r.get(f)).filter(Boolean),c=t==="3d"?i.length:o.length;if(s.length!==c)return;const u=t==="2d"?s:o,d=t==="3d"?s:i;l.projects=[...n,...u,...d],bn(),ga(),we()}Mn(Ga,e=>mi(e,"2d"));Mn(Wa,e=>mi(e,"3d"));Ha.addEventListener("click",()=>{if(!l.selectedId)return;const e=l.projects.find(t=>t.id===l.selectedId);dr.textContent=e?`Delete "${e.name}"? This removes its files permanently and cannot be undone.`:"This removes its files permanently and cannot be undone.",rt.hidden=!1});pr.addEventListener("click",()=>{rt.hidden=!0});rt.addEventListener("click",e=>{e.target===rt&&(rt.hidden=!0)});fr.addEventListener("click",()=>{rt.hidden=!0,l.selectedId&&Ul(l.selectedId)});window.addEventListener("beforeunload",()=>{ta();const e=D.contentWindow,t=l.projects.find(a=>a.id===l.selectedId);!e||!t||e.postMessage({type:"shaderops/request-preview",projectId:t.id},"*")});document.addEventListener("visibilitychange",()=>{document.visibilityState==="hidden"&&ta()});let ge=null,Qa=null,na=0,eo=performance.now()/1e3;const at=document.createElement("canvas"),to=at.getContext("2d",{willReadFrequently:!1});function _l(){return ge||(ge=new Ii(R),ge)}let en=null;const ot=new Map;let ao=null;function Ol(e,t){if(e){ao!==e&&(ot.clear(),ao=e);for(const[a,n]of t){let o=ot.get(a);o||(o=e.createElement("canvas"),o.className="fx-underlay-item",o.style.cssText="position:fixed;pointer-events:none;z-index:1;display:none;",e.body.appendChild(o),ot.set(a,o)),o.style.display="block",o.style.left=`${n.x}px`,o.style.top=`${n.y}px`,o.style.width=`${n.w}px`,o.style.height=`${n.h}px`}for(const[a,n]of ot)t.has(a)||(n.style.display="none")}}function zl(e,t){const a=e.width||0,n=e.height||0;if(!(a<=0||n<=0))for(const[o,i]of ot){if(i.style.display==="none")continue;const r=parseFloat(i.style.width)||0,s=parseFloat(i.style.height)||0;if(r<=0||s<=0)continue;const c=Math.max(1,Math.round(r*t)),u=Math.max(1,Math.round(s*t));i.width!==c&&(i.width=c),i.height!==u&&(i.height=u);const d=Math.max(0,Math.min(a-1,Math.round(parseFloat(i.style.left)*t))),f=Math.max(0,Math.min(n-1,Math.round(parseFloat(i.style.top)*t))),p=Math.max(1,Math.min(a-d,c)),h=Math.max(1,Math.min(n-f,u)),v=i.getContext("2d");if(v)try{v.clearRect(0,0,c,u),v.drawImage(e,d,f,p,h,0,0,c,u)}catch{}}}function Hl(e,t,a){const n=[],o=new Map,i=0;try{if(e)for(const f of["hud","panelToggleDock","shaderopsCheckpoints","shaderopsCheckpointMenu"]){const p=e.getElementById(f);if(!p)continue;const h=e.defaultView?.getComputedStyle(p);if(h?.display==="none"||h?.visibility==="hidden"||h?.contentVisibility==="hidden")continue;const v=p.getBoundingClientRect();if(v.width>0&&v.height>0){const m=v.left-i,g=v.top-i,S=Math.max(0,m),M=Math.max(0,g),x=Math.min(t,m+v.width+i*2),X=Math.min(a,g+v.height+i*2);x>S&&X>M&&(n.push({x:S,y:M,w:x-S,h:X-M}),o.set(f,{x:S,y:M,w:x-S,h:X-M}))}}}catch{}Ol(e,o);const r=`${t}x${a}|`+n.map(f=>`${f.x},${f.y},${f.w},${f.h}`).join(";");if(r===en)return;if(en=r,R.style.clipPath="none",!n.length){R.style.maskImage="none",R.style.webkitMaskImage="none";return}const s=[];for(const f of n){let p=f;for(let h=s.length-1;h>=0;h--){const v=s[h];if(p.x<v.x+v.w&&p.x+p.w>v.x&&p.y<v.y+v.h&&p.y+p.h>v.y){const g=Math.min(p.x,v.x),S=Math.min(p.y,v.y),M=Math.max(p.x+p.w,v.x+v.w),x=Math.max(p.y+p.h,v.y+v.h);p={x:g,y:S,w:M-g,h:x-S},s.splice(h,1)}}s.push(p)}const c=s.map(f=>`<rect x="${f.x}" y="${f.y}" width="${f.w}" height="${f.h}" fill="#000"/>`).join(""),u=`<svg xmlns="http://www.w3.org/2000/svg" width="${t}" height="${a}"><defs><mask id="m" maskUnits="userSpaceOnUse" x="0" y="0" width="${t}" height="${a}"><rect x="0" y="0" width="${t}" height="${a}" fill="#fff"/><g>${c}</g></mask></defs></svg>`,d=`url("data:image/svg+xml,${encodeURIComponent(u)}#m")`;R.style.maskImage=d,R.style.webkitMaskImage=d,R.style.maskMode="luminance",R.style.maskRepeat="no-repeat",R.style.webkitMaskRepeat="no-repeat",R.style.maskSize="100% 100%",R.style.webkitMaskSize="100% 100%"}function hi(){requestAnimationFrame(hi);const e=performance.now()/1e3,t=Math.max(0,Math.min(.1,e-eo));eo=e;const a=l.selectedId,o=(a?W(a):[]).filter(M=>M.enabled),i=a?Ne(a):!1,r=!!a&&i&&o.length>0,s=a?G.get(a):null;a&&s?.extras?.paused||(na+=t);let u=null;try{u=D.contentDocument||null}catch{u=null}let d=null;try{d=u?.querySelector("canvas")||null}catch{d=null}if(!r||!d){R.style.display!=="none"&&(R.style.display="none"),(R.style.clipPath!=="none"||R.style.maskImage)&&(R.style.clipPath="none",R.style.maskImage="none",R.style.webkitMaskImage="none",en=null);for(const M of ot.values())M.style.display="none";return}R.style.display!=="block"&&(R.style.display="block");const f=_l();d!==Qa&&(Qa=d,f.setSource(at));const p=d.width||1,h=d.height||1;(at.width!==p||at.height!==h)&&(at.width=p,at.height=h);try{to.clearRect(0,0,p,h),to.drawImage(d,0,0,p,h)}catch{}const v=dn.getBoundingClientRect(),m=Math.max(1,Math.round(v.width)),g=Math.max(1,Math.round(v.height));(f.width!==m||f.height!==g)&&f.resize(m,g),Hl(u,m,g);const S=[...o].reverse();f.renderFrame(S,na),zl(R,f.renderer.getPixelRatio())}requestAnimationFrame(hi);nl();vl();ca.addEventListener("click",e=>{e.stopPropagation(),Z.hidden?yl():ha()});Z.addEventListener("click",e=>{const t=e.target.closest(".filter-add-option");!t||!l.selectedId||(ul(l.selectedId,t.dataset.filterId),ha())});document.addEventListener("click",e=>{const t=e.target.closest(".filter-add-wrap"),a=e.target.closest("#filterAddMenu");!Z.hidden&&!t&&!a&&ha()});document.addEventListener("keydown",e=>{e.key==="Escape"&&ha()});_.addEventListener("click",e=>{const t=e.target.closest("[data-action]");if(!t||!l.selectedId)return;const{action:a,instanceId:n,paramKey:o}=t.dataset;if(a==="collapse")pl(l.selectedId,n);else if(a==="remove")dl(l.selectedId,n);else if(a==="upload-image"){const i=t.closest(".filter-param-row").querySelector(".filter-image-input");i&&i.click()}else a==="clear-image"?(B(l.selectedId),Ee(l.selectedId,n,o,null),F()):a==="open-curve"?Xl(n,o,t.dataset.label,t):a==="open-gradient"&&jl(n,o,t.dataset.label,t)});let At=null;_.addEventListener("pointerdown",e=>{const t=e.target.closest(".filter-levels-handle[data-instance-id][data-handle]");if(!t||!l.selectedId)return;const a=t.closest(".filter-levels-graph");if(!a)return;e.preventDefault(),e.stopPropagation();const n=t.dataset.instanceId,o=`levelsgraph:${n}`;ye!==o&&(B(l.selectedId),ye=o),At={instanceId:n,handle:t.dataset.handle,graph:a}});_.addEventListener("change",e=>{const t=e.target;if(l.selectedId)if(t.matches('[data-action="toggle"]'))fl(l.selectedId,t.dataset.instanceId);else if(t.classList.contains("filter-param-toggle"))B(l.selectedId),Ee(l.selectedId,t.dataset.instanceId,t.dataset.paramKey,t.checked),F();else if(t.classList.contains("filter-param-select"))B(l.selectedId),Ee(l.selectedId,t.dataset.instanceId,t.dataset.paramKey,t.value),hl(l.selectedId,t.dataset.instanceId,t.dataset.paramKey,t.value),F();else if(t.classList.contains("filter-param-color")){const a=`color:${t.dataset.instanceId}:${t.dataset.paramKey}`;ye!==a&&B(l.selectedId),Ee(l.selectedId,t.dataset.instanceId,t.dataset.paramKey,t.value),ye=null}else if(t.classList.contains("filter-image-input")){const a=t.files&&t.files[0];if(!a)return;const{instanceId:n,paramKey:o}=t.dataset,i=new FileReader;i.onload=()=>{B(l.selectedId),Ee(l.selectedId,n,o,i.result),F()},i.readAsDataURL(a)}else(t.classList.contains("filter-param-range")||t.classList.contains("filter-param-number")||t.classList.contains("filter-param-color"))&&(ye=null)});let ye=null;_.addEventListener("input",e=>{const t=e.target;if(l.selectedId){if(t.classList.contains("filter-param-range")||t.classList.contains("filter-param-number")){const{instanceId:a,paramKey:n}=t.dataset,o=Number(t.value),i=`${a}:${n}`;ye!==i&&(B(l.selectedId),ye=i),Ee(l.selectedId,a,n,o,!0),_.querySelectorAll(`[data-instance-id="${a}"][data-param-key="${n}"]`).forEach(r=>{r!==t&&(r.value=o)}),wn(a)}else if(t.classList.contains("filter-param-color")){const{instanceId:a,paramKey:n}=t.dataset,o=`color:${a}:${n}`;ye!==o&&(B(l.selectedId),ye=o),Ee(l.selectedId,a,n,t.value,!0)}}});window.addEventListener("pointermove",e=>{if(!At||!l.selectedId)return;const{instanceId:t,handle:a,graph:n}=At,o=n.getBoundingClientRect();if(o.width<=1)return;const i=Math.max(0,Math.min(1,(e.clientX-o.left)/o.width)),r=(d,f)=>{const p=_.querySelector(`input[data-instance-id="${t}"][data-param-key="${d}"]`),h=p?Number(p.value):f;return Number.isFinite(h)?h:f};let s=r("blackPoint",0),c=r("gamma",1),u=r("whitePoint",1);if(a==="black")s=Math.max(0,Math.min(u-.001,i)),Ta(t,"blackPoint",s);else if(a==="white")u=Math.max(s+.001,Math.min(1,i)),Ta(t,"whitePoint",u);else{const d=Math.max(0,Math.min(.99,s)),f=Math.max(d+.001,Math.min(1,u)),p=Math.max(.001,Math.min(.999,(i-d)/(f-d)));c=Math.max(.2,Math.min(3,Math.log(p)/Math.log(.5))),Ta(t,"gamma",c)}wn(t)});window.addEventListener("pointerup",()=>{At&&(At=null,ye=null)});let oa=null,qe=null;_.addEventListener("dragstart",e=>{const t=e.target.closest(".filter-item");if(!t||!e.target.closest(".filter-name")){e.preventDefault();return}oa=t.dataset.instanceId,qe=t,t.classList.add("dragging"),e.dataTransfer.effectAllowed="move",e.dataTransfer.setData("text/plain",oa);const a=t.getBoundingClientRect();e.dataTransfer.setDragImage(t,e.clientX-a.left,e.clientY-a.top)});_.addEventListener("dragover",e=>{if(!qe)return;e.preventDefault(),e.dataTransfer.dropEffect="move";const t=e.target.closest(".filter-item");if(!t||t===qe){t||_.appendChild(qe);return}const a=t.getBoundingClientRect();e.clientY<a.top+a.height/2?t.parentNode.insertBefore(qe,t):t.parentNode.insertBefore(qe,t.nextSibling)});_.addEventListener("drop",e=>{if(e.preventDefault(),!l.selectedId||!oa)return;const t=W(l.selectedId),a=[..._.querySelectorAll(".filter-item")].map(i=>i.dataset.instanceId),n=new Map(t.map(i=>[i.instanceId,i])),o=a.map(i=>n.get(i)).filter(Boolean);o.length===t.length&&(o.some((r,s)=>r!==t[s])&&B(l.selectedId),t.length=0,t.push(...o),me()),F()});_.addEventListener("dragend",()=>{oa=null,qe=null,_.querySelectorAll(".filter-item.dragging, .filter-item.drag-over").forEach(e=>{e.classList.remove("dragging","drag-over")})});const ql={w:320,h:240};function Gl(){try{const e=E.getItem(So),t=e?JSON.parse(e):null;if(t&&typeof t.w=="number"&&typeof t.h=="number")return t}catch{}return{...ql}}let Le=Gl();function Wl(){E.setItem(So,JSON.stringify(Le))}const C=document.createElement("div");C.id="curvePopup";C.className="curve-popup";C.hidden=!0;C.innerHTML=`
...
`;document.body.appendChild(A);let O=null,ct=null,$=null;function An(e){return Math.min(1,Math.max(0,e))}function Bn(){return!O||!l.selectedId?null:W(l.selectedId).find(t=>t.instanceId===O.instanceId)||null}function kt(){const e=Bn();return!e||!O?null:gt(e.params[O.paramKey])}function St(e){const t=Bn();!t||!O||(t.params[O.paramKey]={stops:e},me())}function vi(){if(A.hidden||!O||!O.trigger)return;const e=O.trigger.getBoundingClientRect(),t=A.offsetWidth||300,a=A.offsetHeight||140;let n=e.right-t,o=e.top-a-10;o<8&&(o=e.bottom+10),n=Math.min(Math.max(8,n),window.innerWidth-t-8),o=Math.min(Math.max(8,o),window.innerHeight-a-8),A.style.left=`${n}px`,A.style.top=`${o}px`}function Me(){const e=kt(),t=A.querySelector(".gradient-popup-track"),a=A.querySelector(".gradient-popup-editor");if(!(!e||!t))if($!==null&&($<0||$>=e.stops.length)&&($=null),t.style.background=ro(e),t.innerHTML=e.stops.map((n,o)=>`<button type="button" class="gradient-popup-handle${o===$?" active":""}"
      style="left:${(n.t*100).toFixed(2)}%; background:${n.color}" data-stop-index="${o}" title="${n.color}"></button>`).join(""),$!==null){const n=e.stops[$];a.hidden=!1,a.querySelector(".gradient-popup-color").value=n.color,a.querySelector(".gradient-popup-pos").value=Math.round(n.t*100),a.querySelector(".gradient-popup-delete").disabled=e.stops.length<=2}else a.hidden=!0}function jl(e,t,a,n){O={instanceId:e,paramKey:t,trigger:n},$=null,A.querySelector(".curve-popup-title").textContent=a||"Gradient",A.hidden=!1,vi(),Me()}function xa(){A.hidden=!0,O=null,ct=null,$=null}function Vl(){const e=Bn();if(!e||!O)return;const t=Re(e.defId),a=t&&t.params.find(o=>o.key===O.paramKey);if(!a)return;const n=gt(a.default);B(l.selectedId),$=null,St(n.stops.map(o=>({...o}))),Me(),F()}A.querySelector(".gradient-popup-close").addEventListener("click",xa);A.querySelector(".gradient-popup-reset").addEventListener("click",Vl);A.querySelector(".gradient-popup-bar").addEventListener("pointerdown",e=>{const t=e.target.closest(".gradient-popup-handle");t&&($=Number(t.dataset.stopIndex),ct=$,B(l.selectedId),Me(),e.preventDefault())});A.querySelector(".gradient-popup-bar").addEventListener("dblclick",e=>{if(e.target.closest(".gradient-popup-handle"))return;const t=kt(),a=e.currentTarget;if(!t)return;const n=a.getBoundingClientRect(),o=An((e.clientX-n.left)/n.width),i=Ei(t,o),r=t.stops.map(c=>({...c})),s={t:o,color:i};r.push(s),r.sort((c,u)=>c.t-u.t),$=r.indexOf(s),B(l.selectedId),St(r),Me(),F()});document.addEventListener("pointermove",e=>{if(ct===null)return;const t=A.querySelector(".gradient-popup-bar"),a=kt();if(!t||!a)return;const n=t.getBoundingClientRect(),o=An((e.clientX-n.left)/n.width),i=a.stops.map(s=>({...s})),r=i[ct];r&&(r.t=o,i.sort((s,c)=>s.t-c.t),$=i.indexOf(r),ct=$,St(i),Me(),F())});document.addEventListener("pointerup",()=>{ct=null});A.querySelector(".gradient-popup-color").addEventListener("input",e=>{const t=kt();if(!t||$===null)return;const a=t.stops.map(n=>({...n}));a[$].color=e.target.value,St(a),Me(),F()});A.querySelector(".gradient-popup-color").addEventListener("change",()=>{B(l.selectedId)});A.querySelector(".gradient-popup-pos").addEventListener("change",e=>{const t=kt();if(!t||$===null)return;const a=t.stops.map(o=>({...o})),n=a[$];B(l.selectedId),n.t=An(Number(e.target.value)/100),a.sort((o,i)=>o.t-i.t),$=a.indexOf(n),St(a),Me(),F()});A.querySelector(".gradient-popup-delete").addEventListener("click",()=>{const e=kt();if(!e||$===null||e.stops.length<=2)return;const t=e.stops.filter((a,n)=>n!==$);B(l.selectedId),$=null,St(t),Me(),F()});let z=null,mt=null,ht=null,Ae=null,ut=null,j=null;function Tn(){return!z||!l.selectedId?null:W(l.selectedId).find(t=>t.instanceId===z.instanceId)||null}function Ye(){const e=Tn();return e?Tt(e.params[z.paramKey]):null}function vt(e){const t=Tn();!t||!z||(t.params[z.paramKey]={points:e},me())}function Xl(e,t,a,n){z={instanceId:e,paramKey:t,trigger:n},j=null,C.querySelector(".curve-popup-title").textContent=a||"Curve",C.style.width=`${Le.w}px`,C.style.height=`${Le.h}px`,C.hidden=!1,Ln(),pe()}function wa(){C.hidden=!0,z=null,mt=null,ht=null,j=null,Bt()}function Kl(){const e=Tn();if(!e||!z)return;const t=Re(e.defId),a=t&&t.params.find(i=>i.key===z.paramKey);if(!a)return;const o=Tt(a.default).points.map(i=>({...i}));j=null,B(l.selectedId),vt(o),pe(),F()}function Ln(){if(C.hidden||!z||!z.trigger)return;const e=z.trigger.getBoundingClientRect(),t=C.offsetWidth||Le.w,a=C.offsetHeight||Le.h;let n=e.right-t,o=e.top-a-10;o<8&&(o=e.bottom+10),n=Math.min(Math.max(8,n),window.innerWidth-t-8),o=Math.min(Math.max(8,o),window.innerHeight-a-8),C.style.left=`${n}px`,C.style.top=`${o}px`}function pe(){const e=Ye(),t=C.querySelector(".curve-popup-svg");if(!e||!t)return;const a=t.getBoundingClientRect(),n=Math.max(1,Math.round(a.width)||100),o=Math.max(1,Math.round(a.height)||100);t.setAttribute("viewBox",`0 0 ${n} ${o}`),j!==null&&(j<0||j>=e.points.length-1)&&(j=null);const i=ti(e,n,o,48);let r="";for(let p=1;p<4;p++){const h=p/4*n,v=p/4*o;r+=`<line class="curve-grid-line" x1="0" y1="${v.toFixed(2)}" x2="${n}" y2="${v.toFixed(2)}" />`,r+=`<line class="curve-grid-line" x1="${h.toFixed(2)}" y1="0" x2="${h.toFixed(2)}" y2="${o}" />`}const s=Math.min(n,o),c=Math.max(4.5,s*.03),u=Math.max(9,s*.05);let d="";for(let p=0;p<e.points.length-1;p++){const h=e.points[p];if(h.type==="constant"||p!==j)continue;const v=e.points[p+1],m=h.x*n,g=(1-h.y)*o,S=Math.max(v.x-h.x,1e-5),M=h.tanSet?h.tanDx:S/3,x=h.tanSet?h.tanDy:0;let X=M*n,ce=-x*o;const ee=Math.hypot(X,ce),ue=Math.max(u*1.1,12);if(!h.tanSet)if(ee>1e-4&&ee<ue){const P=ue/ee;X*=P,ce*=P}else ee<=1e-4&&(X=ue,ce=0);const Ct=m+X,et=g+ce,b=m-X,T=g-ce;d+=`<line class="curve-handle-line" x1="${Ct.toFixed(2)}" y1="${et.toFixed(2)}" x2="${b.toFixed(2)}" y2="${T.toFixed(2)}" />`;const N=u/2,k=(P,J)=>`M ${P.toFixed(2)} ${(J-N).toFixed(2)} L ${(P+N).toFixed(2)} ${J.toFixed(2)} L ${P.toFixed(2)} ${(J+N).toFixed(2)} L ${(P-N).toFixed(2)} ${J.toFixed(2)} Z`;d+=`<path class="curve-handle-point" data-handle-index="${p}" d="${k(Ct,et)}" />`,d+=`<path class="curve-handle-point" data-handle-index="${p}" d="${k(b,T)}" />`}const f=e.points.map((p,h)=>{const v=h===0||h===e.points.length-1,m=h===j,g=["curve-point",v?"curve-point--endpoint":"",p.type==="constant"?"curve-point--constant":"",m?"curve-point--active":""].filter(Boolean).join(" "),S=p.x*n,M=(1-p.y)*o;if(m){const x=c*1.7;return`<rect class="${g}" data-point-index="${h}" x="${(S-x/2).toFixed(2)}" y="${(M-x/2).toFixed(2)}" width="${x.toFixed(2)}" height="${x.toFixed(2)}" />`}return`<circle class="${g}" data-point-index="${h}" cx="${S.toFixed(2)}" cy="${M.toFixed(2)}" r="${c.toFixed(2)}" />`}).join("");t.innerHTML=`${r}${d}<path class="curve-popup-path" d="${i}" />${f}`}C.querySelector(".curve-popup-close").addEventListener("click",wa);C.querySelector(".curve-popup-reset").addEventListener("click",Kl);C.querySelector(".curve-popup-svg").addEventListener("pointerdown",e=>{const t=e.currentTarget,a=e.target.closest(".curve-handle-point"),n=e.target.closest(".curve-point"),o=Ye();if(o){if(a){const i=Number(a.dataset.handleIndex);B(l.selectedId),ht={index:i},j=i,t.setPointerCapture(e.pointerId),e.stopPropagation();return}if(n){const i=Number(n.dataset.pointIndex);B(l.selectedId),mt={index:i},i<o.points.length-1&&o.points[i].type!=="constant"?j=i:j=null,t.setPointerCapture(e.pointerId),e.stopPropagation(),pe();return}j!==null&&(j=null,pe())}});C.querySelector(".curve-popup-svg").addEventListener("dblclick",e=>{const t=e.target.closest(".curve-point"),a=Ye();if(!a)return;if(e.preventDefault(),t){const u=Number(t.dataset.pointIndex),d=a.points.map(f=>({...f}));d[u].type=d[u].type==="constant"?"bezier":"constant",B(l.selectedId),vt(d),pe(),F();return}if(e.target.closest(".curve-handle-point"))return;const o=C.querySelector(".curve-popup-svg").getBoundingClientRect(),i=Math.min(1,Math.max(0,(e.clientX-o.left)/o.width)),r=Math.min(1,Math.max(0,1-(e.clientY-o.top)/o.height)),s=a.points.map(u=>({...u}));s.push({x:i,y:r,type:"bezier",tanDx:0,tanDy:0,tanSet:!1}),s.sort((u,d)=>u.x-d.x),B(l.selectedId),vt(s);const c=s.findIndex(u=>u.x===i&&u.y===r);j=c<s.length-1?c:null,pe(),F()});C.querySelector(".curve-popup-svg").addEventListener("contextmenu",e=>{const t=e.target.closest(".curve-point");if(!t)return;e.preventDefault(),ut=Number(t.dataset.pointIndex);const a=Ye(),n=a&&(ut===0||ut===a.points.length-1);le.querySelector('[data-action="delete"]').disabled=!!n,le.style.left=`${e.clientX}px`,le.style.top=`${e.clientY}px`,le.hidden=!1});function Bt(){le.hidden=!0,ut=null}le.addEventListener("click",e=>{const t=e.target.closest("button[data-action]");if(!t||t.disabled||ut===null){Bt();return}const a=Ye();if(a){const n=a.points.map(r=>({...r})),o=ut,i=o===0||o===n.length-1;t.dataset.action==="type-bezier"?n[o].type="bezier":t.dataset.action==="type-constant"?n[o].type="constant":t.dataset.action==="delete"&&!i&&n.length>2&&n.splice(o,1),B(l.selectedId),vt(n),pe(),F()}Bt()});C.querySelector(".curve-popup-resize").addEventListener("pointerdown",e=>{Ae={startX:e.clientX,startY:e.clientY,startW:Le.w,startH:Le.h},e.target.setPointerCapture(e.pointerId),e.preventDefault()});document.addEventListener("pointermove",e=>{if(mt){const t=Ye(),a=C.querySelector(".curve-popup-svg");if(!t||!a)return;const n=a.getBoundingClientRect(),o=t.points.map(u=>({...u})),i=mt.index,r=i===0||i===o.length-1,s=Math.min(1,Math.max(0,1-(e.clientY-n.top)/n.height));let c=o[i].x;if(!r){const u=o[i-1].x+.001,d=o[i+1].x-.001;c=Math.min(d,Math.max(u,(e.clientX-n.left)/n.width))}o[i]={...o[i],x:c,y:s},vt(o),pe(),F()}else if(ht){const t=Ye(),a=C.querySelector(".curve-popup-svg");if(!t||!a)return;const n=a.getBoundingClientRect(),o=t.points.map(x=>({...x})),i=ht.index,r=o[i],s=o[i+1];if(!r||!s)return;const c=n.width,u=n.height,d=r.x*c,f=(1-r.y)*u,p=e.clientX-n.left,h=e.clientY-n.top;let v=(p-d)/c,m=-(h-f)/u;const S=Math.max(.001,Math.hypot(s.x-r.x,s.y-r.y))*1.5,M=Math.hypot(v,m);if(M>S){const x=S/M;v*=x,m*=x}o[i]={...r,tanDx:v,tanDy:m,tanSet:!0},vt(o),pe(),F()}else if(Ae){const t=Math.max(240,Math.min(720,Ae.startW+(e.clientX-Ae.startX))),a=Math.max(160,Math.min(560,Ae.startH+(e.clientY-Ae.startY)));Le={w:t,h:a},C.style.width=`${t}px`,C.style.height=`${a}px`,pe()}});document.addEventListener("pointerup",()=>{mt&&(mt=null),ht&&(ht=null),Ae&&(Ae=null,Wl())});document.addEventListener("click",e=>{!C.hidden&&!e.target.closest("#curvePopup")&&!e.target.closest('[data-action="open-curve"]')&&!e.target.closest("#curvePointMenu")&&wa(),!le.hidden&&!e.target.closest("#curvePointMenu")&&Bt(),!A.hidden&&!e.target.closest("#gradientPopup")&&!e.target.closest('[data-action="open-gradient"]')&&xa()});document.addEventListener("keydown",e=>{e.key==="Escape"&&(wa(),Bt(),xa())});document.addEventListener("keydown",e=>{const t=e.key.toLowerCase();if(!(e.ctrlKey||e.metaKey)||t!=="z"||!l.selectedId)return;const a=e.target;a&&(a.tagName==="INPUT"||a.tagName==="TEXTAREA")&&a.type==="text"||(e.preventDefault(),e.shiftKey?cl(l.selectedId):sl(l.selectedId))});window.addEventListener("resize",Ln);Xt.addEventListener("change",()=>{l.selectedId&&(l.filterPreviewOn[l.selectedId]=Xt.checked,me(),F())});function gi(){return!ge||!l.selectedId||R.style.display==="none"||!Ne(l.selectedId)?!1:W(l.selectedId).some(e=>e.enabled)}function Rn(e){if(gi()){const c=[...W(l.selectedId).filter(u=>u.enabled)].reverse();return ge.captureAtScale(c,na,e)}const t=D.contentDocument,a=t?t.querySelector("canvas"):null;if(!a||!a.width||!a.height)return null;const n=Math.round(a.width*e),o=Math.round(a.height*e),i=document.createElement("canvas");return i.width=n,i.height=o,i.getContext("2d").drawImage(a,0,0,n,o),{dataUrl:i.toDataURL("image/png"),width:n,height:o}}function Yl(e,t){const a=URL.createObjectURL(e),n=document.createElement("a");n.href=a,n.download=t,n.click(),setTimeout(()=>URL.revokeObjectURL(a),1e3)}function yi(e,t){const a=zr(e),n=Vr(t.dataUrl);if(!n)return console.warn("Failed to read exported PNG data for metadata embedding."),null;let o;try{const i=Kr(n,Co,JSON.stringify(a));o=new Blob([i],{type:"image/png"})}catch(i){console.warn("Failed to embed state metadata into PNG export; downloading raw image.",i),o=new Blob([n],{type:"image/png"})}return o}function Jl(e,t,a){const n=yi(e,t);return n?(Yl(n,a),!0):!1}function ia(e,t,a,n){const o=Number(e);return Number.isFinite(o)?Math.min(a,Math.max(t,o)):n}function bi(e){return new Promise(t=>setTimeout(t,e))}async function Zl(){if(typeof window.showDirectoryPicker!="function")throw new Error("Directory picker is not supported in this runtime.");return window.showDirectoryPicker({mode:"readwrite"})}async function Ql(e,t,a){const o=await(await e.getFileHandle(t,{create:!0})).createWritable();await o.write(a),await o.close()}async function es(e=140){await new Promise(t=>requestAnimationFrame(()=>requestAnimationFrame(t))),e>0&&await bi(e)}let dt=!1;const ka=new Map;function oe(e,t="neutral"){Gn.textContent=e||"",Gn.dataset.tone=t}function no(e){const t=Math.floor(ia(e,1,200,1));Y.value=String(t)}function oo(e){const t=ia(e,.1,4,1);Ge.value=String(Number(t.toFixed(1)))}function Sa(e){const t=e?ka.get(e):null;Wn.textContent=t?t.name:"Not set",Wn.title=t?t.name:"No folder selected",pn.disabled=!t}function ts(){const e=Number(Y.value);Y.dataset.prevValue=String(Number.isFinite(e)?e:8),Y.addEventListener("input",()=>{const t=Number(Y.dataset.prevValue||"8"),a=Number(Y.value);if(!Number.isFinite(a))return;let n=a;Number.isFinite(t)&&Math.abs(a-(t+1))<1e-9?n=t*2:Number.isFinite(t)&&Math.abs(a-(t-1))<1e-9&&(n=t/2),no(n),Y.dataset.prevValue=Y.value}),Y.addEventListener("change",()=>{no(Number(Y.value||8)),Y.dataset.prevValue=Y.value}),Ge.addEventListener("input",()=>{oo(Number(Ge.value||1))}),Ge.addEventListener("change",()=>{oo(Number(Ge.value||1))})}Eo.addEventListener("click",()=>{l.selectedId&&ns()});he.addEventListener("click",async()=>{if(!l.selectedId||he.disabled)return;const e=Rn(1);if(e)try{const t=await(await fetch(e.dataUrl)).blob();await navigator.clipboard.write([new ClipboardItem({"image/png":t})]),as()}catch(t){console.warn("[filter] copy to clipboard failed:",t)}});function as(){const e=he.dataset.baseTitle||he.title;he.dataset.baseTitle=e,he.classList.add("copied"),he.title="Copied!",setTimeout(()=>{he.classList.remove("copied"),he.title=e},1200)}function ns(){let e,t;if(gi())e=ge.width,t=ge.height;else{const a=D.contentDocument,n=a?a.querySelector("canvas"):null;e=n?n.width:R.width,t=n?n.height:R.height}sr.textContent=`${Math.round(e)} × ${Math.round(t)} px`,oe(""),Ot.disabled=!1,Sa(l.selectedId),Kt.hidden=!1}function ra(){dt||(Kt.hidden=!0)}Va.addEventListener("click",e=>{const t=e.target.closest(".export-size-option");if(!t||!l.selectedId)return;const a=Number(t.dataset.scale)||1,n=l.selectedId,o=Rn(a);if(!o){ra();return}const i=Ko(n);Jl(n,o,i),ra()});Ot.addEventListener("click",async()=>{if(!l.selectedId||dt)return;const e=l.selectedId,t=Math.floor(ia(Y.value,1,200,1)),a=ia(Ge.value,.1,4,1),n=ur.value==="seed"?"seed":"all-random";Y.value=String(t),Ge.value=String(Number(a.toFixed(2)));const o=ka.get(e);if(!o){oe("Please set Batch Path first.","warn");return}dt=!0,Ot.disabled=!0,Xa.disabled=!0,pn.disabled=!0,Va.querySelectorAll(".export-size-option").forEach(s=>{s.disabled=!0}),oe(`Preparing batch export (${t})â€¦`);let i=0,r=0;try{for(let s=0;s<t;s++){if(l.selectedId!==e){oe("Batch stopped: selected effect changed.","warn");break}oe(`Rendering ${s+1}/${t}â€¦`),n==="all-random"?fa(e):$e(e,"rerollSeed"),await Er(e,1800),await es();const c=Rn(a);if(!c){r+=1;continue}const u=`batch-${String(s+1).padStart(3,"0")}`,d=Ko(e,u),f=yi(e,c);if(!f)r+=1;else try{await Ql(o,d,f),i+=1}catch(p){console.warn("Batch export write failed:",p),r+=1}await bi(60)}}finally{dt=!1,Ot.disabled=!1,Xa.disabled=!1,Sa(l.selectedId),Va.querySelectorAll(".export-size-option").forEach(c=>{c.disabled=!1});const s=r>0?"warn":"ok";oe(`Done: ${i} exported${r?`, ${r} failed`:""}.`,s)}});Xa.addEventListener("click",async()=>{const e=l.selectedId;if(!(!e||dt)){oe("Selecting folderâ€¦");try{const t=await Zl();ka.set(e,t),Sa(e),oe("Batch path set.","ok")}catch(t){console.warn("Batch export folder selection failed:",t);const a=t&&typeof t=="object"&&"name"in t?String(t.name):"";if(a==="AbortError"){oe("Folder selection cancelled.","warn");return}if(a==="NotAllowedError"||a==="SecurityError"){oe("Folder access was blocked. Please allow folder access and try again.","warn");return}const n=t instanceof Error&&t.message?t.message:"Unknown error";oe(`Failed to set folder: ${n}`,"warn")}}});pn.addEventListener("click",()=>{const e=l.selectedId;!e||dt||(ka.delete(e),Sa(e),oe("Batch path cleared."))});ts();cr.addEventListener("click",ra);Kt.addEventListener("click",e=>{e.target===Kt&&ra()});dn.addEventListener("dragover",e=>{(e.dataTransfer?.types?Array.from(e.dataTransfer.types):[]).includes("Files")&&(e.preventDefault(),e.dataTransfer.dropEffect="copy")});dn.addEventListener("drop",async e=>{if(!e.dataTransfer?.files?.length)return;e.preventDefault();const t=[...e.dataTransfer.files].find(a=>/image\/png/i.test(a.type)||/\.png$/i.test(a.name));if(t)try{await Qr(t)||console.info("Dropped PNG has no embedded ShaderOps state metadata.")}catch(a){console.warn("Failed to restore ShaderOps state from dropped PNG:",a)}});er.addEventListener("click",()=>{fn.classList.add("collapsed"),E.setItem(sn,"1")});Qi.addEventListener("click",()=>{fn.classList.remove("collapsed"),E.setItem(sn,"0")});async function os(){await el(),tl(),ll(),await Nl()}os().catch(e=>{Oa.textContent="Registry load failed",za.textContent=e.message});
