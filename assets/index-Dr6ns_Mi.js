...
`,Sn=[{value:"value",label:"Value"},{value:"cloud",label:"Cloud"},{value:"voronoi",label:"Voronoi"},{value:"ridged",label:"Ridged"},{value:"warp",label:"Domain Warp"},{value:"turbulence",label:"Turbulence"}],ti={value:0,cloud:1,voronoi:2,ridged:3,warp:4,turbulence:5};function ga(e){return ti[e]??1}const we=8,Cn=`
...
`;function he(e){return Math.min(1,Math.max(0,Number.isFinite(e)?e:0))}function Ea(e,t,a){return Math.min(a,Math.max(t,Number.isFinite(e)?e:0))}function ai(e,t,a,n,o){let i=he(o);for(let r=0;r<8;r++){const s=1-i,c=s*s*s*e+3*s*s*i*t+3*s*i*i*a+i*i*i*n-o,u=3*s*s*(t-e)+6*s*i*(a-t)+3*i*i*(n-a);if(Math.abs(u)<1e-6)break;i=he(i-c/u)}return i}function ni(e){if(typeof e.handleAngle=="number"&&typeof e.handleDist=="number"&&e.handleDist>0){const t=e.handleDist*.33;return{tanDx:Math.cos(e.handleAngle)*t,tanDy:Math.sin(e.handleAngle)*t,tanSet:!0}}return typeof e.bend=="number"&&e.bend!==0?{tanDx:0,tanDy:Ea(e.bend,-1,1)*.25,tanSet:!0}:{tanDx:0,tanDy:0,tanSet:!1}}function It(e){if(e&&Array.isArray(e.points)&&e.points.length>=2){const t=e.points.map(a=>{const o=a.tanSet&&typeof a.tanDx=="number"&&typeof a.tanDy=="number"?{tanDx:a.tanDx,tanDy:a.tanDy,tanSet:!0}:ni(a);return{x:he(a.x),y:he(a.y),type:a.type==="constant"?"constant":"bezier",tanDx:o.tanSet?o.tanDx:0,tanDy:o.tanSet?o.tanDy:0,tanSet:!!o.tanSet}}).sort((a,n)=>a.x-n.x).slice(0,we);return t[0]={...t[0],x:0},t[t.length-1]={...t[t.length-1],x:1},{points:t}}return e&&typeof e.x=="number"&&typeof e.y=="number"?{points:[{x:0,y:0,type:"bezier",tanDx:0,tanDy:0,tanSet:!1},{x:he(e.x),y:he(e.y),type:"bezier",tanDx:0,tanDy:0,tanSet:!1},{x:1,y:1,type:"bezier",tanDx:0,tanDy:0,tanSet:!1}]}:{points:[{x:0,y:0,type:"bezier",tanDx:0,tanDy:0,tanSet:!1},{x:1,y:1,type:"bezier",tanDx:0,tanDy:0,tanSet:!1}]}}function oi(e,t){const{points:a}=It(e),n=he(t);for(let o=0;o<a.length-1;o++){const i=a[o],r=a[o+1];if(n>=i.x&&(n<r.x||o===a.length-2)){if(i.type==="constant")return i.y;const s=Math.max(r.x-i.x,1e-5),c=Ea(i.tanSet?i.x+i.tanDx:i.x+s/3,i.x,r.x),u=i.tanSet?i.y+i.tanDy:i.y,d=Ea(r.tanSet?r.x-r.tanDx:r.x-s/3,i.x,r.x),f=r.tanSet?r.y-r.tanDy:r.y,p=ai(i.x,c,d,r.x,n),h=1-p;return h*h*h*i.y+3*h*h*p*u+3*h*p*p*f+p*p*p*r.y}}return a[a.length-1].y}function En(e){const{points:t}=It(e),a=[],n=[];for(let o=0;o<we;o++){const i=t[Math.min(o,t.length-1)];a.push([i.x,i.y,i.type==="constant"?1:0,0]),n.push([i.tanDx||0,i.tanDy||0,i.tanSet?1:0,0])}return{points:a,tans:n,count:t.length}}const Bt=8;function qn(e){const t=typeof e=="string"?/^#?([0-9a-fA-F]{6})$/.exec(e.trim()):null;return t?`#${t[1].toLowerCase()}`:"#ffffff"}function Ct(e){const t=qn(e).slice(1);return[parseInt(t.slice(0,2),16)/255,parseInt(t.slice(2,4),16)/255,parseInt(t.slice(4,6),16)/255]}function ii([e,t,a]){const n=o=>Math.round(he(o)*255).toString(16).padStart(2,"0");return`#${n(e)}${n(t)}${n(a)}`}function pt(e){let t=e&&Array.isArray(e.stops)?e.stops:null;(!t||t.length<2)&&(t=[{t:0,color:"#1b2a6b"},{t:1,color:"#ffcf6b"}]);const a=t.map(n=>({t:he(Number(n&&n.t)),color:qn(n&&n.color)})).sort((n,o)=>n.t-o.t).slice(0,Bt);return a.length<2&&a.push({t:1,color:"#ffffff"}),{stops:a}}function ri(e,t){const{stops:a}=pt(e),n=he(t);if(n<=a[0].t)return a[0].color;if(n>=a[a.length-1].t)return a[a.length-1].color;for(let o=0;o<a.length-1;o++){const i=a[o],r=a[o+1];if(n>=i.t&&n<=r.t){const s=Math.max(r.t-i.t,1e-5),c=(n-i.t)/s,u=Ct(i.color),d=Ct(r.color);return ii([u[0]+(d[0]-u[0])*c,u[1]+(d[1]-u[1])*c,u[2]+(d[2]-u[2])*c])}}return a[a.length-1].color}function Gn(e){const{stops:t}=pt(e);return`linear-gradient(90deg, ${t.map(a=>`${a.color} ${(a.t*100).toFixed(1)}%`).join(", ")})`}function li(e){const{stops:t}=pt(e),a=[];for(let n=0;n<Bt;n++){const o=t[Math.min(n,t.length-1)],i=Ct(o.color);a.push([i[0],i[1],i[2],o.t])}return{stops:a,count:t.length}}const si=`
...
`;const l={projects:[],selectedId:null,previews:{},captureInFlight:!1,prewarmQueue:[],prewarmActive:!1,filterStacks:{},filterPreviewOn:{},filterHistory:{},navPage:0,favorites:[]},oe=document.getElementById("projectList"),La=document.getElementById("projectTitle"),Pa=document.getElementById("projectMethod"),D=document.getElementById("projectFrame"),Aa=document.getElementById("deleteProjectButton"),Zt=document.getElementById("navGridButton"),tt=document.getElementById("navPager"),Ci=document.getElementById("navPagerLabel"),j=document.getElementById("navGridPanel"),Za=document.getElementById("navGridBackdrop"),Ta=document.getElementById("navGridFav"),Ra=document.getElementById("navGrid2d"),Fa=document.getElementById("navGrid3d"),Qa=document.getElementById("shaderStage"),R=document.getElementById("fxCanvas"),en=document.getElementById("filterRail"),Ei=document.getElementById("filterRailGear"),Di=document.getElementById("filterRailCollapse"),Qt=document.getElementById("filterAddButton"),J=document.getElementById("filterAddMenu"),_=document.getElementById("filterStackList"),Mi=document.getElementById("filterStackEmpty"),Ht=document.getElementById("filterPreviewToggle"),oo=document.getElementById("filterExportButton"),me=document.getElementById("filterCopyButton"),Ii=document.getElementById("profileButton"),Bi=document.getElementById("profileButtonName"),Tt=document.getElementById("profileOverlay"),Li=document.getElementById("profileDialogTitle"),Pi=document.getElementById("profileDialogDescription"),kt=document.getElementById("profileNameInput"),ya=document.getElementById("profileExistingList"),Ai=document.getElementById("profileRestoreHint"),We=document.getElementById("profileDialogStatus"),io=document.getElementById("profileContinueButton"),ro=document.getElementById("profileDownloadButton"),Ti=document.getElementById("profileRestoreLabel"),$a=document.getElementById("profileRestoreInput"),qt=document.getElementById("exportSizeOverlay"),Ri=document.getElementById("exportSizeCurrent"),Na=document.getElementById("exportSizeOptions"),Fi=document.getElementById("exportSizeCancel"),K=document.getElementById("exportBatchCount"),He=document.getElementById("exportBatchScale"),$i=document.getElementById("exportBatchMode"),Rt=document.getElementById("exportBatchRun"),In=document.getElementById("exportBatchStatus"),Bn=document.getElementById("exportBatchPathName"),Ua=document.getElementById("exportBatchChoosePath"),tn=document.getElementById("exportBatchClearPath"),at=document.getElementById("deleteConfirmOverlay"),Ni=document.getElementById("deleteConfirmMessage"),Ui=document.getElementById("deleteConfirmButton"),_i=document.getElementById("deleteConfirmCancel"),ht=document.getElementById("unifiedPanel"),Oi=document.getElementById("unifiedPanelGear"),Ft=document.getElementById("unifiedPanelGroups"),lo=document.getElementById("globalCheckpoints"),zi=document.getElementById("unifiedDiceButton"),Hi=document.getElementById("unifiedWaveButton"),$t=document.getElementById("unifiedPauseButton"),qi=document.getElementById("unifiedUndoButton"),Gi=document.getElementById("unifiedRedoButton"),ba=document.getElementById("unifiedCheckpointSlots"),so=document.getElementById("unifiedCheckpointAddShell");document.getElementById("unifiedCheckpointAdd");const Wi=document.getElementById("topbarReloadButton"),re=document.getElementById("thumbHoverPreview"),ji=document.getElementById("thumbHoverPreviewTitle"),co=document.getElementById("thumbHoverPreviewHero"),Ln=document.getElementById("thumbHoverPreviewCheckpoints");let uo="settings";function Vi(){Bi.textContent=Le()?.name||"Profile"}function Xi(e){try{Va(e),window.location.reload()}catch(t){We.textContent=t instanceof Error?t.message:"Unable to switch profile."}}function fo(){const e=D.contentWindow;try{e?.ShaderOpsControls?.suspendPersistence?.()}catch{}l.selectedId&&e?.postMessage({type:"shaderops/suspend-persistence",projectId:l.selectedId},"*")}function po(){const e=D.contentWindow;try{e?.ShaderOpsControls?.resumePersistence?.()}catch{}l.selectedId&&e?.postMessage({type:"shaderops/resume-persistence",projectId:l.selectedId},"*")}function mo(){const e=Te(),t=Le()?.id;ya.replaceChildren(),ya.hidden=e.length===0,e.forEach(a=>{const n=document.createElement("span");n.className=`profile-existing-list__item${a.id===t?" is-active":""}`;const o=document.createElement("button");o.type="button",o.className="profile-existing-list__switch",o.textContent=a.name,o.title=`Switch to ${a.name}`,o.addEventListener("click",()=>Xi(a.id));const i=document.createElement("button");i.type="button",i.className="profile-existing-list__delete",i.textContent="×",i.title=`Delete ${a.name}`,i.setAttribute("aria-label",`Delete ${a.name}`),i.addEventListener("click",()=>{if(!window.confirm(`Delete profile "${a.name}" from this browser?`))return;const r=a.id===Le()?.id;try{r&&fo(),hi(a.id),r?window.location.reload():mo()}catch(s){r&&po(),We.textContent=s instanceof Error?s.message:"Unable to delete this profile."}}),n.append(o,i),ya.appendChild(n)})}function ho(e){uo=e;const t=Le(),a=e==="onboarding";Li.textContent=a?"Welcome to ShaderOps":"Profile & recovery",Pi.textContent=a?"Choose a name once. Your favorites, checkpoints, looks, and filters save automatically on this computer.":`Working as ${t?.name||"this profile"}. Your work is stored locally and saves automatically.`,kt.value="",kt.placeholder=a?"Your name":"Create another profile",io.textContent=a?"Continue":"Create profile",ro.hidden=a||!t,Ti.hidden=a,Ai.hidden=a,We.textContent="",mo(),Tt.hidden=!1,window.setTimeout(()=>kt.focus(),0)}function vo(){try{const e=mi(kt.value);Va(e.id),window.location.reload()}catch(e){We.textContent=e instanceof Error?e.message:"Enter a profile name to continue."}}function Ki(){try{const e=Le(),t=new Blob([JSON.stringify(yi(),null,2)],{type:"application/json"}),a=URL.createObjectURL(t),n=document.createElement("a");n.href=a,n.download=`${(e?.name||"shaderops-profile").replace(/[^\w.-]+/g,"-")}.shaderops-profile.json`,n.click(),URL.revokeObjectURL(a),We.textContent="Recovery file downloaded."}catch(e){We.textContent=e instanceof Error?e.message:"Unable to create recovery file."}}async function Yi(e){if(e)try{const t=JSON.parse(await e.text()),a=Te().map(i=>i.name),n=a.length?` Existing profiles that will be replaced: ${a.join(", ")}.`:"";if(!window.confirm(`Restore this recovery file and replace all ShaderOps data in this browser?${n}`))return;fo();const o=bi(t);Va(o.id),window.location.reload()}catch(t){po(),We.textContent=t instanceof Error?t.message:"Unable to restore this recovery file."}finally{$a.value=""}}Ii.addEventListener("click",()=>ho("settings"));io.addEventListener("click",vo);kt.addEventListener("keydown",e=>{e.key==="Enter"&&vo()});ro.addEventListener("click",Ki);$a.addEventListener("change",()=>Yi($a.files?.[0]));Tt.addEventListener("click",e=>{e.target===Tt&&uo!=="onboarding"&&(Tt.hidden=!0)});Vi();Le()||ho("onboarding");const an="shaderops:unified-panel-collapsed:v1",Pn=["Global","Color"],q=new Map,Qe=new Map;function go(e){return!!(e&&e.bridged)}function Xe(){const e=l.projects.find(t=>t.id===l.selectedId);return go(e)?e.id:null}function Re(e,t,a){D.contentWindow?.postMessage({type:"shaderops/action",projectId:e,action:t,payload:a},"*")}function Je(e,t,a,n){D.contentWindow?.postMessage({type:"shaderops/set-param",projectId:e,id:t,value:a,commit:n},"*")}function Gt(e,t,a,n=!0){D.contentWindow?.postMessage({type:"shaderops/set-state",projectId:e,payload:{params:t,extras:a},commit:n},"*")}function Ji(e,t=1500){return new Promise(a=>{const n=()=>a(!0),o=Qe.get(e)||[];o.push(n),Qe.set(e,o),setTimeout(()=>{const i=Qe.get(e);!i||!i.includes(n)||(Qe.set(e,i.filter(r=>r!==n)),a(!1))},t)})}const _a=new Map,Nt=new Map;function yo(e){return`${wi}${e}`}function Oa(){return{loaded:!0,name:Ve,sourcePath:mt}}function bo(e){if(!Ie(e))return Oa();const t=e.loaded!==!1,a=typeof e.name=="string"&&e.name.trim()?e.name.trim():Ve,n=typeof e.sourcePath=="string"&&e.sourcePath.trim()?e.sourcePath.trim():mt;return t?{loaded:!0,name:a,sourcePath:n}:Oa()}function Zi(e,t){S.setItem(yo(e),JSON.stringify(t))}function nn(e,t){const a=bo(t);return _a.set(e,a),Zi(e,a),a}function ea(e){const t=_a.get(e);if(t)return t;const a=fe(S.getItem(yo(e)),null),n=bo(a);return _a.set(e,n),n}function ta(e){return Array.isArray(e)&&e.some(t=>t&&t.type==="image-loader")}function Qi(e){return new Promise((t,a)=>{const n=new Image;n.onload=()=>{const i=Math.min(1,1024/Math.max(n.width,n.height)),r=Math.max(1,Math.floor(n.width*i)),s=Math.max(1,Math.floor(n.height*i)),c=document.createElement("canvas");c.width=r,c.height=s;const u=c.getContext("2d",{willReadFrequently:!0});u.drawImage(n,0,0,r,s),t({imageData:u.getImageData(0,0,r,s),dataUrl:c.toDataURL("image/png")})},n.onerror=()=>a(new Error(`Unable to decode image source: ${e.slice(0,64)}`)),n.src=e})}function er(e){return e===mt?di:e}async function tr(e){const t=typeof e=="string"&&e.trim()?e.trim():mt,a=t;if(!Nt.has(a)){const n=er(t);Nt.set(a,Qi(n).then(({imageData:o})=>o))}return Nt.get(a)}async function on(e,t,a){if(!e||!a||!a.loaded||!a.sourcePath)return!1;const n=await tr(a.sourcePath);return e.postMessage({type:"shaderops/image-load",projectId:t,name:a.name||Ve,sourcePath:a.sourcePath,imageData:n},"*"),!0}async function Wt(e,t=D.contentWindow){const a=nn(e,Oa());await on(t,e,a),l.selectedId===e&&ae(e)}async function ar(e,t=D.contentWindow){const a=ea(e);try{if(await on(t,e,a)){l.selectedId===e&&ae(e);return}}catch(n){console.warn(`ShaderOps: failed to restore image source for ${e}, falling back to Default.`,n)}await Wt(e,t)}async function nr(e,t,a,n=D.contentWindow){const o=typeof t=="string"?t.trim():"";if(!o){await Wt(e,n);return}const i=nn(e,{loaded:!0,name:a||Ve,sourcePath:o});try{await on(n,e,i)}catch(r){console.warn(`ShaderOps: checkpoint image path restore failed for ${e}; using Default.`,r),await Wt(e,n);return}l.selectedId===e&&ae(e)}let ce=null;function or(e){ce&&ce.remove(),ce=document.createElement("input"),ce.type="file",ce.accept="image/*",ce.style.display="none",document.body.appendChild(ce),ce.addEventListener("change",()=>{const t=ce.files&&ce.files[0];if(!t)return;const a=URL.createObjectURL(t),n=new Image;n.onload=()=>{const i=Math.min(1,1024/Math.max(n.width,n.height)),r=Math.max(1,Math.floor(n.width*i)),s=Math.max(1,Math.floor(n.height*i)),c=document.createElement("canvas");c.width=r,c.height=s;const u=c.getContext("2d",{willReadFrequently:!0});u.drawImage(n,0,0,r,s);const d=u.getImageData(0,0,r,s),f=c.toDataURL("image/png");nn(e,{loaded:!0,name:t.name||"image",sourcePath:f}),Nt.set(f,Promise.resolve(d)),D.contentWindow?.postMessage({type:"shaderops/image-load",projectId:e,name:t.name,sourcePath:f,imageData:d},"*"),URL.revokeObjectURL(a),l.selectedId===e&&ae(e)},n.onerror=()=>URL.revokeObjectURL(a),n.src=a}),ce.click()}function ir(e){Wt(e,D.contentWindow)}function xo(e){$t.querySelector(".icon-play").classList.toggle("is-hidden",!e),$t.querySelector(".icon-pause").classList.toggle("is-hidden",!!e),$t.title=e?"Resume (Space)":"Pause (Space)"}function wo(){ht.hidden=!1,lo.hidden=!1}function xa(){ht.hidden=!0,lo.hidden=!0}function ko(e){return`shaderops:locked-controls:${e}`}function So(e){const t=fe(S.getItem(ko(e)),[]);return new Set(Array.isArray(t)?t:[])}function Ut(e,t){S.setItem(ko(e),JSON.stringify([...t]))}function wa(){return`<svg viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
  ...
  </svg>`}function aa(e){const t=q.get(e);if(!t)return;const a={};t.lockedIds.forEach(n=>{n in t.params&&(a[n]=t.params[n])}),t.pendingLockRestore=Object.keys(a).length?a:null,Re(e,"randomizeAll")}function Co(e){return`shaderops:unified-checkpoints:${e}`}function Ke(e){const t=fe(S.getItem(Co(e)),null);return t&&Array.isArray(t.slots)?{slots:t.slots,nextId:t.nextId||t.slots.length+1,activeId:t.activeId??null}:{slots:[],nextId:1,activeId:null}}function na(e,t){S.setItem(Co(e),JSON.stringify(t))}function Eo(e,t){try{return JSON.parse(JSON.stringify(e))}catch{return t}}async function Do(e){return e!==l.selectedId||await da(),l.previews[e]||null}function za(e,t){const a=q.get(e);if(!a)return;const n=Ke(e),o=n.slots.find(s=>s.id===t);if(!o)return;const i={};Object.entries(o.values||{}).forEach(([s,c])=>{s in a.params&&(i[s]=c)}),Object.assign(a.params,i);const r=o.extras&&typeof o.extras=="object";r&&Object.assign(a.extras,o.extras),Gt(e,i,r?o.extras:void 0,!0),o.filters&&Array.isArray(o.filters.stack)&&(P(e),l.filterPreviewOn[e]=o.filters.previewOn!==!1,sn(e,JSON.stringify(o.filters.stack))),ta(a.schema)&&Object.prototype.hasOwnProperty.call(o,"imagePath")&&nr(e,o.imagePath,o.imageName,D.contentWindow),n.activeId=t,na(e,n),l.selectedId===e&&rn(e),Lt(e)}async function rr(e,t){const a=q.get(e);if(!a)return;const n=Ke(e),o=n.slots.find(i=>i.id===t);if(o){if(o.values={...a.params},o.extras=Eo(a.extras||{},{}),o.filters={stack:JSON.parse(vt(e)),previewOn:Fe(e)},ta(a.schema)){const i=ea(e);o.imagePath=i.sourcePath||mt,o.imageName=i.name||Ve}o.preview=await Do(e),n.activeId=t,na(e,n),Lt(e)}}function lr(e,t){const a=Ke(e);a.slots=a.slots.filter(n=>n.id!==t),a.activeId===t&&(a.activeId=null),na(e,a),Lt(e)}async function sr(e){const t=q.get(e);if(!t)return;const a=Ke(e),n=a.nextId++,o=await Do(e),i={id:n,values:{...t.params},extras:Eo(t.extras||{},{}),filters:{stack:JSON.parse(vt(e)),previewOn:Fe(e)},preview:o};if(ta(t.schema)){const r=ea(e);i.imagePath=r.sourcePath||mt,i.imageName=r.name||Ve}a.slots.push(i),a.activeId=n,na(e,a),Lt(e)}function Mo(e,{onTap:t,onHoldComplete:a,holdMs:n=800}={}){const o=e.querySelector(".ucp-ring");let i=null,r=!1,s=!1;const c=()=>{i&&(clearTimeout(i),i=null),e.classList.remove("holding")},u=p=>{p.button!==void 0&&p.button!==0||(r=!0,s=!1,e.classList.add("holding"),o&&o.style.setProperty("--ucp-ring-duration",`${n}ms`),i=setTimeout(()=>{s=!0,e.classList.remove("holding"),e.classList.add("completed"),setTimeout(()=>e.classList.remove("completed"),260),a?.()},n))},d=()=>{r&&(r=!1,c(),s||t?.())},f=()=>{r&&(r=!1,c())};e.addEventListener("pointerdown",u),e.addEventListener("pointerup",d),e.addEventListener("pointerleave",f),e.addEventListener("pointercancel",f)}let xe=null;function cr(){return xe||(xe=document.createElement("div"),xe.className="ucp-menu",xe.hidden=!0,document.body.appendChild(xe),document.addEventListener("click",()=>Ha()),window.addEventListener("blur",()=>Ha()),xe)}function Ha(){xe&&(xe.hidden=!0)}function ur(e,t,a){const n=cr();n.innerHTML="",a.forEach(s=>{const c=document.createElement("button");c.type="button",c.className="ucp-menu-item",c.textContent=s.label,c.addEventListener("click",u=>{u.stopPropagation(),Ha(),s.onClick()}),n.appendChild(c)}),n.hidden=!1;const o=n.getBoundingClientRect(),i=Math.min(e,window.innerWidth-o.width-8),r=Math.min(t,window.innerHeight-o.height-8);n.style.left=`${Math.max(8,i)}px`,n.style.top=`${Math.max(8,r)}px`}function Lt(e){ba.innerHTML="";const t=Ke(e);t.slots.forEach((a,n)=>{const o=document.createElement("span");o.className="ucp-btn-shell",t.activeId===a.id&&o.classList.add("is-active");const i=document.createElement("button");i.type="button",i.className="ucp-slot",i.textContent=String(n+1),i.title="Tap: load this checkpoint Â· Hold 0.8s: overwrite with the current look Â· Right-click: delete",o.appendChild(i);const r=document.createElementNS("http://www.w3.org/2000/svg","svg");r.setAttribute("viewBox","0 0 30 30"),r.setAttribute("class","ucp-ring"),r.setAttribute("aria-hidden","true");const s=document.createElementNS("http://www.w3.org/2000/svg","circle");s.setAttribute("cx","15"),s.setAttribute("cy","15"),s.setAttribute("r","13.3"),s.setAttribute("pathLength","100"),r.appendChild(s),o.appendChild(r),Mo(o,{onTap:()=>za(e,a.id),onHoldComplete:()=>{rr(e,a.id)}}),o.addEventListener("contextmenu",c=>{c.preventDefault(),ur(c.clientX,c.clientY,[{label:"Delete checkpoint",onClick:()=>lr(e,a.id)}])}),ba.appendChild(o)}),ba.appendChild(so)}function nt(e,t){if(!e||!e.showIf)return!0;const a=e.showIf;if(typeof a=="function")try{return!!a(t||{})}catch{return!0}if(typeof a=="string")return!!(t||{})[a];if(Array.isArray(a))return a.every(n=>nt({showIf:n},t));if(a&&typeof a=="object"){if(Array.isArray(a.all))return a.all.every(n=>nt({showIf:n},t));if(Array.isArray(a.any))return a.any.some(n=>nt({showIf:n},t));if(a.eq&&typeof a.eq=="object")return Object.entries(a.eq).every(([n,o])=>String((t||{})[n])===String(o));if(a.neq&&typeof a.neq=="object")return Object.entries(a.neq).every(([n,o])=>String((t||{})[n])!==String(o))}return!0}function ae(e){const t=q.get(e);if(!t)return;t.lockedIds||(t.lockedIds=So(e)),Ft.innerHTML="";const a=new Map;(t.schema||[]).filter(c=>!c.hidden&&nt(c,t.params)).forEach(c=>{const u=c.group||"Controls";a.has(u)||a.set(u,[]),a.get(u).push(c)});const n=[...Pn.filter(c=>a.has(c)),...[...a.keys()].filter(c=>!Pn.includes(c))],o=`shaderops:unified-group-collapsed:${e}`,i=fe(S.getItem(o),{}),r=(c,u)=>{const d=document.createElement("button");d.type="button",d.className="shaderops-lock-button",d.innerHTML=wa();const f=()=>{const p=t.lockedIds.has(c);d.classList.toggle("is-locked",p),u&&u.classList.toggle("is-locked",p),d.title=p?"Unlock this value":"Lock this value (skip on All Random)"};return f(),d.addEventListener("click",p=>{p.preventDefault(),p.stopPropagation(),t.lockedIds.has(c)?t.lockedIds.delete(c):t.lockedIds.add(c),Ut(e,t.lockedIds),f()}),d},s=(c,u)=>{const d=String(c||"").trim(),f=String(u||"").trim();return f?`${d}
${f}`:d};n.forEach((c,u)=>{const d=a.get(c),f=document.createElement("div");f.className="unified-control-group";const p=c.toLowerCase();i[p]&&f.classList.add("is-collapsed");const h=document.createElement("h3");h.textContent=c,h.setAttribute("role","button"),h.setAttribute("tabindex","0");const v=()=>{f.classList.toggle("is-collapsed"),i[p]=f.classList.contains("is-collapsed"),S.setItem(o,JSON.stringify(i))};if(h.addEventListener("click",v),h.addEventListener("keydown",m=>{(m.key==="Enter"||m.key===" ")&&(m.preventDefault(),v())}),f.appendChild(h),u===0){const m=document.createElement("button");m.type="button",m.className="unified-panel-close",m.title="Collapse controls",m.innerHTML='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" aria-hidden="true"><path d="M3.5 3.5l9 9M12.5 3.5l-9 9"/></svg>',m.addEventListener("click",g=>{g.preventDefault(),g.stopPropagation(),Io()}),h.appendChild(m)}d.forEach(m=>{const g=document.createElement("div");g.dataset.controlId=m.id;const C=document.createElement("label"),M=String(m.label||m.id);if(C.textContent=M,C.title=s(M,m.hint),m.type==="shape-stack"){g.className="filter-param-row filter-param-row--shape-stack";const b=document.createElement("div");b.className="shaderops-shape-stack";const A=Array.isArray(m.shapes)?m.shapes:[],N=t.params[m.selectedParam],k=A.find(ee=>String(ee.value)===String(N))||null,I=A.filter(ee=>Number(t.params[ee.enabledParam])>.5),Y=document.createElement("button");if(Y.type="button",Y.className="shaderops-shape-stack-trigger",Y.textContent=k?`${k.label} Â· ${I.length} active`:`${I.length} active`,Y.addEventListener("click",()=>{t.openShapeStackId=t.openShapeStackId===m.id?null:m.id,ae(e)}),b.appendChild(Y),t.openShapeStackId===m.id){const ee=document.createElement("div");ee.className="shaderops-shape-stack-menu",A.forEach(w=>{const H=document.createElement("div");H.className="shaderops-shape-stack-item",k&&String(k.value)===String(w.value)&&H.classList.add("is-selected");const te=document.createElement("input");te.type="checkbox",te.checked=Number(t.params[w.enabledParam])>.5,te.addEventListener("click",B=>B.stopPropagation()),te.addEventListener("change",()=>{const B={[w.enabledParam]:te.checked?1:0};if(te.checked)B[m.selectedParam]=w.value;else if(String(t.params[m.selectedParam])===String(w.value)){const xt=A.find(De=>De!==w&&Number(t.params[De.enabledParam])>.5);B[m.selectedParam]=xt?xt.value:0}Object.assign(t.params,B),Gt(e,B,void 0,!0),ae(e)});const Ee=document.createElement("button");Ee.type="button",Ee.className="shaderops-shape-stack-item-button",Ee.textContent=w.label,Ee.addEventListener("click",()=>{const B={[w.enabledParam]:1,[m.selectedParam]:w.value};Object.assign(t.params,B),Gt(e,B,void 0,!0),ae(e)}),H.appendChild(te),H.appendChild(Ee),ee.appendChild(H)}),b.appendChild(ee)}if(k&&Array.isArray(k.controls)&&Number(t.params[k.enabledParam])>.5){const ee=document.createElement("div");ee.className="shaderops-shape-stack-detail",k.controls.filter(w=>!w.hidden&&nt(w,t.params)).forEach(w=>{const H=document.createElement("div");H.dataset.controlId=w.id;const te=document.createElement("label"),Ee=String(w.label||w.id);if(te.textContent=Ee,te.title=s(Ee,w.hint),w.type==="select"){H.className="filter-param-row filter-param-row--select";const U=document.createElement("select");U.className="filter-param-select";const wt=t.params[w.id]??w.default??0;(w.options||[]).forEach(_e=>{const Oe=document.createElement("option");Oe.value=String(_e.value),Oe.textContent=_e.label,String(_e.value)===String(wt)&&(Oe.selected=!0),U.appendChild(Oe)}),U.addEventListener("change",()=>{const _e=U.value,Oe=Number(_e),wn=Number.isFinite(Oe)&&_e.trim()!==""?Oe:_e;t.params[w.id]=wn,Je(e,w.id,wn,!0)});const Ue=document.createElement("span");Ue.className="shaderops-lock-wrap shaderops-lock-wrap--aux shaderops-lock-wrap--full",Ue.appendChild(U),Ue.appendChild(r(w.id,Ue)),H.appendChild(te),H.appendChild(Ue),ee.appendChild(H);return}H.className="filter-param-row";const B=document.createElement("input");B.type="range",B.min=String(w.min??0),B.max=String(w.max??1),B.step=String(w.step??.01),w.pillMode&&(B.dataset.pillMode=String(w.pillMode));const xt=t.params[w.id]??w.default??0;B.value=String(xt);const De=document.createElement("span");De.className="shaderops-lock-wrap";const $e=document.createElement("input");$e.type="number",$e.step=B.step,$e.value=String(xt);const Ne=document.createElement("button");Ne.type="button",Ne.className="shaderops-lock-button",Ne.innerHTML=wa();const xn=()=>{const U=t.lockedIds.has(w.id);Ne.classList.toggle("is-locked",U),De.classList.toggle("is-locked",U),Ne.title=U?"Unlock this value":"Lock this value (skip on All Random)"};xn(),Ne.addEventListener("click",U=>{U.preventDefault(),U.stopPropagation(),t.lockedIds.has(w.id)?t.lockedIds.delete(w.id):t.lockedIds.add(w.id),Ut(e,t.lockedIds),xn()});const va=(U,wt)=>{t.params[w.id]=U,Je(e,w.id,U,wt)};B.addEventListener("input",()=>{const U=Number(B.value);$e.value=B.value,va(U,!1)}),B.addEventListener("change",()=>{va(Number(B.value),!0)}),$e.addEventListener("change",()=>{let U=Number($e.value);Number.isFinite(U)||(U=Number(B.value));const wt=Number(B.min),Ue=Number(B.max);B.value=String(Math.min(Ue,Math.max(wt,U))),va(U,!0)}),De.appendChild($e),De.appendChild(Ne),H.appendChild(te),H.appendChild(B),H.appendChild(De),ee.appendChild(H)}),b.appendChild(ee)}g.appendChild(C),g.appendChild(b),f.appendChild(g);return}if(m.type==="image-loader"){g.className="filter-param-row filter-param-row--image-loader";const b=ea(e),A=document.createElement("button");A.type="button",A.className="image-loader-load",A.textContent="Load Image",A.title="Load an image â€” strokes will sample its colors",A.addEventListener("click",()=>or(e));const N=document.createElement("span");N.className="image-loader-name",N.textContent=b.loaded?b.name:Ve,N.title=b.loaded?b.name:"Using default image";const k=document.createElement("button");k.type="button",k.className="image-loader-clear",k.textContent="✕",k.title="Reset to Default image",k.hidden=!b.loaded,k.addEventListener("click",()=>ir(e));const I=document.createElement("span");I.className="image-loader-wrap",I.appendChild(A),I.appendChild(N),I.appendChild(k),g.appendChild(C),g.appendChild(I),f.appendChild(g);return}if(m.type==="toggle"){g.className="filter-param-row filter-param-row--toggle";const b=Number(t.params[m.id]??m.default??0)>.5,A=document.createElement("label");A.className="filter-param-switch",A.innerHTML=`
        ...
        `;const N=A.querySelector(".filter-param-toggle");N.addEventListener("change",()=>{const I=N.checked?1:0;t.params[m.id]=I,Je(e,m.id,I,!0),ae(e)});const k=document.createElement("span");k.className="shaderops-lock-wrap shaderops-lock-wrap--aux",k.appendChild(A),k.appendChild(r(m.id,k)),g.appendChild(C),g.appendChild(k),f.appendChild(g);return}if(m.type==="select"){g.className="filter-param-row filter-param-row--select";const b=document.createElement("select");b.className="filter-param-select";const A=t.params[m.id]??m.default??0;(m.options||[]).forEach(k=>{const I=document.createElement("option");I.value=String(k.value),I.textContent=k.label,String(k.value)===String(A)&&(I.selected=!0),b.appendChild(I)}),b.addEventListener("change",()=>{const k=b.value,I=Number(k),Y=Number.isFinite(I)&&k.trim()!==""?I:k;t.params[m.id]=Y,Je(e,m.id,Y,!0),ae(e)});const N=document.createElement("span");N.className="shaderops-lock-wrap shaderops-lock-wrap--aux shaderops-lock-wrap--full",N.appendChild(b),N.appendChild(r(m.id,N)),g.appendChild(C),g.appendChild(N),f.appendChild(g);return}g.className="filter-param-row";const x=document.createElement("input");x.type="range",x.min=String(m.min??0),x.max=String(m.max??1),x.step=String(m.step??.01),m.pillMode&&(x.dataset.pillMode=String(m.pillMode));const V=t.params[m.id]??m.default??0;x.value=String(V);const le=document.createElement("span");le.className="shaderops-lock-wrap";const Q=document.createElement("input");Q.type="number",Q.step=x.step,Q.value=String(V);const se=document.createElement("button");se.type="button",se.className="shaderops-lock-button",se.innerHTML=wa();const bt=()=>{const b=t.lockedIds.has(m.id);se.classList.toggle("is-locked",b),le.classList.toggle("is-locked",b),se.title=b?"Unlock this value":"Lock this value (skip on All Random)"};bt(),se.addEventListener("click",b=>{b.preventDefault(),b.stopPropagation(),t.lockedIds.has(m.id)?t.lockedIds.delete(m.id):t.lockedIds.add(m.id),Ut(e,t.lockedIds),bt()});const Ye=(b,A)=>{t.params[m.id]=b,Je(e,m.id,b,A)};x.addEventListener("input",()=>{const b=Number(x.value);Q.value=x.value,Ye(b,!1)}),x.addEventListener("change",()=>{Ye(Number(x.value),!0)}),Q.addEventListener("change",()=>{let b=Number(Q.value);Number.isFinite(b)||(b=Number(x.value));const A=Number(x.min),N=Number(x.max);x.value=String(Math.min(N,Math.max(A,b))),Ye(b,!0)}),le.appendChild(Q),le.appendChild(se),g.appendChild(C),g.appendChild(x),g.appendChild(le),f.appendChild(g)}),Ft.appendChild(f)}),Ja(Ft),xo(!!t.extras.paused),Lt(e)}function rn(e){const t=q.get(e);if(t){if((t.schema||[]).some(a=>a.type==="shape-stack")){ae(e);return}Ft.querySelectorAll(".filter-param-row").forEach(a=>{const n=a.dataset.controlId;if(!(n in t.params))return;const o=a.querySelector('input[type="range"]'),i=a.querySelector('input[type="number"]'),r=a.querySelector("select.filter-param-select"),s=a.querySelector("input.filter-param-toggle"),c=t.params[n];o&&document.activeElement!==o&&(o.value=String(c)),i&&document.activeElement!==i&&(i.value=String(c)),r&&document.activeElement!==r&&(r.value=String(c)),s&&document.activeElement!==s&&(s.checked=Number(c)>.5)}),xo(!!t.extras.paused)}}function dr(e,t=null){const{type:a,projectId:n}=e;if(n){if(a==="shaderops/ready"){const o=Array.isArray(e.schema)?e.schema:[];if(q.set(n,{schema:o,params:{...e.values||{}},extras:{...e.extras||{}},lockedIds:So(n)}),ta(o)){const i=t&&typeof t.postMessage=="function"?t:D.contentWindow;ar(n,i)}l.selectedId===n&&(ae(n),wo(),ot())}else if(a==="shaderops/values-changed"){const o=q.get(n);if(!o)return;if(e.values&&Object.assign(o.params,e.values),e.extras&&Object.assign(o.extras,e.extras),o.pendingLockRestore){const r=o.pendingLockRestore;o.pendingLockRestore=null,Object.entries(r).forEach(([s,c])=>{o.params[s]=c,Je(n,s,c,!0)})}const i=Qe.get(n);i&&i.length&&(Qe.set(n,[]),i.forEach(r=>{try{r()}catch{}})),l.selectedId===n&&rn(n)}else if(a==="shaderops/keydown"){if(l.selectedId!==n)return;e.key==="r"?aa(n):e.key==="space"&&Re(n,"togglePause")}}}Oi.addEventListener("click",()=>{ht.classList.remove("is-collapsed"),S.setItem(an,"0")});function Io(){ht.classList.add("is-collapsed"),S.setItem(an,"1")}ht.addEventListener("dblclick",e=>{e.target.closest(".unified-panel-body")||Io()});zi.addEventListener("click",()=>{const e=Xe();e&&aa(e)});Hi.addEventListener("click",()=>{const e=Xe();e&&Re(e,"rerollSeed")});$t.addEventListener("click",()=>{const e=Xe();e&&Re(e,"togglePause")});qi.addEventListener("click",()=>{const e=Xe();e&&Re(e,"undo")});Gi.addEventListener("click",()=>{const e=Xe();e&&Re(e,"redo")});Wi?.addEventListener("click",()=>{window.location.reload()});Mo(so,{onHoldComplete:()=>{const e=Xe();e&&sr(e)}});document.addEventListener("keydown",e=>{const t=Xe();t&&(e.target?.matches("input, textarea, select, button")||((e.key==="r"||e.key==="R")&&!e.metaKey&&!e.ctrlKey&&!e.altKey?(e.preventDefault(),aa(t)):e.code==="Space"&&(e.preventDefault(),Re(t,"togglePause"))))});S.getItem(an)==="1"&&ht.classList.add("is-collapsed");function fe(e,t){if(e==null)return t;try{const a=JSON.parse(e);return a??t}catch{return t}}function Ie(e){return e!==null&&typeof e=="object"&&!Array.isArray(e)}function fr(){const e="ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789";let t="";for(let a=0;a<3;a++)t+=e[Math.floor(Math.random()*e.length)];return t}function Bo(e,t=""){const a=new Date,n=String(a.getFullYear()),o=String(a.getMonth()+1).padStart(2,"0"),i=String(a.getDate()).padStart(2,"0"),r=String(a.getHours()).padStart(2,"0"),s=String(a.getMinutes()).padStart(2,"0"),c=String(a.getSeconds()).padStart(2,"0"),u=String(t||"").trim(),d=u?`-${u}`:"";return`${e||"shader"}-${n}${o}${i}-${r}${s}${c}${d}-${fr()}.png`}function pr(e){const t=q.get(e),a=JSON.parse(vt(e));return{source:"shaderops-png-state",schemaVersion:Ba,exportedAt:new Date().toISOString(),projectId:e,bridge:{params:t?{...t.params}:{},extras:t?{...t.extras}:{},lockedIds:t&&t.lockedIds?[...t.lockedIds]:[]},filters:{previewOn:Fe(e),stack:Array.isArray(a)?a:[]}}}function mr(e,t=2500){return q.has(e)?Promise.resolve(!0):new Promise(a=>{const n=performance.now(),o=()=>{if(q.has(e)){a(!0);return}if(performance.now()-n>=t){a(!1);return}requestAnimationFrame(o)};o()})}const qe=new Uint8Array([137,80,78,71,13,10,26,10]),jt=new TextEncoder,Lo=new TextDecoder,hr=(()=>{const e=new Uint32Array(256);for(let t=0;t<256;t++){let a=t;for(let n=0;n<8;n++)a=a&1?3988292384^a>>>1:a>>>1;e[t]=a>>>0}return e})();function vr(e){let t=4294967295;for(let a=0;a<e.length;a++)t=hr[(t^e[a])&255]^t>>>8;return(t^4294967295)>>>0}function gr(e,t){const a=new Uint8Array(12+t.length),n=new DataView(a.buffer);n.setUint32(0,t.length),a.set(e,4),a.set(t,8);const o=new Uint8Array(4+t.length);return o.set(e,0),o.set(t,4),n.setUint32(8+t.length,vr(o)),a}function yr(e){const t=e.reduce((o,i)=>o+i.length,0),a=new Uint8Array(t);let n=0;for(const o of e)a.set(o,n),n+=o.length;return a}function br(e){if(!e||typeof e!="string")return null;const t=e.indexOf(",");if(t===-1)return null;const a=e.slice(t+1),n=atob(a),o=new Uint8Array(n.length);for(let i=0;i<n.length;i++)o[i]=n.charCodeAt(i);return o}function xr(e,t){const a=jt.encode(e),n=jt.encode(t),o=new Uint8Array(a.length+5+n.length);o.set(a,0);let i=a.length;return o[i++]=0,o[i++]=0,o[i++]=0,o[i++]=0,o[i++]=0,o.set(n,i),o}function wr(e,t,a){if(!e||e.length<12)throw new Error("PNG data is empty.");for(let u=0;u<qe.length;u++)if(e[u]!==qe[u])throw new Error("Exported image is not a valid PNG.");const n=jt.encode(t),o=jt.encode("iTXt"),i=gr(o,xr(t,a)),r=[qe];let s=qe.length,c=!1;for(;s+12<=e.length;){const d=new DataView(e.buffer,e.byteOffset+s,8).getUint32(0),f=s+4,p=s+8,h=d+12;if(s+h>e.length)throw new Error("PNG chunk stream is truncated.");const v=e.subarray(f,f+4),m=e.subarray(p,p+d),C=v[0]===105&&v[1]===84&&v[2]===88&&v[3]===116&&m.length>=n.length+1&&m.subarray(0,n.length).every((x,V)=>x===n[V])&&m[n.length]===0,M=v[0]===73&&v[1]===69&&v[2]===78&&v[3]===68;C||(!c&&M&&(r.push(i),c=!0),r.push(e.subarray(s,s+h))),s+=h}return c||r.push(i),yr(r)}function ka(e,t){let a=t;for(;a<e.length&&e[a]!==0;)a+=1;return{text:Lo.decode(e.subarray(t,a)),next:a+1}}function kr(e,t){if(!e||e.length<12)return null;for(let n=0;n<qe.length;n++)if(e[n]!==qe[n])return null;let a=qe.length;for(;a+12<=e.length;){const o=new DataView(e.buffer,e.byteOffset+a,8).getUint32(0),i=a+4,r=a+8,s=o+12;if(a+s>e.length)return null;const c=e.subarray(i,i+4),u=e.subarray(r,r+o),d=String.fromCharCode(c[0],c[1],c[2],c[3]);if(d==="iTXt"){const f=ka(u,0);if(f.text===t&&f.next+3<=u.length){const p=u[f.next];let h=f.next+2;if(h=ka(u,h).next,h=ka(u,h).next,p===0&&h<=u.length)return Lo.decode(u.subarray(h))}}if(d==="IEND")break;a+=s}return null}function Sr(e){return Ie(e)?{stack:(Array.isArray(e.stack)?e.stack:[]).map(n=>!Ie(n)||typeof n.defId!="string"||!Ae(n.defId)?null:{instanceId:typeof n.instanceId=="string"?n.instanceId:`f${Date.now()}_${To++}`,defId:n.defId,enabled:n.enabled!==!1,collapsed:!!n.collapsed,params:{...Wa(n.defId),...Ie(n.params)?n.params:{}}}).filter(Boolean),previewOn:e.previewOn!==!1}:null}async function Cr(e){if(!Ie(e))throw new Error("PNG metadata is invalid JSON.");const t=Number(e.schemaVersion)||1;t>Ba&&console.warn(`PNG state schema v${t} is newer than this app (v${Ba}); applying compatible fields only.`);const a=typeof e.projectId=="string"?e.projectId:null,o=a&&l.projects.some(c=>c.id===a)?a:l.selectedId;if(!o)throw new Error("No target project is available for restore.");if(l.selectedId!==o&&await mn(o),!await mr(o))throw new Error("Project controls are not ready yet. Please try dropping the PNG again.");const r=q.get(o);if(r&&Ie(e.bridge)){const c=Ie(e.bridge.params)?{...e.bridge.params}:{},u=Ie(e.bridge.extras)?{...e.bridge.extras}:void 0;Gt(o,c,u,!0),Object.assign(r.params,c),u&&Object.assign(r.extras,u),Array.isArray(e.bridge.lockedIds)&&(r.lockedIds=new Set(e.bridge.lockedIds.filter(d=>typeof d=="string")),Ut(o,r.lockedIds)),l.selectedId===o&&rn(o)}const s=Sr(e.filters);s&&(l.filterStacks[o]=s.stack,l.filterPreviewOn[o]=s.previewOn,pe(),l.selectedId===o&&F()),ye()}async function Er(e){if(!e||!/\.png$/i.test(e.name||""))return!1;const t=new Uint8Array(await e.arrayBuffer()),a=kr(t,no);if(!a)return!1;const n=fe(a,null);if(!n)throw new Error("PNG state chunk exists but contains invalid JSON.");return await Cr(n),!0}function Dr(){const e=fe(S.getItem(Jn),{});l.previews=e&&typeof e=="object"&&!Array.isArray(e)?e:{}}function Mr(){const e=fe(S.getItem(Qn),[]);l.favorites=Array.isArray(e)?e:[]}function Po(){S.setItem(Qn,JSON.stringify(l.favorites))}function ln(){S.setItem(eo,JSON.stringify(l.projects.map(e=>e.id)))}function Ir(){const e=fe(S.getItem(eo),null);if(!Array.isArray(e)||e.length===0)return;const t=new Map(l.projects.map(i=>[i.id,i])),a=e.map(i=>t.get(i)).filter(Boolean),n=new Set(a.map(i=>i.id)),o=l.projects.filter(i=>!n.has(i.id));l.projects=[...a,...o]}function Br(){const e=fe(S.getItem(to),null);l.filterAddOrder=e&&typeof e=="object"?e:{}}function Lr(){S.setItem(to,JSON.stringify(l.filterAddOrder))}function Pr(e,t){const a=l.filterAddOrder&&l.filterAddOrder[e];if(!Array.isArray(a)||a.length===0)return t;const n=new Map(t.map(s=>[s.id,s])),o=a.map(s=>n.get(s)).filter(Boolean),i=new Set(o.map(s=>s.id)),r=t.filter(s=>!i.has(s.id));return[...o,...r]}function oa(){const e={...l.previews};let t=Object.keys(e);for(let a=0;a<=t.length;a++)try{S.setItem(Jn,JSON.stringify(e));return}catch(n){const o=t.find(i=>i!==l.selectedId&&e[i]);if(!o){console.warn("ShaderOps: preview storage quota exceeded; skipping persistence this cycle.",n);return}delete e[o],t=t.filter(i=>i!==o)}}function Ao(e,t=320,a=.72){return new Promise(n=>{if(!e||typeof e!="string"||!e.startsWith("data:image")){n(e);return}const o=new Image;o.onload=()=>{const i=Math.min(1,t/(o.width||t)),r=Math.max(1,Math.round((o.width||t)*i)),s=Math.max(1,Math.round((o.height||t)*i));try{const c=document.createElement("canvas");c.width=r,c.height=s,c.getContext("2d").drawImage(o,0,0,r,s),n(c.toDataURL("image/jpeg",a))}catch{n(e)}},o.onerror=()=>n(e),o.src=e})}function Ar(){const e=fe(S.getItem(Zn),{}),t=e&&typeof e=="object"&&!Array.isArray(e)?e:{};Object.entries(t).forEach(([a,n])=>{if(!n||typeof n!="object")return;const o=Array.isArray(n.stack)?n.stack:[];l.filterStacks[a]=o.map(i=>!i||typeof i!="object"?i:{...i,params:{...Wa(i.defId),...i.params}}),l.filterPreviewOn[a]=n.previewOn!==!1}),en.classList.toggle("collapsed",S.getItem(Ya)==="1")}function pe(){const e={};Object.keys(l.filterStacks).forEach(t=>{e[t]={stack:l.filterStacks[t],previewOn:l.filterPreviewOn[t]!==!1}}),S.setItem(Zn,JSON.stringify(e))}function G(e){return l.filterStacks[e]||(l.filterStacks[e]=[]),l.filterStacks[e]}function Fe(e){return l.filterPreviewOn[e]!==!1}function An(e){return e?Fe(e)&&G(e).some(t=>t&&t.enabled):!1}function ia(e){return l.filterHistory[e]||(l.filterHistory[e]={undoStack:[],redoStack:[],suppress:!1,limit:100}),l.filterHistory[e]}function vt(e){return JSON.stringify(G(e))}function P(e,t){if(!e)return;const a=ia(e);if(a.suppress)return;const n=vt(e);a.undoStack[a.undoStack.length-1]!==n&&(a.undoStack.push(n),a.undoStack.length>a.limit&&a.undoStack.shift(),a.redoStack.length=0)}function sn(e,t){const a=ia(e);let n;try{n=JSON.parse(t)}catch{return}a.suppress=!0,l.filterStacks[e]=n,l.filterPreviewOn[e]===void 0&&(l.filterPreviewOn[e]=!0),pe(),F(),!E.hidden&&z&&l.selectedId===e&&de(),!L.hidden&&O&&l.selectedId===e&&Ce(),a.suppress=!1}function Tr(e){if(!e)return;const t=ia(e);if(t.undoStack.length===0)return;const a=vt(e),n=t.undoStack.pop();t.redoStack.push(a),sn(e,n)}function Rr(e){if(!e)return;const t=ia(e);if(t.redoStack.length===0)return;const a=vt(e),n=t.redoStack.pop();t.undoStack.push(a),sn(e,n)}let To=1;function Fr(e,t){if(!Ae(t))return;P(e),G(e).unshift({instanceId:`f${Date.now()}_${To++}`,defId:t,enabled:!0,collapsed:!1,params:Wa(t)}),l.filterPreviewOn[e]===void 0&&(l.filterPreviewOn[e]=!0),pe(),F()}function $r(e,t){const a=G(e),n=a.findIndex(o=>o.instanceId===t);n!==-1&&(P(e),a.splice(n,1),pe(),F())}function Nr(e,t){const n=G(e).find(o=>o.instanceId===t);n&&(P(e),n.enabled=!n.enabled,pe(),F())}function Ur(e,t){const n=G(e).find(o=>o.instanceId===t);n&&(n.collapsed=!n.collapsed,pe(),F())}function ke(e,t,a,n,o){const r=G(e).find(s=>s.instanceId===t);r&&(o||P(e),r.params[a]=n,pe())}const _r={oilPaint:{key:"preset",values:{custom:null,paletteKnife:{brushType:"palette",brushSize:10.5,strokeStrength:.84,detail:.74,edgeBlend:.33,strokePresence:.86,mix:.92},softBrush:{brushType:"round",brushSize:14,strokeStrength:.62,detail:.52,edgeBlend:.72,strokePresence:.56,mix:.78}}},filmEmulation:{key:"stock",values:{neutral:{grain:.35,grainSize:1.1,grainAnimated:!0,halation:.25,haloSize:2.6,contrast:.05,saturation:1,fade:.02,temperature:0,tint:0,vignette:.25},portra:{grain:.45,grainSize:1.4,grainAnimated:!0,halation:.55,haloSize:3.2,contrast:.12,saturation:1,fade:.08,temperature:0,tint:0,vignette:.4},velvia:{grain:.38,grainSize:1.2,grainAnimated:!0,halation:.4,haloSize:2.8,contrast:.28,saturation:1.45,fade:.05,temperature:.05,tint:.03,vignette:.45},trix:{grain:.9,grainSize:1,grainAnimated:!0,halation:.2,haloSize:2.3,contrast:.45,saturation:0,fade:.1,temperature:-.02,tint:.02,vignette:.55},cinestill:{grain:.65,grainSize:1.8,grainAnimated:!0,halation:1.4,haloSize:4.5,contrast:.18,saturation:1.05,fade:.1,temperature:-.2,tint:.05,vignette:.5},bleach:{grain:.5,grainSize:1.3,grainAnimated:!0,halation:.35,haloSize:2.8,contrast:.6,saturation:.55,fade:.03,temperature:0,tint:0,vignette:.62}}}};function Or(e,t,a,n){const i=G(e).find(c=>c.instanceId===t);if(!i)return;const r=_r[i.defId||i.filterId];if(!r||r.key!==a)return;const s=r.values?.[String(n)];!s||typeof s!="object"||Object.entries(s).forEach(([c,u])=>{ke(e,t,c,u,!0)})}const Tn=["Color","Distort","Blur","Effect"];function zr(){const e=new Map;Wn.forEach(a=>{const n=a.group||"Other";e.has(n)||e.set(n,[]),e.get(n).push(a)});const t=[...Tn.filter(a=>e.has(a)),...[...e.keys()].filter(a=>!Tn.includes(a))];J.innerHTML=t.map(a=>`
        ...
        </div>`).join(""),J.querySelectorAll(".filter-add-grid[data-group]").forEach(a=>{Hr(a,a.dataset.group)})}function Hr(e,t){let a=null;e.addEventListener("dragstart",n=>{const o=n.target.closest(".filter-add-option");if(!o){n.preventDefault();return}a=o,o.classList.add("dragging"),n.dataTransfer.effectAllowed="move",n.dataTransfer.setData("text/plain",o.dataset.filterId)}),e.addEventListener("dragover",n=>{if(!a)return;n.preventDefault(),n.dataTransfer.dropEffect="move";const o=n.target.closest(".filter-add-option");if(!o||o===a){o||e.appendChild(a);return}const i=o.getBoundingClientRect();n.clientX<i.left+i.width/2?o.parentNode.insertBefore(a,o):o.parentNode.insertBefore(a,o.nextSibling)}),e.addEventListener("drop",n=>{if(n.preventDefault(),!a)return;const o=[...e.querySelectorAll(".filter-add-option")].map(i=>i.dataset.filterId);l.filterAddOrder[t]=o,Lr()}),e.addEventListener("dragend",()=>{a=null,e.querySelectorAll(".filter-add-option.dragging").forEach(n=>n.classList.remove("dragging"))})}document.body.appendChild(J);function Ro(){const e=Qt.getBoundingClientRect(),t=J.querySelectorAll(".filter-add-group").length||4,a=t*150+(t-1)*14+20,n=Math.max(320,Math.min(a,window.innerWidth-16));let o=e.right-n;o=Math.max(8,Math.min(o,window.innerWidth-n-8)),J.style.width=`${n}px`,J.style.left=`${o}px`,J.style.top=`${e.bottom+6}px`}function qr(){Ro(),J.hidden=!1,Qt.classList.add("open")}function ra(){J.hidden=!0,Qt.classList.remove("open")}window.addEventListener("resize",()=>{J.hidden||Ro()});function Fo(e,t,a,n=48){let o="";for(let i=0;i<=n;i++){const r=i/n,s=oi(e,r),c=r*t,u=(1-s)*a;o+=i===0?`M${c.toFixed(2)},${u.toFixed(2)}`:` L${c.toFixed(2)},${u.toFixed(2)}`}return o}function $o(e,t,a,n,o,i=56){const r=Math.max(0,Math.min(.99,e)),s=Math.max(r+.001,Math.min(1,a)),c=Math.max(.05,t);let u="";for(let d=0;d<=i;d++){const f=d/i,p=Math.max(0,Math.min(1,(f-r)/(s-r))),h=Math.pow(p,1/c),v=f*n,m=(1-h)*o;u+=d===0?`M${v.toFixed(2)},${m.toFixed(2)}`:` L${v.toFixed(2)},${m.toFixed(2)}`}return u}function No(e,t,a){const n=Math.max(0,Math.min(.99,e)),o=Math.max(n+.001,Math.min(1,a)),i=Math.max(.05,t),r=n+(o-n)*Math.pow(.5,i);return{black:n,mid:Math.max(n,Math.min(o,r)),white:o}}function cn(e){if(!e)return;const t=_.querySelector(`.filter-levels-graph[data-instance-id="${e}"]`);if(!t)return;const a=(m,g)=>{const C=_.querySelector(`input[data-instance-id="${e}"][data-param-key="${m}"]`),M=C?Number(C.value):g;return Number.isFinite(M)?M:g},n=a("blackPoint",0),o=a("gamma",1),i=a("whitePoint",1),r=t.querySelector(".filter-levels-curve"),s=t.querySelector(".filter-levels-marker--black"),c=t.querySelector(".filter-levels-marker--mid"),u=t.querySelector(".filter-levels-marker--white"),d=t.querySelector('.filter-levels-handle[data-handle="black"]'),f=t.querySelector('.filter-levels-handle[data-handle="mid"]'),p=t.querySelector('.filter-levels-handle[data-handle="white"]');if(!r||!s||!c||!u)return;const h=$o(n,o,i,100,46,56),v=No(n,o,i);r.setAttribute("d",h),s.setAttribute("x1",(v.black*100).toFixed(2)),s.setAttribute("x2",(v.black*100).toFixed(2)),c.setAttribute("x1",(v.mid*100).toFixed(2)),c.setAttribute("x2",(v.mid*100).toFixed(2)),u.setAttribute("x1",(v.white*100).toFixed(2)),u.setAttribute("x2",(v.white*100).toFixed(2)),d&&(d.style.left=`${(v.black*100).toFixed(2)}%`),f&&(f.style.left=`${(v.mid*100).toFixed(2)}%`),p&&(p.style.left=`${(v.white*100).toFixed(2)}%`)}function Sa(e,t,a){ke(l.selectedId,e,t,a,!0),_.querySelectorAll(`input[data-instance-id="${e}"][data-param-key="${t}"]`).forEach(n=>{n.value=a})}function Gr(e,t){const a=e.params[t.key];if(t.type==="select"){const n=(t.options||[]).map(o=>`<option value="${o.value}" ${a===o.value?"selected":""}>${o.label}</option>`).join("");return`
      ...
      </div>`}if(t.type==="curve"){const n=It(a),o=Fo(n,100,100,40);return`
      ...
      </div>`}if(t.type==="gradient"){const n=pt(a),o=Gn(n);return`
      ...
      </div>`}if(t.type==="levelsGraph"){const n=Number.isFinite(e.params[t.blackKey])?e.params[t.blackKey]:0,o=Number.isFinite(e.params[t.gammaKey])?e.params[t.gammaKey]:1,i=Number.isFinite(e.params[t.whiteKey])?e.params[t.whiteKey]:1,r=$o(n,o,i,100,46,56),s=No(n,o,i);return`
    ...
    <div class="filter-param-row">
    ...
    </div>`}function F(){const e=l.selectedId,t=e?G(e):[];if(Mi.style.display=t.length===0?"block":"none",Ht.checked=e?Fe(e):!1,Ht.disabled=!e,oo.disabled=!e,me.disabled=!e,_.innerHTML=t.map((a,n)=>{const o=Ae(a.defId);if(!o)return"";const i=o.params.filter(r=>nt(r,a.params)).map(r=>Gr(a,r)).join("");return`
      ...
      </div>`}).join(""),z&&!E.hidden){const a=_.querySelector(`[data-action="open-curve"][data-instance-id="${z.instanceId}"][data-param-key="${z.paramKey}"]`);a?(z.trigger=a,yn()):pa()}if(O&&!L.hidden){const a=_.querySelector(`[data-action="open-gradient"][data-instance-id="${O.instanceId}"][data-param-key="${O.paramKey}"]`);a?(O.trigger=a,Jo()):fa()}Ja(_),_.querySelectorAll(".filter-levels-graph[data-instance-id]").forEach(a=>{cn(a.dataset.instanceId)}),Wr()}let Rn=null;function Wr(){clearTimeout(Rn),Rn=setTimeout(jr,220)}async function un(e){if(!e||!ve||l.selectedId!==e)return!1;const t=G(e).filter(i=>i.enabled);if(R.style.display==="none"||!Fe(e)||t.length===0)return!1;const a=[...t].reverse(),n=ve.captureAtScale(a,Xt,1);if(!n)return!1;const o=await Ao(n.dataUrl);return l.selectedId!==e?!1:(l.previews[e]=o,oa(),zo(e),!0)}function jr(){const e=l.selectedId;e&&un(e).then(t=>{t||da()})}function la(e){const t=["linear-gradient(135deg,#2a3f88 0%,#18274f 100%)","linear-gradient(135deg,#5d2a83 0%,#251749 100%)","linear-gradient(135deg,#85481e 0%,#3a2112 100%)","linear-gradient(135deg,#165b67 0%,#0f2e35 100%)"];return t[e%t.length]}function Uo(e){const t=e.entry.includes("?")?"&":"?";return`${e.entry}${t}project=${encodeURIComponent(e.id)}`}function _o(e,t){if(!e||e.getElementById("shaderopsUiEnhancements")||!e.getElementById("app")&&!e.body)return;const a=e.createElement("style");a.id="shaderopsUiEnhancements",a.textContent=`
  ...
  `,e.head?.appendChild(a),Ja(e);const n=e.getElementById("allRandomButton");n&&!n.querySelector("svg")&&(n.textContent="",n.title="All random (R)",n.innerHTML=`
      ...
      <svg viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
        ...
        </button>`}).join("")}function zo(e){if(!e)return;const t=l.projects.findIndex(i=>i.id===e);if(t===-1)return;const a=l.previews[e],n=a?`background-image:url('${a}');background-size:cover;`:`background:${la(t)};`,o=CSS.escape(e);document.querySelectorAll(`[data-project-id="${o}"]`).forEach(i=>i.setAttribute("style",n))}let St=null,ue=null,ct=null,Fn="rail",Z=[],Be=0;function sa(){St&&(clearTimeout(St),St=null)}function be(){sa(),ue=null,ct=null,Z=[],Be=0,re.hidden=!0}function Kr(e){return l.projects.find(t=>t.id===e)||null}function Yr(e,t){const a=Ke(e);return a.slots.length?a.slots.map((n,o)=>{const i=typeof n.preview=="string"&&n.preview?n.preview:null,r=i?`background-image:url('${i}');background-size:cover;background-position:center;`:t+"opacity:0.42;",s=i?`background-image:url('${i}');background-size:cover;background-position:center;`:t;return{slotId:n.id,image:i,thumbStyle:r,heroStyle:s,title:i?`Checkpoint ${o+1}`:`Checkpoint ${o+1} (no snapshot yet)`}}):[]}function Ho(){if(!Z.length){Ln.innerHTML='<span class="thumb-hover-preview__empty">No checkpoints</span>';return}Ln.innerHTML=Z.map((e,t)=>`<span class="thumb-hover-preview__cp${e.image?"":" thumb-hover-preview__cp--empty"}${t===Be?" is-active":""}" style="${e.thumbStyle}" title="${e.title}"></span>`).join("")}function Jr(e,t){const a=re.getBoundingClientRect(),n=e.getBoundingClientRect(),o=10,i=window.innerWidth-a.width-8,r=window.innerHeight-a.height-8;let s=n.right+o,c=n.top+n.height*.5-a.height*.5;if(t==="effect-panel"){const u=j.hidden?n:j.getBoundingClientRect();s=u.right+o,s>i&&(s=u.left-a.width-o)}else s>i&&(s=n.left-a.width-o);re.style.left=`${Math.max(8,Math.min(s,i))}px`,re.style.top=`${Math.max(8,Math.min(c,r))}px`}function Zr(e,t,a){const n=Kr(t);if(!n)return;const o=Math.max(0,l.projects.findIndex(u=>u.id===t)),i=`background:${la(o)};`,r=l.previews[t]||null,s=r?`background-image:url('${r}');background-size:cover;background-position:center;`:i;Z=Yr(t,i),Be=0;const c=Z[0]?.heroStyle||s;ji.textContent=n.name,co.setAttribute("style",c),Ho(),re.dataset.source=a,re.hidden=!1,Jr(e,a)}function qo(e,t,a){sa(),ue=e,ct=t,Fn=a,St=setTimeout(()=>{St=null,!(!ue||!ue.isConnected)&&Zr(ue,ct,Fn)},xi)}function Go(e){if(!Z.length)return;Be=(Be+e+Z.length)%Z.length;const t=Z[Be]?.heroStyle;t&&co.setAttribute("style",t),Ho()}function Wo(e,t){if(ct!==e||Z.length===0||t&&ue&&ue.dataset.projectId!==t.dataset.projectId)return null;const a=Z[Be]?.slotId;return a||(Ke(e).slots[Be]?.id??null)}function jo(e,t){if(!t)return;if(q.has(e)){za(e,t);return}let a=0;const n=setInterval(()=>{if(a+=1,q.has(e)){clearInterval(n),za(e,t);return}a>30&&clearInterval(n)},50)}function Qr(e){const t=l.projects.findIndex(n=>n.id===e);if(t===-1)return;const a=Oo();l.navPage=Math.floor(t/a),ye()}const el='<svg viewBox="0 0 16 16" fill="currentColor" aria-hidden="true"><path d="M8 1.4l1.98 4.02 4.44.65-3.21 3.13.76 4.42L8 11.55l-3.97 2.07.76-4.42-3.21-3.13 4.44-.65z"/></svg>',tl='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.1" stroke-linejoin="round" aria-hidden="true"><path d="M8 1.4l1.98 4.02 4.44.65-3.21 3.13.76 4.42L8 11.55l-3.97 2.07.76-4.42-3.21-3.13 4.44-.65z"/></svg>';function Ca(e,t){const a=e.id===l.selectedId,n=l.favorites.includes(e.id),o=l.previews[e.id],i=o?`background-image:url('${o}');background-size:cover;`:`background:${la(t)};`;return`
    ...
    </div>`}function Vt(e){return!e||typeof e!="object"?!1:Array.isArray(e.tags)&&e.tags.some(t=>String(t).toLowerCase()==="3d")?!0:/\b3d\b/i.test(String(e.engine||""))}function ca(){const e=Ta.closest(".nav-grid-section"),t=l.favorites.map(c=>l.projects.find(u=>u.id===c)).filter(Boolean);e.classList.toggle("empty",t.length===0),Ta.innerHTML=t.map(c=>Ca(c,l.projects.indexOf(c))).join("");const a=new Set(l.favorites),n=Ra.closest(".nav-grid-section"),o=Fa.closest(".nav-grid-section"),i=l.projects.filter(c=>!a.has(c.id)),r=i.filter(c=>!Vt(c)),s=i.filter(c=>Vt(c));n&&n.classList.toggle("empty",r.length===0),o&&o.classList.toggle("empty",s.length===0),Ra.innerHTML=r.map(c=>Ca(c,l.projects.indexOf(c))).join(""),Fa.innerHTML=s.map(c=>Ca(c,l.projects.indexOf(c))).join("")}function Vo(){const e=new Set(l.favorites),t=l.favorites.map(n=>l.projects.find(o=>o.id===n)).filter(Boolean),a=l.projects.filter(n=>!e.has(n.id));l.projects=[...t,...a]}function al(e){const t=l.favorites.indexOf(e);t===-1?l.favorites.push(e):l.favorites.splice(t,1),Po(),Vo(),ln(),ca(),ye()}function nl(){ca(),j.hidden=!1,Za.hidden=!1,Zt.classList.add("active")}function ua(){j.hidden||(j.hidden=!0,Za.hidden=!0,Zt.classList.remove("active"),be())}function fn(e,t){let a=null,n=null;e.addEventListener("dragstart",o=>{if(o.target.closest("[data-fav-toggle]")){o.preventDefault();return}const i=o.target.closest(".nav-grid-thumb");if(!i){o.preventDefault();return}a=i.dataset.projectId,n=i,i.classList.add("dragging"),o.dataTransfer.effectAllowed="move",o.dataTransfer.setData("text/plain",a)}),e.addEventListener("dragover",o=>{if(!n)return;o.preventDefault(),o.dataTransfer.dropEffect="move";const i=o.target.closest(".nav-grid-thumb");if(!i||i===n){i||e.appendChild(n);return}const r=i.getBoundingClientRect();o.clientX<r.left+r.width/2?i.parentNode.insertBefore(n,i):i.parentNode.insertBefore(n,i.nextSibling)}),e.addEventListener("drop",o=>{if(o.preventDefault(),!a)return;const i=[...e.querySelectorAll(".nav-grid-thumb")].map(r=>r.dataset.projectId);t(i)}),e.addEventListener("dragend",()=>{a=null,n=null,e.querySelectorAll(".nav-grid-thumb.dragging").forEach(o=>o.classList.remove("dragging"))})}function pn(){const e=l.projects.find(t=>t.id===l.selectedId);if(!e){La.textContent="No project selected",Pa.textContent="â€”",D.src="about:blank",Aa.disabled=!0,xa();return}La.textContent=e.name,Pa.textContent=e.engine,D.classList.add("frame-swapping"),D.src=Uo(e),Aa.disabled=!1,S.setItem(zt,e.id),qa=null,F(),go(e)?(xa(),q.get(e.id)&&(ae(e.id),wo()),requestAnimationFrame(()=>requestAnimationFrame(ot))):(xa(),ol(D,e.id))}function ot(){D.classList.remove("frame-swapping")}function ol(e,t){let a=0;const n=()=>{if(e!==D||l.selectedId!==t)return;a+=1;let o=null;try{o=e.contentDocument}catch{o=null}if(o&&o.readyState!=="loading"&&(o.getElementById("app")||o.body)){try{_o(o,t)}catch(i){console.warn("ShaderOps: unable to install project UI enhancements (early).",i)}requestAnimationFrame(()=>requestAnimationFrame(ot));return}a<300?requestAnimationFrame(n):ot()};requestAnimationFrame(n),setTimeout(ot,1500)}window.addEventListener("message",async e=>{const t=e.data;if(t&&typeof t.type=="string"&&t.type.startsWith("shaderops/")&&(t.type==="shaderops/ready"||t.type==="shaderops/values-changed"||t.type==="shaderops/keydown")){dr(t,e.source||null);return}const a=dn(e.data);if(!a)return;const{projectId:n}=e.data;n&&n!==l.selectedId&&An(n)&&l.previews[n]||n===l.selectedId&&n&&An(n)&&(await un(n)||l.previews[n])||(l.previews[n]=await Ao(a),oa(),zo(n))});function da(){if(l.captureInFlight)return Promise.resolve();const e=l.projects.find(a=>a.id===l.selectedId),t=D.contentWindow;return!e||!t?Promise.resolve():un(e.id).then(a=>{if(!a)return l.captureInFlight=!0,new Promise(n=>{const o=setTimeout(()=>{window.removeEventListener("message",i),l.captureInFlight=!1,n()},1500),i=r=>{dn(r.data)&&r.data.projectId===e.id&&(clearTimeout(o),window.removeEventListener("message",i),l.captureInFlight=!1,n())};window.addEventListener("message",i),t.postMessage({type:"shaderops/request-preview",projectId:e.id},"*")})})}async function il(e){return new Promise(t=>{const a=document.createElement("iframe");a.style.cssText="position:fixed;width:2px;height:2px;opacity:0;pointer-events:none;left:-9999px;top:-9999px;border:none;",document.body.appendChild(a);let n=!1;const o=()=>{n||(n=!0,clearTimeout(i),window.removeEventListener("message",r),a.remove(),t())},i=setTimeout(o,1e4),r=s=>{dn(s.data)&&s.data.projectId===e.id&&o()};window.addEventListener("message",r),a.addEventListener("load",()=>{setTimeout(()=>{try{a.contentWindow?.postMessage({type:"shaderops/request-preview",projectId:e.id},"*")}catch{o()}},2500)}),a.src=Uo(e)})}async function rl(){if(!l.prewarmActive){for(l.prewarmActive=!0;l.prewarmQueue.length>0;){const e=l.prewarmQueue.shift();l.previews[e.id]||await il(e)}l.prewarmActive=!1}}async function mn(e){e!==l.selectedId&&(await da(),l.selectedId=e,ye(),pn())}D.addEventListener("load",()=>{const e=l.projects.find(t=>t.id===l.selectedId);if(e){try{_o(D.contentDocument,e.id)}catch(t){console.warn("ShaderOps: unable to install project UI enhancements.",t)}ot(),setTimeout(()=>{try{D.contentWindow?.postMessage({type:"shaderops/request-preview",projectId:e.id},"*")}catch{}},2e3)}});async function ll(){const e=await fetch("/registry.json",{cache:"no-store"});if(!e.ok)throw new Error(`Unable to load registry (${e.status})`);const t=await e.json(),a=Array.isArray(t.projects)?t.projects:[],n=await Promise.all(a.map(c=>Vr(c)));l.projects=a.filter((c,u)=>n[u]),Ir();const o=new Set(l.projects.map(c=>c.id));let i=!1;Object.keys(l.previews).forEach(c=>{o.has(c)||(delete l.previews[c],i=!0)}),i&&oa();const r=S.getItem(zt),s=l.projects.find(c=>c.id===r)?r:l.projects[0]?.id??null;l.selectedId=s,ye(),pn(),l.prewarmQueue=l.projects.filter(c=>!l.previews[c.id]),l.prewarmQueue.length>0&&setTimeout(rl,3e3)}async function sl(e){if(l.projects.find(a=>a.id===e))try{l.selectedId===e&&(await da(),D.src="about:blank",await new Promise(o=>{let i=!1;const r=()=>{i||(i=!0,D.removeEventListener("load",s),clearTimeout(c),o())},s=()=>r(),c=setTimeout(r,1200);D.addEventListener("load",s,{once:!0})}));const a=await fetch(`/api/projects/${encodeURIComponent(e)}`,{method:"DELETE"}),n=await a.json().catch(()=>({}));if(!a.ok){const o=n?.detail?`: ${n.detail}`:"",i=n?.error?`${n.error}${o}`:`Delete failed (${a.status})`;throw new Error(i)}if(l.projects=l.projects.filter(o=>o.id!==e),delete l.previews[e],oa(),l.selectedId===e){const o=l.projects[0]?.id??null;l.selectedId=o,o?S.setItem(zt,o):S.removeItem(zt)}l.prewarmQueue=l.prewarmQueue.filter(o=>o.id!==e),ye(),pn()}catch(a){const n=a instanceof Error?a.message:String(a);window.alert(`Unable to delete project: ${n}`)}}oe.addEventListener("click",async e=>{const t=e.target.closest("[data-project-id]");if(!t)return;const a=t.dataset.projectId,n=Wo(a,t);await mn(a),jo(a,n),be()});oe.addEventListener("pointerover",e=>{const t=e.target.closest(".project-thumb[data-project-id]");!t||!oe.contains(t)||qo(t,t.dataset.projectId,"rail")});oe.addEventListener("pointerout",e=>{const t=e.target.closest(".project-thumb[data-project-id]");if(!t)return;const a=e.relatedTarget;a&&(t.contains(a)||re.contains(a))||(sa(),ue===t&&be())});oe.addEventListener("pointerleave",e=>{const t=e.relatedTarget;t&&re.contains(t)||be()});function $n(e){l.navPage+=e,ye()}function Xo(e){oe.classList.contains("paged")&&(e.preventDefault(),e.deltaY>0?$n(1):e.deltaY<0&&$n(-1))}oe.addEventListener("wheel",e=>{const t=e.target.closest(".project-thumb[data-project-id]");if(t&&!re.hidden&&ue===t&&ct===t.dataset.projectId&&Z.length>0){e.preventDefault();const a=Math.abs(e.deltaY)>=Math.abs(e.deltaX)?e.deltaY:e.deltaX;a!==0&&Go(a>0?1:-1);return}Xo(e)},{passive:!1});tt.addEventListener("wheel",Xo,{passive:!1});let Nn=null;window.addEventListener("resize",()=>{clearTimeout(Nn),Nn=setTimeout(ye,120),be()});Zt.addEventListener("click",e=>{e.stopPropagation(),j.hidden?nl():ua()});j.addEventListener("click",e=>{const t=e.target.closest("[data-fav-toggle]");if(t){e.stopPropagation(),al(t.dataset.favToggle);return}const a=e.target.closest(".nav-grid-thumb[data-project-id]");if(!a)return;const n=a.dataset.projectId,o=Wo(n,a);ua(),Qr(n),mn(n).then(()=>{jo(n,o)}),be()});j.addEventListener("pointerover",e=>{if(j.hidden)return;const t=e.target.closest(".nav-grid-thumb[data-project-id]");!t||!j.contains(t)||qo(t,t.dataset.projectId,"effect-panel")});j.addEventListener("pointerout",e=>{const t=e.target.closest(".nav-grid-thumb[data-project-id]");if(!t)return;const a=e.relatedTarget;a&&(t.contains(a)||re.contains(a))||(sa(),ue===t&&be())});j.addEventListener("pointerleave",e=>{const t=e.relatedTarget;t&&re.contains(t)||be()});j.addEventListener("wheel",e=>{const t=e.target.closest(".nav-grid-thumb[data-project-id]");if(!t||re.hidden||ue!==t||ct!==t.dataset.projectId||Z.length===0)return;e.preventDefault(),e.stopPropagation();const a=Math.abs(e.deltaY)>=Math.abs(e.deltaX)?e.deltaY:e.deltaX;a!==0&&Go(a>0?1:-1)},{passive:!1});Za.addEventListener("click",()=>ua());document.addEventListener("click",e=>{j.hidden||j.contains(e.target)||Zt.contains(e.target)||ua()});document.addEventListener("dragstart",()=>be());window.addEventListener("blur",()=>be());fn(Ta,e=>{l.favorites=e,Po(),Vo(),ln(),ca(),ye()});function Ko(e,t){const a=new Set(l.favorites),n=l.favorites.map(f=>l.projects.find(p=>p.id===f)).filter(Boolean),o=l.projects.filter(f=>!a.has(f.id)&&!Vt(f)),i=l.projects.filter(f=>!a.has(f.id)&&Vt(f)),r=new Map(l.projects.map(f=>[f.id,f])),s=e.map(f=>r.get(f)).filter(Boolean),c=t==="3d"?i.length:o.length;if(s.length!==c)return;const u=t==="2d"?s:o,d=t==="3d"?s:i;l.projects=[...n,...u,...d],ln(),ca(),ye()}fn(Ra,e=>Ko(e,"2d"));fn(Fa,e=>Ko(e,"3d"));Aa.addEventListener("click",()=>{if(!l.selectedId)return;const e=l.projects.find(t=>t.id===l.selectedId);Ni.textContent=e?`Delete "${e.name}"? This removes its files permanently and cannot be undone.`:"This removes its files permanently and cannot be undone.",at.hidden=!1});_i.addEventListener("click",()=>{at.hidden=!0});at.addEventListener("click",e=>{e.target===at&&(at.hidden=!0)});Ui.addEventListener("click",()=>{at.hidden=!0,l.selectedId&&sl(l.selectedId)});window.addEventListener("beforeunload",()=>{const e=D.contentWindow,t=l.projects.find(a=>a.id===l.selectedId);!e||!t||e.postMessage({type:"shaderops/request-preview",projectId:t.id},"*")});let ve=null,qa=null,Xt=0,Un=performance.now()/1e3;const Ze=document.createElement("canvas"),_n=Ze.getContext("2d",{willReadFrequently:!1});function cl(){return ve||(ve=new ui(R),ve)}let Ga=null;const et=new Map;let On=null;function ul(e,t){if(e){On!==e&&(et.clear(),On=e);for(const[a,n]of t){let o=et.get(a);o||(o=e.createElement("canvas"),o.className="fx-underlay-item",o.style.cssText="position:fixed;pointer-events:none;z-index:1;display:none;",e.body.appendChild(o),et.set(a,o)),o.style.display="block",o.style.left=`${n.x}px`,o.style.top=`${n.y}px`,o.style.width=`${n.w}px`,o.style.height=`${n.h}px`}for(const[a,n]of et)t.has(a)||(n.style.display="none")}}function dl(e,t){const a=e.width||0,n=e.height||0;if(!(a<=0||n<=0))for(const[o,i]of et){if(i.style.display==="none")continue;const r=parseFloat(i.style.width)||0,s=parseFloat(i.style.height)||0;if(r<=0||s<=0)continue;const c=Math.max(1,Math.round(r*t)),u=Math.max(1,Math.round(s*t));i.width!==c&&(i.width=c),i.height!==u&&(i.height=u);const d=Math.max(0,Math.min(a-1,Math.round(parseFloat(i.style.left)*t))),f=Math.max(0,Math.min(n-1,Math.round(parseFloat(i.style.top)*t))),p=Math.max(1,Math.min(a-d,c)),h=Math.max(1,Math.min(n-f,u)),v=i.getContext("2d");if(v)try{v.clearRect(0,0,c,u),v.drawImage(e,d,f,p,h,0,0,c,u)}catch{}}}function fl(e,t,a){const n=[],o=new Map,i=0;try{if(e)for(const f of["hud","panelToggleDock","shaderopsCheckpoints","shaderopsCheckpointMenu"]){const p=e.getElementById(f);if(!p)continue;const h=e.defaultView?.getComputedStyle(p);if(h?.display==="none"||h?.visibility==="hidden"||h?.contentVisibility==="hidden")continue;const v=p.getBoundingClientRect();if(v.width>0&&v.height>0){const m=v.left-i,g=v.top-i,C=Math.max(0,m),M=Math.max(0,g),x=Math.min(t,m+v.width+i*2),V=Math.min(a,g+v.height+i*2);x>C&&V>M&&(n.push({x:C,y:M,w:x-C,h:V-M}),o.set(f,{x:C,y:M,w:x-C,h:V-M}))}}}catch{}ul(e,o);const r=`${t}x${a}|`+n.map(f=>`${f.x},${f.y},${f.w},${f.h}`).join(";");if(r===Ga)return;if(Ga=r,R.style.clipPath="none",!n.length){R.style.maskImage="none",R.style.webkitMaskImage="none";return}const s=[];for(const f of n){let p=f;for(let h=s.length-1;h>=0;h--){const v=s[h];if(p.x<v.x+v.w&&p.x+p.w>v.x&&p.y<v.y+v.h&&p.y+p.h>v.y){const g=Math.min(p.x,v.x),C=Math.min(p.y,v.y),M=Math.max(p.x+p.w,v.x+v.w),x=Math.max(p.y+p.h,v.y+v.h);p={x:g,y:C,w:M-g,h:x-C},s.splice(h,1)}}s.push(p)}const c=s.map(f=>`<rect x="${f.x}" y="${f.y}" width="${f.w}" height="${f.h}" fill="#000"/>`).join(""),u=`<svg xmlns="http://www.w3.org/2000/svg" width="${t}" height="${a}"><defs><mask id="m" maskUnits="userSpaceOnUse" x="0" y="0" width="${t}" height="${a}"><rect x="0" y="0" width="${t}" height="${a}" fill="#fff"/><g>${c}</g></mask></defs></svg>`,d=`url("data:image/svg+xml,${encodeURIComponent(u)}#m")`;R.style.maskImage=d,R.style.webkitMaskImage=d,R.style.maskMode="luminance",R.style.maskRepeat="no-repeat",R.style.webkitMaskRepeat="no-repeat",R.style.maskSize="100% 100%",R.style.webkitMaskSize="100% 100%"}function Yo(){requestAnimationFrame(Yo);const e=performance.now()/1e3,t=Math.max(0,Math.min(.1,e-Un));Un=e;const a=l.selectedId,o=(a?G(a):[]).filter(M=>M.enabled),i=a?Fe(a):!1,r=!!a&&i&&o.length>0,s=a?q.get(a):null;a&&s?.extras?.paused||(Xt+=t);let u=null;try{u=D.contentDocument||null}catch{u=null}let d=null;try{d=u?.querySelector("canvas")||null}catch{d=null}if(!r||!d){R.style.display!=="none"&&(R.style.display="none"),(R.style.clipPath!=="none"||R.style.maskImage)&&(R.style.clipPath="none",R.style.maskImage="none",R.style.webkitMaskImage="none",Ga=null);for(const M of et.values())M.style.display="none";return}R.style.display!=="block"&&(R.style.display="block");const f=cl();d!==qa&&(qa=d,f.setSource(Ze));const p=d.width||1,h=d.height||1;(Ze.width!==p||Ze.height!==h)&&(Ze.width=p,Ze.height=h);try{_n.clearRect(0,0,p,h),_n.drawImage(d,0,0,p,h)}catch{}const v=Qa.getBoundingClientRect(),m=Math.max(1,Math.round(v.width)),g=Math.max(1,Math.round(v.height));(f.width!==m||f.height!==g)&&f.resize(m,g),fl(u,m,g);const C=[...o].reverse();f.renderFrame(C,Xt),dl(R,f.renderer.getPixelRatio())}requestAnimationFrame(Yo);Br();zr();Qt.addEventListener("click",e=>{e.stopPropagation(),J.hidden?qr():ra()});J.addEventListener("click",e=>{const t=e.target.closest(".filter-add-option");!t||!l.selectedId||(Fr(l.selectedId,t.dataset.filterId),ra())});document.addEventListener("click",e=>{const t=e.target.closest(".filter-add-wrap"),a=e.target.closest("#filterAddMenu");!J.hidden&&!t&&!a&&ra()});document.addEventListener("keydown",e=>{e.key==="Escape"&&ra()});_.addEventListener("click",e=>{const t=e.target.closest("[data-action]");if(!t||!l.selectedId)return;const{action:a,instanceId:n,paramKey:o}=t.dataset;if(a==="collapse")Ur(l.selectedId,n);else if(a==="remove")$r(l.selectedId,n);else if(a==="upload-image"){const i=t.closest(".filter-param-row").querySelector(".filter-image-input");i&&i.click()}else a==="clear-image"?(P(l.selectedId),ke(l.selectedId,n,o,null),F()):a==="open-curve"?yl(n,o,t.dataset.label,t):a==="open-gradient"&&vl(n,o,t.dataset.label,t)});let Dt=null;_.addEventListener("pointerdown",e=>{const t=e.target.closest(".filter-levels-handle[data-instance-id][data-handle]");if(!t||!l.selectedId)return;const a=t.closest(".filter-levels-graph");if(!a)return;e.preventDefault(),e.stopPropagation();const n=t.dataset.instanceId,o=`levelsgraph:${n}`;ge!==o&&(P(l.selectedId),ge=o),Dt={instanceId:n,handle:t.dataset.handle,graph:a}});_.addEventListener("change",e=>{const t=e.target;if(l.selectedId)if(t.matches('[data-action="toggle"]'))Nr(l.selectedId,t.dataset.instanceId);else if(t.classList.contains("filter-param-toggle"))P(l.selectedId),ke(l.selectedId,t.dataset.instanceId,t.dataset.paramKey,t.checked),F();else if(t.classList.contains("filter-param-select"))P(l.selectedId),ke(l.selectedId,t.dataset.instanceId,t.dataset.paramKey,t.value),Or(l.selectedId,t.dataset.instanceId,t.dataset.paramKey,t.value),F();else if(t.classList.contains("filter-param-color")){const a=`color:${t.dataset.instanceId}:${t.dataset.paramKey}`;ge!==a&&P(l.selectedId),ke(l.selectedId,t.dataset.instanceId,t.dataset.paramKey,t.value),ge=null}else if(t.classList.contains("filter-image-input")){const a=t.files&&t.files[0];if(!a)return;const{instanceId:n,paramKey:o}=t.dataset,i=new FileReader;i.onload=()=>{P(l.selectedId),ke(l.selectedId,n,o,i.result),F()},i.readAsDataURL(a)}else(t.classList.contains("filter-param-range")||t.classList.contains("filter-param-number")||t.classList.contains("filter-param-color"))&&(ge=null)});let ge=null;_.addEventListener("input",e=>{const t=e.target;if(l.selectedId){if(t.classList.contains("filter-param-range")||t.classList.contains("filter-param-number")){const{instanceId:a,paramKey:n}=t.dataset,o=Number(t.value),i=`${a}:${n}`;ge!==i&&(P(l.selectedId),ge=i),ke(l.selectedId,a,n,o,!0),_.querySelectorAll(`[data-instance-id="${a}"][data-param-key="${n}"]`).forEach(r=>{r!==t&&(r.value=o)}),cn(a)}else if(t.classList.contains("filter-param-color")){const{instanceId:a,paramKey:n}=t.dataset,o=`color:${a}:${n}`;ge!==o&&(P(l.selectedId),ge=o),ke(l.selectedId,a,n,t.value,!0)}}});window.addEventListener("pointermove",e=>{if(!Dt||!l.selectedId)return;const{instanceId:t,handle:a,graph:n}=Dt,o=n.getBoundingClientRect();if(o.width<=1)return;const i=Math.max(0,Math.min(1,(e.clientX-o.left)/o.width)),r=(d,f)=>{const p=_.querySelector(`input[data-instance-id="${t}"][data-param-key="${d}"]`),h=p?Number(p.value):f;return Number.isFinite(h)?h:f};let s=r("blackPoint",0),c=r("gamma",1),u=r("whitePoint",1);if(a==="black")s=Math.max(0,Math.min(u-.001,i)),Sa(t,"blackPoint",s);else if(a==="white")u=Math.max(s+.001,Math.min(1,i)),Sa(t,"whitePoint",u);else{const d=Math.max(0,Math.min(.99,s)),f=Math.max(d+.001,Math.min(1,u)),p=Math.max(.001,Math.min(.999,(i-d)/(f-d)));c=Math.max(.2,Math.min(3,Math.log(p)/Math.log(.5))),Sa(t,"gamma",c)}cn(t)});window.addEventListener("pointerup",()=>{Dt&&(Dt=null,ge=null)});let Kt=null,ze=null;_.addEventListener("dragstart",e=>{const t=e.target.closest(".filter-item");if(!t||!e.target.closest(".filter-name")){e.preventDefault();return}Kt=t.dataset.instanceId,ze=t,t.classList.add("dragging"),e.dataTransfer.effectAllowed="move",e.dataTransfer.setData("text/plain",Kt);const a=t.getBoundingClientRect();e.dataTransfer.setDragImage(t,e.clientX-a.left,e.clientY-a.top)});_.addEventListener("dragover",e=>{if(!ze)return;e.preventDefault(),e.dataTransfer.dropEffect="move";const t=e.target.closest(".filter-item");if(!t||t===ze){t||_.appendChild(ze);return}const a=t.getBoundingClientRect();e.clientY<a.top+a.height/2?t.parentNode.insertBefore(ze,t):t.parentNode.insertBefore(ze,t.nextSibling)});_.addEventListener("drop",e=>{if(e.preventDefault(),!l.selectedId||!Kt)return;const t=G(l.selectedId),a=[..._.querySelectorAll(".filter-item")].map(i=>i.dataset.instanceId),n=new Map(t.map(i=>[i.instanceId,i])),o=a.map(i=>n.get(i)).filter(Boolean);o.length===t.length&&(o.some((r,s)=>r!==t[s])&&P(l.selectedId),t.length=0,t.push(...o),pe()),F()});_.addEventListener("dragend",()=>{Kt=null,ze=null,_.querySelectorAll(".filter-item.dragging, .filter-item.drag-over").forEach(e=>{e.classList.remove("dragging","drag-over")})});const pl={w:320,h:240};function ml(){try{const e=S.getItem(ao),t=e?JSON.parse(e):null;if(t&&typeof t.w=="number"&&typeof t.h=="number")return t}catch{}return{...pl}}let Pe=ml();function hl(){S.setItem(ao,JSON.stringify(Pe))}const E=document.createElement("div");E.id="curvePopup";E.className="curve-popup";E.hidden=!0;E.innerHTML=`
...
`;document.body.appendChild(ie);const L=document.createElement("div");L.id="gradientPopup";L.className="curve-popup gradient-popup";L.hidden=!0;L.innerHTML=`
  <div class="curve-popup-header">
    <span class="curve-popup-title"></span>
    <div class="curve-popup-header-actions">
...
`;document.body.appendChild(L);let O=null,it=null,$=null;function hn(e){return Math.min(1,Math.max(0,e))}function vn(){return!O||!l.selectedId?null:G(l.selectedId).find(t=>t.instanceId===O.instanceId)||null}function gt(){const e=vn();return!e||!O?null:pt(e.params[O.paramKey])}function yt(e){const t=vn();!t||!O||(t.params[O.paramKey]={stops:e},pe())}function Jo(){if(L.hidden||!O||!O.trigger)return;const e=O.trigger.getBoundingClientRect(),t=L.offsetWidth||300,a=L.offsetHeight||140;let n=e.right-t,o=e.top-a-10;o<8&&(o=e.bottom+10),n=Math.min(Math.max(8,n),window.innerWidth-t-8),o=Math.min(Math.max(8,o),window.innerHeight-a-8),L.style.left=`${n}px`,L.style.top=`${o}px`}function Ce(){const e=gt(),t=L.querySelector(".gradient-popup-track"),a=L.querySelector(".gradient-popup-editor");if(!(!e||!t))if($!==null&&($<0||$>=e.stops.length)&&($=null),t.style.background=Gn(e),t.innerHTML=e.stops.map((n,o)=>`<button type="button" class="gradient-popup-handle${o===$?" active":""}"
      style="left:${(n.t*100).toFixed(2)}%; background:${n.color}" data-stop-index="${o}" title="${n.color}"></button>`).join(""),$!==null){const n=e.stops[$];a.hidden=!1,a.querySelector(".gradient-popup-color").value=n.color,a.querySelector(".gradient-popup-pos").value=Math.round(n.t*100),a.querySelector(".gradient-popup-delete").disabled=e.stops.length<=2}else a.hidden=!0}function vl(e,t,a,n){O={instanceId:e,paramKey:t,trigger:n},$=null,L.querySelector(".curve-popup-title").textContent=a||"Gradient",L.hidden=!1,Jo(),Ce()}function fa(){L.hidden=!0,O=null,it=null,$=null}function gl(){const e=vn();if(!e||!O)return;const t=Ae(e.defId),a=t&&t.params.find(o=>o.key===O.paramKey);if(!a)return;const n=pt(a.default);P(l.selectedId),$=null,yt(n.stops.map(o=>({...o}))),Ce(),F()}L.querySelector(".gradient-popup-close").addEventListener("click",fa);L.querySelector(".gradient-popup-reset").addEventListener("click",gl);L.querySelector(".gradient-popup-bar").addEventListener("pointerdown",e=>{const t=e.target.closest(".gradient-popup-handle");t&&($=Number(t.dataset.stopIndex),it=$,P(l.selectedId),Ce(),e.preventDefault())});L.querySelector(".gradient-popup-bar").addEventListener("dblclick",e=>{if(e.target.closest(".gradient-popup-handle"))return;const t=gt(),a=e.currentTarget;if(!t)return;const n=a.getBoundingClientRect(),o=hn((e.clientX-n.left)/n.width),i=ri(t,o),r=t.stops.map(c=>({...c})),s={t:o,color:i};r.push(s),r.sort((c,u)=>c.t-u.t),$=r.indexOf(s),P(l.selectedId),yt(r),Ce(),F()});document.addEventListener("pointermove",e=>{if(it===null)return;const t=L.querySelector(".gradient-popup-bar"),a=gt();if(!t||!a)return;const n=t.getBoundingClientRect(),o=hn((e.clientX-n.left)/n.width),i=a.stops.map(s=>({...s})),r=i[it];r&&(r.t=o,i.sort((s,c)=>s.t-c.t),$=i.indexOf(r),it=$,yt(i),Ce(),F())});document.addEventListener("pointerup",()=>{it=null});L.querySelector(".gradient-popup-color").addEventListener("input",e=>{const t=gt();if(!t||$===null)return;const a=t.stops.map(n=>({...n}));a[$].color=e.target.value,yt(a),Ce(),F()});L.querySelector(".gradient-popup-color").addEventListener("change",()=>{P(l.selectedId)});L.querySelector(".gradient-popup-pos").addEventListener("change",e=>{const t=gt();if(!t||$===null)return;const a=t.stops.map(o=>({...o})),n=a[$];P(l.selectedId),n.t=hn(Number(e.target.value)/100),a.sort((o,i)=>o.t-i.t),$=a.indexOf(n),yt(a),Ce(),F()});L.querySelector(".gradient-popup-delete").addEventListener("click",()=>{const e=gt();if(!e||$===null||e.stops.length<=2)return;const t=e.stops.filter((a,n)=>n!==$);P(l.selectedId),$=null,yt(t),Ce(),F()});let z=null,ut=null,dt=null,Me=null,rt=null,W=null;function gn(){return!z||!l.selectedId?null:G(l.selectedId).find(t=>t.instanceId===z.instanceId)||null}function je(){const e=gn();return e?It(e.params[z.paramKey]):null}function ft(e){const t=gn();!t||!z||(t.params[z.paramKey]={points:e},pe())}function yl(e,t,a,n){z={instanceId:e,paramKey:t,trigger:n},W=null,E.querySelector(".curve-popup-title").textContent=a||"Curve",E.style.width=`${Pe.w}px`,E.style.height=`${Pe.h}px`,E.hidden=!1,yn(),de()}function pa(){E.hidden=!0,z=null,ut=null,dt=null,W=null,Mt()}function bl(){const e=gn();if(!e||!z)return;const t=Ae(e.defId),a=t&&t.params.find(i=>i.key===z.paramKey);if(!a)return;const o=It(a.default).points.map(i=>({...i}));W=null,P(l.selectedId),ft(o),de(),F()}function yn(){if(E.hidden||!z||!z.trigger)return;const e=z.trigger.getBoundingClientRect(),t=E.offsetWidth||Pe.w,a=E.offsetHeight||Pe.h;let n=e.right-t,o=e.top-a-10;o<8&&(o=e.bottom+10),n=Math.min(Math.max(8,n),window.innerWidth-t-8),o=Math.min(Math.max(8,o),window.innerHeight-a-8),E.style.left=`${n}px`,E.style.top=`${o}px`}function de(){const e=je(),t=E.querySelector(".curve-popup-svg");if(!e||!t)return;const a=t.getBoundingClientRect(),n=Math.max(1,Math.round(a.width)||100),o=Math.max(1,Math.round(a.height)||100);t.setAttribute("viewBox",`0 0 ${n} ${o}`),W!==null&&(W<0||W>=e.points.length-1)&&(W=null);const i=Fo(e,n,o,48);let r="";for(let p=1;p<4;p++){const h=p/4*n,v=p/4*o;r+=`<line class="curve-grid-line" x1="0" y1="${v.toFixed(2)}" x2="${n}" y2="${v.toFixed(2)}" />`,r+=`<line class="curve-grid-line" x1="${h.toFixed(2)}" y1="0" x2="${h.toFixed(2)}" y2="${o}" />`}const s=Math.min(n,o),c=Math.max(4.5,s*.03),u=Math.max(9,s*.05);let d="";for(let p=0;p<e.points.length-1;p++){const h=e.points[p];if(h.type==="constant"||p!==W)continue;const v=e.points[p+1],m=h.x*n,g=(1-h.y)*o,C=Math.max(v.x-h.x,1e-5),M=h.tanSet?h.tanDx:C/3,x=h.tanSet?h.tanDy:0;let V=M*n,le=-x*o;const Q=Math.hypot(V,le),se=Math.max(u*1.1,12);if(!h.tanSet)if(Q>1e-4&&Q<se){const I=se/Q;V*=I,le*=I}else Q<=1e-4&&(V=se,le=0);const bt=m+V,Ye=g+le,b=m-V,A=g-le;d+=`<line class="curve-handle-line" x1="${bt.toFixed(2)}" y1="${Ye.toFixed(2)}" x2="${b.toFixed(2)}" y2="${A.toFixed(2)}" />`;const N=u/2,k=(I,Y)=>`M ${I.toFixed(2)} ${(Y-N).toFixed(2)} L ${(I+N).toFixed(2)} ${Y.toFixed(2)} L ${I.toFixed(2)} ${(Y+N).toFixed(2)} L ${(I-N).toFixed(2)} ${Y.toFixed(2)} Z`;d+=`<path class="curve-handle-point" data-handle-index="${p}" d="${k(bt,Ye)}" />`,d+=`<path class="curve-handle-point" data-handle-index="${p}" d="${k(b,A)}" />`}const f=e.points.map((p,h)=>{const v=h===0||h===e.points.length-1,m=h===W,g=["curve-point",v?"curve-point--endpoint":"",p.type==="constant"?"curve-point--constant":"",m?"curve-point--active":""].filter(Boolean).join(" "),C=p.x*n,M=(1-p.y)*o;if(m){const x=c*1.7;return`<rect class="${g}" data-point-index="${h}" x="${(C-x/2).toFixed(2)}" y="${(M-x/2).toFixed(2)}" width="${x.toFixed(2)}" height="${x.toFixed(2)}" />`}return`<circle class="${g}" data-point-index="${h}" cx="${C.toFixed(2)}" cy="${M.toFixed(2)}" r="${c.toFixed(2)}" />`}).join("");t.innerHTML=`${r}${d}<path class="curve-popup-path" d="${i}" />${f}`}E.querySelector(".curve-popup-close").addEventListener("click",pa);E.querySelector(".curve-popup-reset").addEventListener("click",bl);E.querySelector(".curve-popup-svg").addEventListener("pointerdown",e=>{const t=e.currentTarget,a=e.target.closest(".curve-handle-point"),n=e.target.closest(".curve-point"),o=je();if(o){if(a){const i=Number(a.dataset.handleIndex);P(l.selectedId),dt={index:i},W=i,t.setPointerCapture(e.pointerId),e.stopPropagation();return}if(n){const i=Number(n.dataset.pointIndex);P(l.selectedId),ut={index:i},i<o.points.length-1&&o.points[i].type!=="constant"?W=i:W=null,t.setPointerCapture(e.pointerId),e.stopPropagation(),de();return}W!==null&&(W=null,de())}});E.querySelector(".curve-popup-svg").addEventListener("dblclick",e=>{const t=e.target.closest(".curve-point"),a=je();if(!a)return;if(e.preventDefault(),t){const u=Number(t.dataset.pointIndex),d=a.points.map(f=>({...f}));d[u].type=d[u].type==="constant"?"bezier":"constant",P(l.selectedId),ft(d),de(),F();return}if(e.target.closest(".curve-handle-point"))return;const o=E.querySelector(".curve-popup-svg").getBoundingClientRect(),i=Math.min(1,Math.max(0,(e.clientX-o.left)/o.width)),r=Math.min(1,Math.max(0,1-(e.clientY-o.top)/o.height)),s=a.points.map(u=>({...u}));s.push({x:i,y:r,type:"bezier",tanDx:0,tanDy:0,tanSet:!1}),s.sort((u,d)=>u.x-d.x),P(l.selectedId),ft(s);const c=s.findIndex(u=>u.x===i&&u.y===r);W=c<s.length-1?c:null,de(),F()});E.querySelector(".curve-popup-svg").addEventListener("contextmenu",e=>{const t=e.target.closest(".curve-point");if(!t)return;e.preventDefault(),rt=Number(t.dataset.pointIndex);const a=je(),n=a&&(rt===0||rt===a.points.length-1);ie.querySelector('[data-action="delete"]').disabled=!!n,ie.style.left=`${e.clientX}px`,ie.style.top=`${e.clientY}px`,ie.hidden=!1});function Mt(){ie.hidden=!0,rt=null}ie.addEventListener("click",e=>{const t=e.target.closest("button[data-action]");if(!t||t.disabled||rt===null){Mt();return}const a=je();if(a){const n=a.points.map(r=>({...r})),o=rt,i=o===0||o===n.length-1;t.dataset.action==="type-bezier"?n[o].type="bezier":t.dataset.action==="type-constant"?n[o].type="constant":t.dataset.action==="delete"&&!i&&n.length>2&&n.splice(o,1),P(l.selectedId),ft(n),de(),F()}Mt()});E.querySelector(".curve-popup-resize").addEventListener("pointerdown",e=>{Me={startX:e.clientX,startY:e.clientY,startW:Pe.w,startH:Pe.h},e.target.setPointerCapture(e.pointerId),e.preventDefault()});document.addEventListener("pointermove",e=>{if(ut){const t=je(),a=E.querySelector(".curve-popup-svg");if(!t||!a)return;const n=a.getBoundingClientRect(),o=t.points.map(u=>({...u})),i=ut.index,r=i===0||i===o.length-1,s=Math.min(1,Math.max(0,1-(e.clientY-n.top)/n.height));let c=o[i].x;if(!r){const u=o[i-1].x+.001,d=o[i+1].x-.001;c=Math.min(d,Math.max(u,(e.clientX-n.left)/n.width))}o[i]={...o[i],x:c,y:s},ft(o),de(),F()}else if(dt){const t=je(),a=E.querySelector(".curve-popup-svg");if(!t||!a)return;const n=a.getBoundingClientRect(),o=t.points.map(x=>({...x})),i=dt.index,r=o[i],s=o[i+1];if(!r||!s)return;const c=n.width,u=n.height,d=r.x*c,f=(1-r.y)*u,p=e.clientX-n.left,h=e.clientY-n.top;let v=(p-d)/c,m=-(h-f)/u;const C=Math.max(.001,Math.hypot(s.x-r.x,s.y-r.y))*1.5,M=Math.hypot(v,m);if(M>C){const x=C/M;v*=x,m*=x}o[i]={...r,tanDx:v,tanDy:m,tanSet:!0},ft(o),de(),F()}else if(Me){const t=Math.max(240,Math.min(720,Me.startW+(e.clientX-Me.startX))),a=Math.max(160,Math.min(560,Me.startH+(e.clientY-Me.startY)));Pe={w:t,h:a},E.style.width=`${t}px`,E.style.height=`${a}px`,de()}});document.addEventListener("pointerup",()=>{ut&&(ut=null),dt&&(dt=null),Me&&(Me=null,hl())});document.addEventListener("click",e=>{!E.hidden&&!e.target.closest("#curvePopup")&&!e.target.closest('[data-action="open-curve"]')&&!e.target.closest("#curvePointMenu")&&pa(),!ie.hidden&&!e.target.closest("#curvePointMenu")&&Mt(),!L.hidden&&!e.target.closest("#gradientPopup")&&!e.target.closest('[data-action="open-gradient"]')&&fa()});document.addEventListener("keydown",e=>{e.key==="Escape"&&(pa(),Mt(),fa())});document.addEventListener("keydown",e=>{const t=e.key.toLowerCase();if(!(e.ctrlKey||e.metaKey)||t!=="z"||!l.selectedId)return;const a=e.target;a&&(a.tagName==="INPUT"||a.tagName==="TEXTAREA")&&a.type==="text"||(e.preventDefault(),e.shiftKey?Rr(l.selectedId):Tr(l.selectedId))});window.addEventListener("resize",yn);Ht.addEventListener("change",()=>{l.selectedId&&(l.filterPreviewOn[l.selectedId]=Ht.checked,pe(),F())});function Zo(){return!ve||!l.selectedId||R.style.display==="none"||!Fe(l.selectedId)?!1:G(l.selectedId).some(e=>e.enabled)}function bn(e){if(Zo()){const c=[...G(l.selectedId).filter(u=>u.enabled)].reverse();return ve.captureAtScale(c,Xt,e)}const t=D.contentDocument,a=t?t.querySelector("canvas"):null;if(!a||!a.width||!a.height)return null;const n=Math.round(a.width*e),o=Math.round(a.height*e),i=document.createElement("canvas");return i.width=n,i.height=o,i.getContext("2d").drawImage(a,0,0,n,o),{dataUrl:i.toDataURL("image/png"),width:n,height:o}}function xl(e,t){const a=URL.createObjectURL(e),n=document.createElement("a");n.href=a,n.download=t,n.click(),setTimeout(()=>URL.revokeObjectURL(a),1e3)}function Qo(e,t){const a=pr(e),n=br(t.dataUrl);if(!n)return console.warn("Failed to read exported PNG data for metadata embedding."),null;let o;try{const i=wr(n,no,JSON.stringify(a));o=new Blob([i],{type:"image/png"})}catch(i){console.warn("Failed to embed state metadata into PNG export; downloading raw image.",i),o=new Blob([n],{type:"image/png"})}return o}function wl(e,t,a){const n=Qo(e,t);return n?(xl(n,a),!0):!1}function Yt(e,t,a,n){const o=Number(e);return Number.isFinite(o)?Math.min(a,Math.max(t,o)):n}function ei(e){return new Promise(t=>setTimeout(t,e))}async function kl(){if(typeof window.showDirectoryPicker!="function")throw new Error("Directory picker is not supported in this runtime.");return window.showDirectoryPicker({mode:"readwrite"})}async function Sl(e,t,a){const o=await(await e.getFileHandle(t,{create:!0})).createWritable();await o.write(a),await o.close()}async function Cl(e=140){await new Promise(t=>requestAnimationFrame(()=>requestAnimationFrame(t))),e>0&&await ei(e)}let lt=!1;const ma=new Map;function ne(e,t="neutral"){In.textContent=e||"",In.dataset.tone=t}function zn(e){const t=Math.floor(Yt(e,1,200,1));K.value=String(t)}function Hn(e){const t=Yt(e,.1,4,1);He.value=String(Number(t.toFixed(1)))}function ha(e){const t=e?ma.get(e):null;Bn.textContent=t?t.name:"Not set",Bn.title=t?t.name:"No folder selected",tn.disabled=!t}function El(){const e=Number(K.value);K.dataset.prevValue=String(Number.isFinite(e)?e:8),K.addEventListener("input",()=>{const t=Number(K.dataset.prevValue||"8"),a=Number(K.value);if(!Number.isFinite(a))return;let n=a;Number.isFinite(t)&&Math.abs(a-(t+1))<1e-9?n=t*2:Number.isFinite(t)&&Math.abs(a-(t-1))<1e-9&&(n=t/2),zn(n),K.dataset.prevValue=K.value}),K.addEventListener("change",()=>{zn(Number(K.value||8)),K.dataset.prevValue=K.value}),He.addEventListener("input",()=>{Hn(Number(He.value||1))}),He.addEventListener("change",()=>{Hn(Number(He.value||1))})}oo.addEventListener("click",()=>{l.selectedId&&Ml()});me.addEventListener("click",async()=>{if(!l.selectedId||me.disabled)return;const e=bn(1);if(e)try{const t=await(await fetch(e.dataUrl)).blob();await navigator.clipboard.write([new ClipboardItem({"image/png":t})]),Dl()}catch(t){console.warn("[filter] copy to clipboard failed:",t)}});function Dl(){const e=me.dataset.baseTitle||me.title;me.dataset.baseTitle=e,me.classList.add("copied"),me.title="Copied!",setTimeout(()=>{me.classList.remove("copied"),me.title=e},1200)}function Ml(){let e,t;if(Zo())e=ve.width,t=ve.height;else{const a=D.contentDocument,n=a?a.querySelector("canvas"):null;e=n?n.width:R.width,t=n?n.height:R.height}Ri.textContent=`${Math.round(e)} × ${Math.round(t)} px`,ne(""),Rt.disabled=!1,ha(l.selectedId),qt.hidden=!1}function Jt(){lt||(qt.hidden=!0)}Na.addEventListener("click",e=>{const t=e.target.closest(".export-size-option");if(!t||!l.selectedId)return;const a=Number(t.dataset.scale)||1,n=l.selectedId,o=bn(a);if(!o){Jt();return}const i=Bo(n);wl(n,o,i),Jt()});Rt.addEventListener("click",async()=>{if(!l.selectedId||lt)return;const e=l.selectedId,t=Math.floor(Yt(K.value,1,200,1)),a=Yt(He.value,.1,4,1),n=$i.value==="seed"?"seed":"all-random";K.value=String(t),He.value=String(Number(a.toFixed(2)));const o=ma.get(e);if(!o){ne("Please set Batch Path first.","warn");return}lt=!0,Rt.disabled=!0,Ua.disabled=!0,tn.disabled=!0,Na.querySelectorAll(".export-size-option").forEach(s=>{s.disabled=!0}),ne(`Preparing batch export (${t})â€¦`);let i=0,r=0;try{for(let s=0;s<t;s++){if(l.selectedId!==e){ne("Batch stopped: selected effect changed.","warn");break}ne(`Rendering ${s+1}/${t}â€¦`),n==="all-random"?aa(e):Re(e,"rerollSeed"),await Ji(e,1800),await Cl();const c=bn(a);if(!c){r+=1;continue}const u=`batch-${String(s+1).padStart(3,"0")}`,d=Bo(e,u),f=Qo(e,c);if(!f)r+=1;else try{await Sl(o,d,f),i+=1}catch(p){console.warn("Batch export write failed:",p),r+=1}await ei(60)}}finally{lt=!1,Rt.disabled=!1,Ua.disabled=!1,ha(l.selectedId),Na.querySelectorAll(".export-size-option").forEach(c=>{c.disabled=!1});const s=r>0?"warn":"ok";ne(`Done: ${i} exported${r?`, ${r} failed`:""}.`,s)}});Ua.addEventListener("click",async()=>{const e=l.selectedId;if(!(!e||lt)){ne("Selecting folderâ€¦");try{const t=await kl();ma.set(e,t),ha(e),ne("Batch path set.","ok")}catch(t){console.warn("Batch export folder selection failed:",t);const a=t&&typeof t=="object"&&"name"in t?String(t.name):"";if(a==="AbortError"){ne("Folder selection cancelled.","warn");return}if(a==="NotAllowedError"||a==="SecurityError"){ne("Folder access was blocked. Please allow folder access and try again.","warn");return}const n=t instanceof Error&&t.message?t.message:"Unknown error";ne(`Failed to set folder: ${n}`,"warn")}}});tn.addEventListener("click",()=>{const e=l.selectedId;!e||lt||(ma.delete(e),ha(e),ne("Batch path cleared."))});El();Fi.addEventListener("click",Jt);qt.addEventListener("click",e=>{e.target===qt&&Jt()});Qa.addEventListener("dragover",e=>{(e.dataTransfer?.types?Array.from(e.dataTransfer.types):[]).includes("Files")&&(e.preventDefault(),e.dataTransfer.dropEffect="copy")});Qa.addEventListener("drop",async e=>{if(!e.dataTransfer?.files?.length)return;e.preventDefault();const t=[...e.dataTransfer.files].find(a=>/image\/png/i.test(a.type)||/\.png$/i.test(a.name));if(t)try{await Er(t)||console.info("Dropped PNG has no embedded ShaderOps state metadata.")}catch(a){console.warn("Failed to restore ShaderOps state from dropped PNG:",a)}});Di.addEventListener("click",()=>{en.classList.add("collapsed"),S.setItem(Ya,"1")});Ei.addEventListener("click",()=>{en.classList.remove("collapsed"),S.setItem(Ya,"0")});Dr();Mr();Ar();ll().catch(e=>{La.textContent="Registry load failed",Pa.textContent=e.message});
