const PROJECT_ID = new URLSearchParams(location.search).get("project") || "divfree-height-streamlines";
...
const schema = [
  ...
  { id: "imageMix", label: "Image Color Mix", group: "Stroke", min: 0.0, max: 1.0, step: 0.01, default: PRESET.imageMix },
  { id: "imagePrecision", label: "Image Precision", group: "Stroke", min: 0.0, max: 1.0, step: 0.01, default: PRESET.imagePrecision, hint: "Image color sampling rate — 0: one solid color per stroke, 1: resampled at every step" },
...
function drawPolylineSolid(points, color, width, alpha, blur) {
  ...
  trailsLayer.noFill();
  trailsLayer.strokeCap(cap);
  trailsLayer.strokeJoin(join);

  if (blur > 0.001) {
    trailsLayer.stroke(color[0], color[1], color[2], alpha * (0.24 + blur * 0.34));
    trailsLayer.strokeWeight(width * (1 + blur * 2.2));
    trailsLayer.beginShape();
    for (let i = 0; i < points.length; i++) {
      trailsLayer.vertex(fieldCenterX + points[i].x, fieldCenterY + points[i].y);
    }
    trailsLayer.endShape();
  }

  trailsLayer.stroke(color[0], color[1], color[2], alpha);
...
function drawSegment(p0, p1, color, width, alpha, blur) {
  ...
  if (blur > 0.001) {
    trailsLayer.stroke(color[0], color[1], color[2], alpha * (0.22 + blur * 0.32));
    trailsLayer.strokeWeight(width * (1 + blur * 2.0));
    trailsLayer.line(x1, y1, x2, y2);
  }
...
function traceDirection(seedX, seedY, sign, id, baseColor, width, clearance) {
  ...
  const useImage = imageState.loaded && params.imageMix > 0.001;
  const varied = params.colorMode > 0.5 || params.colorJitter > 0.001 || useImage;
  ...
  // precision=0: sample once at the first point, use that colour for the whole stroke
  // precision=1: resample at every step (highest detail)
  const imageInterval = precision < 0.001
    ? maxSteps + 1  // effectively never re-samples after the first
    : Math.max(1, Math.round((1 - precision) * (maxSteps - 1) + 1));
  let sampledColor = baseColor;
  ...
  for (let i = 1; i < points.length; i++) {
    ...
    if (useImage && (i % imageInterval === 0 || i === 1)) {
      const picked = sampleImageColorWorld(points[i].x, points[i].y);
      if (picked) sampledColor = picked;
    }
    if (useImage) c = mixColor(c, sampledColor, params.imageMix);
...
document.title = "Divergence-Free Height Streamlines";
