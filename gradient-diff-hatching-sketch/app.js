const PROJECT_ID = new URLSearchParams(location.search).get("project") || "gradient-diff-hatching-sketch";
...
function flowVectorUv(u, v) {
  const eps = clamp(params.gradientStep, 0.0005, 0.02);
  const hx1 = heightField(u + eps, v);
  const hx2 = heightField(u - eps, v);
  const hy1 = heightField(u, v + eps);
  const hy2 = heightField(u, v - eps);
...
function traceDirection(x0, y0, dirSign, owner, minThreshold, maxThreshold, width, clearance) {
...
function drawCluster(anchor, clusterIndex, layerIndex) {
  ...
  const [vx, vy] = flowVectorUv(baseU, baseV);
  const nx = -vy;
  const ny = vx;
  ...
  for (let j = 0; j < lanes; j++) {
    ...
    const a = traceDirection(sx, sy, -1, owner, anchor.minThreshold, anchor.maxThreshold, anchor.width, clearance);
    const b = traceDirection(sx, sy, 1, owner, anchor.minThreshold, anchor.maxThreshold, anchor.width, clearance);
    if (a || b) drawn += 1;
  }
  return drawn;
}

function buildAnchorsForBand(minThreshold, maxThreshold, density, spacing, width, bandIndex) {
  const anchors = [];
  ...
  for (let y = step * 0.5; y < pInst.height; y += step) {
    for (let x = step * 0.5; x < pInst.width; x += step) {
      ...
      anchors.push({ x: ax, y: ay, minThreshold, maxThreshold, spacing, width });
    }
  }
  return anchors;
}
...
function beginGeneration() {
  ...
  for (let i = 0; i < bands; i++) {
    ...
    const bandAnchors = buildAnchorsForBand(d0, maxThreshold, density, spacing, width, i);
...
document.title = "Gradient Diff Hatching Sketch";
