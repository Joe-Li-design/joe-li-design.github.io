...
const PROJECT_ID = (() => {
  ...
  return p || "vfx-dragonfire-eruption";
})();
const EFFECT_MODE = 1;
...
const controlSchema = [
  ...
  { id: "p1", label: "FlameH", group: "Effect", min: 0.4, max: 1.8, step: 0.01, default: REFERENCE_PRESET.p1 },
  { id: "p2", label: "Core", group: "Effect", min: 0.4, max: 1.8, step: 0.01, default: REFERENCE_PRESET.p2 },
  { id: "p3", label: "Ash", group: "Effect", min: 0.4, max: 1.8, step: 0.01, default: REFERENCE_PRESET.p3 },
  { id: "p4", label: "Jet", group: "Effect", min: 0.4, max: 1.8, step: 0.01, default: REFERENCE_PRESET.p4 },
  { id: "noiseMode", label: "Noise Mode", group: "Effect", min: 0, max: 5, step: 1, default: REFERENCE_PRESET.noiseMode },
  { id: "noiseScale", label: "Noise Scale", group: "Effect", min: 0.35, max: 1.8, step: 0.01, default: REFERENCE_PRESET.noiseScale },
];
const params = {};
...
const uniforms = {
  ...
  u_noiseMode: { value: params.noiseMode },
...
const material = new THREE.ShaderMaterial({
  ...
  fragmentShader: `
    ...
    float fbm(vec2 p){
      float f=0.0, a=0.55;
      for(int i=0;i<4;i++){ f += a*noise(p); p = p*2.03 + vec2(17.1,29.7); a*=0.52; }
      return f;
    }
    float ridgedFbm(vec2 p){
      ...
      for(int i=0;i<4;i++){
        float n = noise(p);
        f += a * (1.0 - abs(n*2.0-1.0));
        p = p*2.17 + vec2(11.8, 37.6);
        a *= 0.55;
      }
      return f;
    }
    float turbulence(vec2 p){
      ...
      float a = 0.6;
      for(int i=0;i<4;i++){
        f += a * abs(noise(p)*2.0-1.0);
        p = p*2.11 + vec2(41.2, 9.3);
        a *= 0.54;
      }
      return f;
    }
    float voroLike(vec2 p){
      vec2 i = floor(p);
      vec2 f = fract(p);
      float md = 10.0;
      for(int y=-1; y<=1; y++){
        for(int x=-1; x<=1; x++){
          ...
          vec2 r = g + vec2(hash21(i + g), hash21(i + g + 19.17)) - f;
          md = min(md, dot(r, r));
        }
      }
      return clamp(1.0 - sqrt(md), 0.0, 1.0);
    }
    float flowNoiseByMode(vec2 p, float mode){
      float m = floor(clamp(mode, 0.0, 5.0) + 0.5);
      float n = fbm(p);
      if(m < 0.5){
        n = fbm(p);
      } else if(m < 1.5){
        ...
        n = fbm(p + vec2(ridgedFbm(p*1.3), turbulence(p*1.2))*0.8);
      } else if(m < 4.5){
        // Replace expensive cellular path with lighter ridge/turb mix.
        n = mix(ridgedFbm(p*1.2), turbulence(p*1.05), 0.4);
      } else {
        n = 0.5 + 0.5*sin(p.x*2.7 + fbm(p*1.25)*2.3 + turbulence(p*0.85)*1.4);
      }
      return n;
    }
    vec3 hsv2rgb(vec3 c){
    ...
    vec3 pal(float t){
      ...
      float drift = (u_time * 0.11 + sin(u_time * 0.37) * 0.15) * u_hueDrift;
      float hue = fract(u_hue/360.0 + drift + t*(0.08 + 0.18*richness));
      float sat = clamp(u_saturation * (0.76 + 0.42*richness), 0.3, 1.55);
    ...
    void main(){
      ...
      if(EFFECT_MODE == 0){
        ...
        vec2 q = uv;
        float noiseMode = floor(clamp(u_noiseMode, 0.0, 5.0) + 0.5);
        float ns = clamp(u_noiseScale, 0.35, 1.25);
        float rise = smoothstep(1.18, -0.82, q.y);
        float taperT = clamp((q.y + 0.92) / 1.95, 0.0, 1.0);
        float taper = mix(1.05 + 0.34*u_p1, 0.08 + 0.03*u_p2, taperT);
        float sway = (flowNoiseByMode(vec2(q.y*1.55*ns - t*0.18, t*0.09 + q.y*0.2*ns), noiseMode) - 0.5) * 0.58 * u_p4;
        q.x += sway;
        vec2 warpA = vec2(
          flowNoiseByMode(q*(2.2*ns) + vec2(0.0, -t*0.42*u_p4), noiseMode),
          flowNoiseByMode(q*(2.5*ns) + vec2(0.0,  t*0.35*u_p2), mod(noiseMode + 1.0, 6.0))
        ) - 0.5;
        vec2 flow = q + warpA * (0.22 + 0.08*u_p3);
        float plumeMask = smoothstep(taper, taper*0.18, abs(flow.x)) * rise;
        float body = flowNoiseByMode(vec2(
          flow.x*(2.1 + 1.2*u_p1)*ns + warpA.y*1.1,
          flow.y*(4.3 + 1.4*u_p2)*ns - t*(1.0 + 0.42*u_p4)
        ), noiseMode);
        float tongues = flowNoiseByMode(vec2(
          flow.x*(5.3 + 2.2*u_p4)*ns + body*2.0,
          flow.y*(8.1 + 1.6*u_p1)*ns - t*(1.85 + 0.55*u_p2)
        ), mod(noiseMode + 2.0, 6.0));
        float strata = 0.5 + 0.5*sin(
          flow.y*(18.0 + 6.0*u_p1)*ns -
          flow.x*(7.0 + 4.5*u_p4)*ns +
          body*4.6 +
          t*(2.1 + 0.55*u_p3)
        );
        float core = exp(-abs(flow.x)*(8.0 + 8.5*u_p2))*smoothstep(0.96, -0.42, q.y);
        float shock = exp(-abs(flow.y + 0.08 - 0.14*sin(t*1.4 + body*4.4))*(10.0 + 17.0*u_p2));
        float ash = smoothstep(0.56, 0.88, flowNoiseByMode(vec2(
          flow.x*15.0*ns + tongues*5.0,
          flow.y*13.0*ns - t*0.92*u_p3 + body*4.0
        ), mod(noiseMode + 1.0, 4.0)));
        float ember = smoothstep(0.82, 0.97, flowNoiseByMode(vec2(
          flow.x*27.0*ns - t*2.4*(0.7 + u_p4*0.3),
          flow.y*21.0*ns + body*7.5 + tongues*3.0
        ), mod(noiseMode + 2.0, 4.0)));
        float combustion = mix(body, tongues, 0.58) * (0.62 + 0.38*strata);
        float carve = 1.0 - ash * (0.20 + 0.18*u_p3);
        v = plumeMask * (combustion*1.02 + core*0.92 + shock*0.24) * carve;
        v += ember * core * plumeMask * 0.24;
        float richness = clamp(u_hueRichness, 0.2, 2.2);
        float hueNoise = (fbm(vec2(flow.x*3.4*ns + body*2.1, flow.y*5.8*ns - t*0.16 + tongues*1.7)) - 0.5)
          * (0.16 + 0.18*richness);
        float hueField = body*0.28 + tongues*0.34 + strata*0.22 + shock*0.10 + hueNoise;
        vec3 cLow = mix(vec3(0.01, 0.11, 0.20), pal(0.01 + hueField), 0.45);
        vec3 cMid = mix(vec3(0.03, 0.34, 0.53), pal(0.15 + hueField*1.08 + ember*0.10), 0.55);
        vec3 cHot = mix(vec3(0.18, 0.73, 0.88), pal(0.31 + core*0.18 + ember*0.22 + hueField*0.9), 0.62);
        vec3 flameCol = mix(cLow, cMid, clamp(0.25 + tongues*0.55 + strata*0.18, 0.0, 1.0));
        flameCol = mix(flameCol, cHot, clamp(core*0.65 + ember*0.55, 0.0, 1.0));
        flameCol -= ash * vec3(0.01, 0.02, 0.03) * plumeMask * (0.34 + 0.22*u_p3);
        col = flameCol * v;
        col += cHot * core * plumeMask * (0.18 + 0.12*ember);
      ...
      float edgeBlend = smoothstep(1.9, 0.42, length(uv));
      vec3 oceanBg = mix(
        vec3(0.015, 0.08, 0.14),
        vec3(0.02, 0.22, 0.33),
        clamp((1.0 - length(uv)*0.68) + 0.08*sin(t*0.22), 0.0, 1.0)
      );
      col = mix(oceanBg, col, clamp(edgeBlend*1.06, 0.0, 1.0));
      col += oceanBg * (1.0-edgeBlend) * 0.22;
...
function applyParamsToUniforms() {
  ...
  uniforms.u_noiseMode.value = params.noiseMode;
